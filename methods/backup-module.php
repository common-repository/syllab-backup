...
abstract class SyllabPlus_BackupModule {
	...
	private function save_options() {
		...
		$current_db_options = SyllabPlus_Storage_Methods_Interface::update_remote_storage_options_format($this->get_id());
		...
		return SyllabPlus_Options::update_syllab_option('syllab_'.$this->get_id(), $current_db_options);
	 ...
	 *
	 * This assumes standardised options handling (i.e. that the options array is syllab_(method-id))
	 *
	...
	public function output_settings_field_name_and_id($field, $return_instead_of_echo = false) {
		...
		$output = "id=\"syllab_${method_id}${id}_${instance_id}\" name=\"syllab_${method_id}[settings][${instance_id}]${name}\" ";
	...
	public function get_css_id($field) {
		$method_id = $this->get_id();
		$instance_id = $this->supports_feature('config_templates') ? '{{instance_id}}' : $this->_instance_id;
		return "syllab_${method_id}_${field}_${instance_id}";
	}
	...
	public function get_template() {
		...
		if ($this->print_shared_settings_fields()) {
			?><tr class="<?php echo $this->get_css_classes(); ?>"><input type="hidden" name="syllab_<?php echo $this->get_id();?>[version]" value="1"></tr><?php
		}
		...
		if ($this->supports_feature('config_templates')) {
			...
			<?php
				...
				if ($this->supports_feature('multi_storage')) {
					do_action('syllabplus_config_print_add_multi_storage', $this->get_id(), $this);
				}
				
			?>
			{{/if}}
			<?php
			do_action('syllabplus_config_print_before_storage', $this->get_id(), $this);
			if ('syllabvault' !== $this->get_id()) do_action('syllabplus_config_print_add_conditional_logic', $this->get_id(), $this);
			if ($this->supports_feature('multi_storage')) {
				do_action('syllabplus_config_print_add_instance_label', $this->get_id(), $this);
			}

			$template = ob_get_clean();
			$template .= $this->get_configuration_template();
			if ('syllabvault' === $this->get_id()) {
				ob_start();
				do_action('syllabplus_config_print_add_conditional_logic', $this->get_id(), $this);
				$template .= ob_get_clean();
			}
		} else {
			do_action('syllabplus_config_print_before_storage', $this->get_id(), $this);
			do_action('syllabplus_config_print_add_conditional_logic', $this->get_id(), $this);
			// N.B. These are mutually exclusive: config_print() is not used if config_templates is supported. So, even during transition, the SyllabPlus_BackupModule instance only needs to support one of the two, not both.
			$this->config_print();
			$template = ob_get_clean();
		}
		return $template;
	...
	public function get_credentials() {
		$keys = array('syllab_ssl_disableverify', 'syllab_ssl_nossl', 'syllab_ssl_useservercerts');
		if (!$this->supports_feature('multi_servers')) $keys[] = 'syllab_'.$this->get_id();
		return $keys;
	}
	...
	public function get_css_classes($include_instance = true) {
		$classes = 'syllabplusmethod '.$this->get_id();
	...
	protected function get_test_button_html($title) {
		...
		<tr class="<?php echo $this->get_css_classes(); ?>">
			<th></th>
			<td><p><button id="syllab-<?php echo $this->get_id();?>-test-<?php echo esc_html($instance_id);?>" type="button" class="button-primary syllab-test-button syllab-<?php echo $this->get_id();?>-test" data-instance_id="<?php echo esc_html($instance_id);?>" data-method="<?php echo $this->get_id();?>" data-method_label="<?php echo esc_attr($title);?>"><?php printf(__('Test %s Settings', 'syllabplus'), $title);?></button></p></td>
		</tr>
		<?php
		return ob_get_clean();
	}
	...
	public function get_id() {
		...
		// SyllabPlus_BackupModule_
		return substr($class, 24);
	}
	...
	public function get_description() {
		...
		$methods = $syllabplus->backup_methods;
	...
	public function get_options() {
		...
		if (is_array($this->_options)) {
			...
			$options = $syllabplus->get_job_option('syllab_'.$this->get_id());
			if (!is_array($options)) $options = array();

			if ($supports_multi_options) {

				if (!isset($options['version'])) {
					$options_full = SyllabPlus_Storage_Methods_Interface::update_remote_storage_options_format($this->get_id());
					
					if (is_wp_error($options_full)) {
						$syllabplus->log("Options retrieval failure: ".$options_full->get_error_code().": ".$options_full->get_error_message()." (".json_encode($options_full->get_error_data()).")");
						return array();
					}
				...
				// SyllabPlus_BackupModule::get_options() is for getting the current instance's options. So, this branch (going via the job option) is a legacy route, and hence we just give back the first one. The non-legacy route is to call the set_options() method externally.
				$options = reset($options_full['settings']);

				if (false === $options) {
					$syllabplus->log("Options retrieval failure (no options set)");
					return array();
				}
		...
		$options = apply_filters(
			'syllabplus_backupmodule_get_options',
	...
	public function jobdata_set($key, $value) {
		...
		$instance_data = $syllabplus->jobdata_get($instance_key);
		
		if (!is_array($instance_data)) $instance_data = array();
		
		$instance_data[$key] = $value;
		
		$syllabplus->jobdata_set($instance_key, $instance_data);
	...
	public function jobdata_get($key, $default = null, $legacy_key = null) {
		...
		return is_string($legacy_key) ? $syllabplus->jobdata_get($legacy_key, $default) : $default;
	...
	public function jobdata_delete($key, $legacy_key = null) {
		...
		if (is_string($legacy_key)) $syllabplus->jobdata_delete($legacy_key);
	...
	public function get_authentication_link($echo_instead_of_return = true, $template_instead_of_notice = true) {
		...
		if ($this->output_account_warning()) {
			$account_warning = __('Ensure you are logged into the correct account before continuing.', 'syllabplus');
		}

		if ($template_instead_of_notice) {
			$instance_id = "{{instance_id}}";
			$text = sprintf(__("<strong>After</strong> you have saved your settings (by clicking 'Save Changes' below), then come back here once and follow this link to complete authentication with %s.", 'syllabplus'), $description);
		} else {
			$instance_id = $this->get_instance_id();
			$text = sprintf(__('Follow this link to authorize access to your %s account (you will not be able to backup to %s without it).', 'syllabplus'), $description, $description);
		}

		echo $account_warning . ' <a class="syllab_authlink" href="'.SyllabPlus_Options::admin_page_url().'?&action=syllabmethod-'.$id.'-auth&page=syllabplus&syllabplus_'.$id.'auth=doit&syllabplus_instance='.$instance_id.'" data-instance_id="'.$instance_id.'" data-remote_method="'.$id.'">'.$text.'</a>';
	...
	public function action_authenticate_storage() {
		if (isset($_GET['syllabplus_'.$this->get_id().'auth']) && 'doit' == $_GET['syllabplus_'.$this->get_id().'auth'] && !empty($_GET['syllabplus_instance'])) {
			$this->authenticate_storage((string) sanitize_text_field($_GET['syllabplus_instance']));
		}
	}
	...
	public function get_deauthentication_link($echo_instead_of_return = true) {
		...
		echo ' <a class="syllab_deauthlink" href="'.SyllabPlus_Options::admin_page_url().'?action=syllabmethod-'.$id.'-auth&page=syllabplus&syllabplus_'.$id.'auth=deauth&nonce='.wp_create_nonce($id.'_deauth_nonce').'&syllabplus_instance={{instance_id}}" data-instance_id="{{instance_id}}" data-remote_method="'.$id.'">'.sprintf(__("Follow this link to remove these settings for %s.", 'syllabplus'), $description).'</a>';
	...
	public function action_deauthenticate_storage() {
		if (isset($_GET['syllabplus_'.$this->get_id().'auth']) && 'deauth' == $_GET['syllabplus_'.$this->get_id().'auth'] && !empty($_GET['nonce']) && !empty($_GET['syllabplus_instance']) && wp_verify_nonce($_GET['nonce'], $this->get_id().'_deauth_nonce')) {
			$this->deauthenticate_storage(sanitize_text_field($_GET['syllabplus_instance']));
		}
	}
	...
	public function get_manual_authorisation_template() {
		...
		$template = "<div id='syllabplus_manual_authorisation_template_{$id}'>";
		$template .= "<strong>".sprintf(__('%s authentication:', 'syllabplus'), $description)."</strong>";
		$template .= "<p>".sprintf(__('If you are having problems authenticating with %s you can manually authorize here.', 'syllabplus'), $description)."</p>";
		$template .= "<p>".__('To complete manual authentication, at the orange SyllabPlus authentication screen select the "Having problems authenticating?" link, then copy and paste the code given here.', 'syllabplus')."</p>";
		$template .= "<label for='syllabplus_manual_authentication_data_{$id}'>".sprintf(__('%s authentication code:', 'syllabplus'), $description)."</label> <input type='text' id='syllabplus_manual_authentication_data_{$id}' name='syllabplus_manual_authentication_data_{$id}'>";
		$template .= "<p id='syllabplus_manual_authentication_error_{$id}'></p>";
		$template .= "<button type='button' data-method='{$id}' class='button button-primary' id='syllabplus_manual_authorisation_submit_{$id}'>".__('Complete manual authentication', 'syllabplus')."</button>";
		$template .= '<span class="syllabplus_spinner spinner">' . __('Processing', 'syllabplus') . '...</span>';
		$template .= "</div>";
	...
	/**
	 * This function is a wrapper and will call $syllabplus->log(), the backup modules should use this so we can add information to the log lines to do with the remote storage and instance settings.
	 *
	...
	public function log($line, $level = 'notice', $uniq_id = false, $skip_dblog = false) {
		...
		$syllabplus->log("$prefix: $line", $level, $uniq_id, $skip_dblog);
	}
