...
if (!class_exists('SyllabPlus_RemoteStorage_Addons_Base_v2')) require_once(SYLLABPLUS_DIR.'/methods/addon-base-v2.php');

class SyllabPlus_BackupModule_remotesend extends SyllabPlus_RemoteStorage_Addons_Base_v2 {
	...
	public function get_credentials() {
		return array('syllab_ssl_disableverify', 'syllab_ssl_nossl', 'syllab_ssl_useservercerts');
	}
	...
	public function do_upload($file, $from) {
		...
		if (!is_array($get_remote_size) || empty($get_remote_size['response'])) throw new Exception(__('Unexpected response:', 'syllabplus').' '.serialize($get_remote_size));
		...
		if ('error' == $get_remote_size['response']) {
			...
			throw new Exception(__('Error:', 'syllabplus').' '.$msg);
		}

		if (empty($get_remote_size['data']) || !isset($get_remote_size['data']['size']) || 'file_status' != $get_remote_size['response']) throw new Exception(__('Unexpected response:', 'syllabplus').' '.serialize($get_remote_size));
		...
		if (empty($this->default_chunk_size)) {
			...
			$default_chunk_size = $syllabplus->jobdata_get('clone_job') ? 4194304 : 2097152;
			
			if (defined('SYLLABPLUS_REMOTESEND_DEFAULT_CHUNK_BYTES') && SYLLABPLUS_REMOTESEND_DEFAULT_CHUNK_BYTES >= 16384) $default_chunk_size = SYLLABPLUS_REMOTESEND_DEFAULT_CHUNK_BYTES;
		...
		$this->remotesend_use_chunk_size = $syllabplus->jobdata_get('remotesend_chunksize', $default_chunk_size);

		if (0 == $remote_size && $this->remotesend_use_chunk_size == $this->default_chunk_size && $syllabplus->current_resumption - max($syllabplus->jobdata_get('uploaded_lastreset'), 1) > 1) {
		...
		try {
			if (false != ($handle = fopen($from, 'rb'))) {
				...
				$ret = $syllabplus->chunked_upload($this, $file, $this->method."://".trailingslashit($opts['url']).$file, $this->description, $this->remotesend_use_chunk_size, $remote_size, true);
	...
	public function chunked_upload($file, $fp, $chunk_index, $upload_size, $upload_start, $upload_end) {
		...
		if ($upload_end+1 >= $this->remotesend_file_size) {
			...
			if ('' != ($label = $syllabplus->jobdata_get('label'))) $data['label'] = $label;
		}
			...
			if (is_wp_error($put_chunk)) {
				...
				if ($this->remotesend_use_chunk_size >= 131072 && ($is_413 || $is_timeout || ('response_not_understood' == $put_chunk->get_error_code() && (false !== strpos($error_data, 'wp-includes') || false !== strpos($error_data, 'wp-content') || false !== strpos($error_data, 'WordPress') || false !== strpos($put_chunk->get_error_data(), '/themes/'))))) {
					if (1 == $chunk_index) {
						...
						$this->log($log_msg);
						SyllabPlus_Job_Scheduler::reschedule(50);
		...
		if (!is_array($put_chunk) || empty($put_chunk['response'])) throw new Exception(__('Unexpected response:', 'syllabplus').' '.serialize($put_chunk));
		...
		if ('file_status' != $put_chunk['response']) throw new Exception(__('Unexpected response:', 'syllabplus').' '.serialize($put_chunk));
	...
	public function upload_completed() {
		...
		$service = $syllabplus->jobdata_get('service');
		...
		if (is_wp_error($storage)) return $syllabplus->log_wp_error($storage, false, true);

		for ($i = 0; $i < 3; $i++) {

			$success = false;

			$response = $this->send_message('upload_complete', array('job_id' => $syllabplus->nonce), 30);
	...
	private function remotesend_set_new_chunk_size($new_chunk_size) {
		...
		$syllabplus->jobdata_set('remotesend_chunksize', $new_chunk_size);
	...
	public function do_bootstrap($opts, $connect = true) {// phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable -- $connect unused
		...
		if (!class_exists('SyllabPlus_Remote_Communications')) include_once(apply_filters('syllabplus_class_slrpc_path', SYLLABPLUS_DIR.'/includes/class-slrpc.php', $syllabplus->version));

		$opts = $this->get_opts();

		try {
			$ud_rpc = new SyllabPlus_Remote_Communications($opts['name_indicator']);
	...
	public function get_opts() {
		global $syllabplus;
		$opts = $syllabplus->jobdata_get('remotesend_info');
	...
	public function clone_remotesend_options($opts) {
		...
		$syllabplus->jobdata_set('jobstatus', 'clonepolling');
		$clone_id = $syllabplus->jobdata_get('clone_id');
		$clone_url = $syllabplus->jobdata_get('clone_url');
		$clone_key = $syllabplus->jobdata_get('clone_key');
		...
		if (!isset($response['status']) || 'success' != $response['status']) {
			$syllabplus->log("SyllabClone migration information poll failed with code: " . $response['code']);
			return $opts;
		}

		if (!isset($response['data']) || !isset($response['data']['url']) || !isset($response['data']['key'])) {
			$syllabplus->log("SyllabClone migration information poll unexpected return information with code:" . $response['code']);
			return $opts;
		}
		...
		if (empty($clone_url) || empty($clone_key)) {
			$syllabplus->log("SyllabClone migration information not found (probably still provisioning): will poll again in 60");
		...
		$syllabplus->jobdata_set_multi('clone_url', $clone_url, 'clone_key', $clone_key, 'remotesend_info', $clone_key, 'jobstatus', 'clouduploading');
