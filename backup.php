...
if (!class_exists('SyllabPlus_PclZip')) require_once(SYLLABPLUS_DIR.'/includes/class-zip.php');

/**
 * This file contains code that is only needed/loaded when a backup is running
 */
class SyllabPlus_Backup {
	...
	private $use_zip_object = 'SyllabPlus_ZipArchive';
	...
	public function __construct($backup_files, $altered_since = -1) {
		...
		$this->debug = SyllabPlus_Options::get_syllab_option('syllab_debug_mode');
		$this->syllab_dir = $syllabplus->backups_dir_location();
		$this->syllab_dir_realpath = realpath($this->syllab_dir);

		require_once(SYLLABPLUS_DIR.'/includes/class-database-utility.php');

		if ('no' === $backup_files) {
			$this->use_zip_object = 'SyllabPlus_PclZip';
			return;
		}

		$this->extensions_to_not_compress = array_unique(array_map('strtolower', array_map('trim', explode(',', SYLLABPLUS_ZIP_NOCOMPRESS))));
		...
		$resumptions_since_last_successful = $syllabplus->current_resumption - $syllabplus->last_successful_resumption;
		...
		if (0 === $this->binzip && (!defined('SYLLABPLUS_PREFERPCLZIP') || !SYLLABPLUS_PREFERPCLZIP) && (!defined('SYLLABPLUS_NO_BINZIP') || !SYLLABPLUS_NO_BINZIP) && ($syllabplus->current_resumption < 9 || $resumptions_since_last_successful < 2)) {
			...
			if (@file_exists('/proc/user_beancounters') && @file_exists('/proc/meminfo') && @is_readable('/proc/meminfo')) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
				...
				if (is_string($meminfo) && preg_match('/MemTotal:\s+(\d+) kB/', $meminfo, $matches)) {
					...
					$syllabplus->log($vz_log);
				}
			}
			if (empty($openvz_lowmem)) {
				$syllabplus->log('Checking if we have a zip executable available');
				$binzip = $syllabplus->find_working_bin_zip();
				if (is_string($binzip)) {
					$syllabplus->log("Zip engine: found/will use a binary zip: $binzip");
					$this->binzip = $binzip;
					$this->use_zip_object = 'SyllabPlus_BinZip';
				}
			}
		}

		// In tests, PclZip was found to be 25% slower than ZipArchive
		if ('SyllabPlus_PclZip' != $this->use_zip_object && empty($this->binzip) && ((defined('SYLLABPLUS_PREFERPCLZIP') && SYLLABPLUS_PREFERPCLZIP == true) || !class_exists('ZipArchive') || !class_exists('SyllabPlus_ZipArchive') || (!extension_loaded('zip') && !method_exists('ZipArchive', 'AddFile')))) {
			global $syllabplus;
			$syllabplus->log("Zip engine: ZipArchive (a.k.a. php-zip) is not available or is disabled (will use PclZip (much slower) if needed)");
			$this->use_zip_object = 'SyllabPlus_PclZip';
		}
		
		$this->zip_batch_ceiling = (defined('SYLLABPLUS_ZIP_BATCH_CEILING') && SYLLABPLUS_ZIP_BATCH_CEILING > 104857600) ? SYLLABPLUS_ZIP_BATCH_CEILING : 200 * 1048576;

		add_filter('syllabplus_exclude_file', array($this, 'backup_exclude_file'), 10, 2);
	...
	private function get_site_name() {
		...
		return apply_filters('syllabplus_blog_name', $site_name);
	}
	...
	public function create_zip($create_from_dir, $whichone, $backup_file_basename, $index, $first_linked_index = false) {
		...
		if (function_exists('set_time_limit')) set_time_limit(SYLLABPLUS_SET_TIME_LIMIT);
		...
		$this->zip_split_every = max((int) $syllabplus->jobdata_get('split_every'), SYLLABPLUS_SPLIT_MIN)*1048576;

		if ('others' != $whichone) $syllabplus->log("Beginning creation of dump of $whichone (split every: ".round($this->zip_split_every/1048576, 1)." MB)");

		if (is_string($create_from_dir) && !file_exists($create_from_dir)) {
			$flag_error = true;
			$syllabplus->log("Does not exist: $create_from_dir");
			if ('mu-plugins' == $whichone) {
				...
				if (count($mu_plugins) == 0) {
					$syllabplus->log("There appear to be no mu-plugins to backup. Will not raise an error.");
					$flag_error = false;
				}
			}
			if ($flag_error) $syllabplus->log(sprintf(__("%s - could not back this entity up; the corresponding directory does not exist (%s)", 'syllabplus'), $whichone, $create_from_dir), 'error');
			return false;
		}

		$itext = empty($index) ? '' : $index+1;
		$base_path = $backup_file_basename.'-'.$whichone.$itext.'.zip';
		$full_path = $this->syllab_dir.'/'.$base_path;
		$time_now = time();
		...
		if (file_exists($full_path)) {
			...
			while (file_exists($full_path)) {
				...
				$syllabplus->log($base_path.": this file has already been created (age: ".round($time_now-$time_mod, 1)." s)");
				if ($time_mod > 100 && ($time_now - $time_mod) < 30) {
					SyllabPlus_Job_Scheduler::terminate_due_to_activity($base_path, $time_now, $time_mod);
				}
				$index++;
				// This is compatible with filenames which indicate increments, as it is looking only for the current increment
		...
		SyllabPlus_Filesystem_Functions::clean_temporary_files('_'.$syllabplus->file_nonce."-$whichone", 600);
		...
		if (file_exists($zip_name) && $time_mod>100 && ($time_now-$time_mod)<30) {
			SyllabPlus_Job_Scheduler::terminate_due_to_activity($zip_name, $time_now, $time_mod);
		}

		if (file_exists($zip_name)) {
			$syllabplus->log("File exists ($zip_name), but was apparently not modified within the last 30 seconds, so we assume that any previous run has now terminated (time_mod=$time_mod, time_now=$time_now, diff=".($time_now-$time_mod).")");
		}
		...
		$d = dir($this->syllab_dir);
		$match = '_'.$syllabplus->file_nonce."-".$whichone;
		while (false !== ($e = $d->read())) {
			if ('.' == $e || '..' == $e || !is_file($this->syllab_dir.'/'.$e)) continue;
			...
			if ($time_now-filemtime($this->syllab_dir.'/'.$e) < 30 && ($ziparchive_match || (0 != $syllabplus->current_resumption && ($binzip_match || $pclzip_match)))) {
				SyllabPlus_Job_Scheduler::terminate_due_to_activity($this->syllab_dir.'/'.$e, $time_now, filemtime($this->syllab_dir.'/'.$e));
			}
		}
		...
		if (true !== $zipcode) {
			$syllabplus->log("ERROR: Zip failure: Could not create $whichone zip (".$this->index." / $index)");
			$syllabplus->log(sprintf(__("Could not create %s zip. Consult the log file for more information.", 'syllabplus'), $whichone), 'error');
			// The caller is required to update $index from $this->index
			return false;
		} else {
			$itext = empty($this->index) ? '' : $this->index+1;
			$full_path = $this->syllab_dir.'/'.$backup_file_basename.'-'.$whichone.$itext.'.zip';
			if (file_exists($full_path.'.tmp')) {
				if (@filesize($full_path.'.tmp') === 0) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
					$syllabplus->log("Did not create $whichone zip (".$this->index.") - not needed");
					...
					foreach ($checksums as $checksum) {
						...
						$syllabplus->jobdata_set($checksum.'-'.$whichone.$this->index, $cksum);
					...
					$syllabplus->log("Created $whichone zip (".$this->index.") - ".round($kbsize, 1)." KB in ".round($timetaken, 1)." s ($rate KB/s) ($checksum_description)");
					// We can now remove any left-over temporary files from this job
				}
			} elseif ($this->index > $original_index) {
				$syllabplus->log("Did not create $whichone zip (".$this->index.") - not needed (2)");
				// Added 12-Feb-2014 (to help multiple morefiles)
				$this->index--;
			} else {
				$syllabplus->log("Looked-for $whichone zip (".$this->index.") was not found (".basename($full_path).".tmp)", 'warning');
			}
			SyllabPlus_Filesystem_Functions::clean_temporary_files('_'.$syllabplus->file_nonce."-$whichone", 0);
		}

		// Remove cache list files as well, if there are any
		SyllabPlus_Filesystem_Functions::clean_temporary_files('_'.$syllabplus->file_nonce."-$whichone", 0, true);
	...
	public function do_prune_standalone() {
		...
		$services = (array) $syllabplus->just_one($syllabplus->jobdata_get('service'));
		...
		foreach ($services as $service) {
			...
			$objname = "SyllabPlus_BackupModule_${service}";
			if (!class_exists($objname) && file_exists(SYLLABPLUS_DIR.'/methods/'.$service.'.php')) {
				include_once(SYLLABPLUS_DIR.'/methods/'.$service.'.php');
			}
			if (class_exists($objname)) {
				$remote_obj = new $objname;
				$prune_services[$service]['all'] = array($remote_obj, null);
			} else {
				$syllabplus->log("Could not prune from service $service: remote method not found");
			}
	...
	public function cloud_backup($backup_array) {
		...
		$services = (array) $syllabplus->just_one($syllabplus->jobdata_get('service'));
		$remote_storage_instances = $syllabplus->jobdata_get('remote_storage_instances', array());

		$serlog = implode(" ", $services);
		$remoteStolog = implode(" ", $remote_storage_instances);

		$services = (array) SyllabPlus_Options::get_syllab_option('syllab_service');

        $syllabplus->log('custom log--');
		$syllabplus->log($services);
		$syllabplus->log($remoteStolog);
		...
		// echo "--------------------";
		// echo "<pre>";
		// print_r($remote_storage_instances);
		// die();

		// We need to make sure that the loop below actually runs
		if (empty($services)) $services = array('none');
		
		$storage_objects_and_ids = SyllabPlus_Storage_Methods_Interface::get_enabled_storage_objects_and_ids($services, $remote_storage_instances);
		...
		$syllabplus->jobdata_set('jobstatus', 'clouduploading');

		$syllabplus->register_wp_http_option_hooks();

		$upload_status = $syllabplus->jobdata_get('uploading_substatus');
		if (!is_array($upload_status) || !isset($upload_status['t'])) {
			...
			$syllabplus->jobdata_set('uploading_substatus', $upload_status);
		}
		...
		if (count($services) >1 && !empty($syllabplus->no_checkin_last_time)) {
			$syllabplus->log('No check-in last time: will try a different remote service first');
			...
			if ($syllabplus->current_resumption > 9) $syllabplus->jobdata_set('uploaded_lastreset', $syllabplus->current_resumption);
			if (1 == ($syllabplus->current_resumption % 2) && count($services)>2) array_push($services, array_shift($services));
		}

		$errors_before_uploads = $syllabplus->error_count();
		...
		foreach ($services as $ind => $service) {
			try {
				...
				$this->last_storage_instance = ($ind+1 >= count($services) && $instance_id_count+1 >= $total_instance_ids && $errors_before_uploads == $syllabplus->error_count()) ? true : false;
				$log_extra = $this->last_storage_instance ? ' (last)' : '';
				$syllabplus->log("Cloud backup selection (".($ind+1)."/".count($services)."): ".$service." with instance (".($instance_id_count+1)."/".$total_instance_ids.")".$log_extra);
				...
				if ('none' == $service || '' == $service) {
					$syllabplus->log('No remote despatch: user chose no remote backup service');
					...
					foreach ($backup_array as $file) {
						if ($syllabplus->is_uploaded($file)) {
							$syllabplus->log("Already uploaded: $file");
						} else {
							$syllabplus->uploaded_file($file, true);
						}
					}
				...
				//	$do_prune = array_merge_recursive($do_prune, $this->upload_cloud($remote_obj, $service, $backup_array, ''));
					...
					foreach ($storage_objects_and_ids[$service]['instance_settings'] as $instance_id => $options) {
						...
						if (1 == $options['instance_enabled'] || !empty($remote_storage_instances)) {
							...
							$syllabplus->log("This instance id ($instance_id) is set as inactive.");
						}
				...
				$syllabplus->log($log_message);
				error_log($log_message);
				$syllabplus->log(sprintf(__('A PHP exception (%s) has occurred: %s', 'syllabplus'), get_class($e), $e->getMessage()), 'error');
			// @codingStandardsIgnoreLine
				...
				$syllabplus->log($log_message);
				error_log($log_message);
				$syllabplus->log(sprintf(__('A PHP fatal error (%s) has occurred: %s', 'syllabplus'), get_class($e), $e->getMessage()), 'error');
			}
		}
	...
	private function upload_cloud($remote_obj, $service, $backup_array, $instance_id) {
		...
		//$syllabplus->log("My Custom Log Remote Obj".$remote_obj);
		...
		if ('' == $instance_id) {
			$syllabplus->log("Beginning dispatch of backup to remote ($service)");
		} else {
			$syllabplus->log("Beginning dispatch of backup to remote ($service) (instance identifier $instance_id)");
		}
		...
		foreach ($backup_array as $bind => $file) {
			if ($syllabplus->is_uploaded($file, $service, $instance_id)) {
				if ('' == $instance_id) {
					$syllabplus->log("Already uploaded to $service: $file", 'notice', false, true);
				} else {
					$syllabplus->log("Already uploaded to $service / $instance_id: $file", 'notice', false, true);
				}
			} else {
				$sarray[$bind] = $file;
			}
		...
		if (count($sarray) > 0) {
			...
			if ($errors_before_uploads == $syllabplus->error_count()) {
				if (is_callable(array($remote_obj, 'upload_completed'))) {
					$result = $remote_obj->upload_completed();
					if ($result) $syllabplus->mark_upload_complete($service);
				} else {
					$syllabplus->mark_upload_complete($service);
				}
			}
		} else {
	...
	private function log_with_db_occasionally($message, $level = 'notice') {
		...
		return $syllabplus->log($message, $level, false, $skip_dblog);
	}
	...
	public function prune_retained_backups($services) {
		...
		global $syllabplus, $wpdb;

		if ('' != $syllabplus->jobdata_get('remotesend_info')) {
			$syllabplus->log("Prune old backups from local store: skipping, as this was a remote send operation");
			return;
		}

		if (method_exists($wpdb, 'check_connection') && (!defined('SYLLABPLUS_SUPPRESS_CONNECTION_CHECKS') || !SYLLABPLUS_SUPPRESS_CONNECTION_CHECKS)) {
			if (!$wpdb->check_connection(false)) {
				SyllabPlus_Job_Scheduler::reschedule(60);
				$syllabplus->log('It seems the database went away; scheduling a resumption and terminating for now');
		...
		if (!SyllabPlus_Options::get_syllab_option('syllab_delete_local', 1) && 1 == count($services) && array_key_exists('none', $services)) {
			$syllabplus->log("Prune old backups from local store: nothing to do, since the user disabled local deletion and we are using local backups");
			return;
		}

		$syllabplus->jobdata_set_multi(array('jobstatus' => 'pruning', 'prune' => 'begun'));

		// Number of backups to retain - files
		$syllab_retain = SyllabPlus_Options::get_syllab_option('syllab_retain', 2);
		$syllab_retain = is_numeric($syllab_retain) ? $syllab_retain : 1;

		// Number of backups to retain - db
		$syllab_retain_db = SyllabPlus_Options::get_syllab_option('syllab_retain_db', $syllab_retain);
		$syllab_retain_db = is_numeric($syllab_retain_db) ? $syllab_retain_db : 1;

		$syllabplus->log("Retain: beginning examination of existing backup sets; user setting: retain_files=$syllab_retain, retain_db=$syllab_retain_db");
		...
		if (!empty($ignored_because_imported)) {
			$syllabplus->log("These backup set(s) were imported from a remote location, so will not be counted or pruned. Skipping: ".implode(', ', $ignored_because_imported));
		}
		...
		if (false === ($backup_db_groups = apply_filters('syllabplus_group_backups_for_pruning', false, $functional_backup_history, 'db'))) {
			$backup_db_groups = $this->group_backups($functional_backup_history);
		}
		$syllabplus->log("Number of backup sets in history: ".count($backup_history)."; groups (db): ".count($backup_db_groups));

		foreach ($backup_db_groups as $group_id => $group) {
			
			// N.B. The array returned by SyllabPlus_Backup_History::get_history() is already sorted, with most-recent first
			...
			foreach ($sets as $backup_datestamp => $backup_to_examine) {
				...
				foreach ($backup_to_examine as $key => $data) {
					...
					if ($nonce == $syllabplus->nonce || $nonce == $syllabplus->file_nonce) {
					...
					if ($is_always_keep) {
						if ($database_backups_found[$key] < $syllab_retain) {
					...
					if ($is_autobackup) {
						if ($any_deleted_via_filter_yet) {
							...
							$prune_it = true;
						} elseif ($database_backups_found[$key] < $syllab_retain_db) {
					...
					if (!$is_autobackup) $prune_it = apply_filters('syllabplus_prune_or_not', $prune_it, 'db', $backup_datestamp, $key, $database_backups_found[$key], $rule, $group_id);
					...
					if (!$prune_it && !$is_autobackup) {
						...
						if ($database_backups_found[$key] + 1 > $syllab_retain_db) {
							$prune_it = true;
							...
							$this->log_with_db_occasionally("$backup_datestamp: $key: over retain limit ($syllab_retain_db); will delete this database");
						}
							...
							foreach ($services as $service => $instance_ids_to_prune) {
								foreach ($instance_ids_to_prune as $instance_id_to_prune => $sd) {
									if ('none' != $service && '' != $service && $sd[0]->supports_feature('multi_options')) {
										$storage_objects_and_ids = SyllabPlus_Storage_Methods_Interface::get_storage_objects_and_ids(array($service));
		...
		if (false === ($backup_files_groups = apply_filters('syllabplus_group_backups_for_pruning', false, $functional_backup_history, 'files'))) {
			$backup_files_groups = $this->group_backups($functional_backup_history);
		}

		$syllabplus->log("Number of backup sets in history: ".count($backup_history)."; groups (files): ".count($backup_files_groups));
			...
			foreach ($sets as $backup_datestamp => $backup_to_examine) {
				...
				foreach ($backupable_entities as $entity => $info) {
					if (!empty($backup_to_examine[$entity])) {
						...
						if ($nonce == $syllabplus->nonce || $nonce == $syllabplus->file_nonce) {
							$syllabplus->log("This backup set is the backup set just made, so will not be deleted.");
							$file_entities_backups_found[$entity]++;
							continue;
						}

						if ($is_always_keep) {
							if ($file_entities_backups_found[$entity] < $syllab_retain) {
								$this->log_with_db_occasionally("This backup set ($backup_datestamp) was an 'Always Keep' backup, and we have not yet reached any retain limits, so it should be counted in the count of how many have been retained for purposes of the 'how many to retain' count. It will not be pruned. Skipping.");
								$file_entities_backups_found[$entity]++;
							} else {
								$this->log_with_db_occasionally("This backup set ($backup_datestamp) was an 'Always Keep' backup, so it will not be pruned. Skipping.");
						...
						if ($is_autobackup) {
							if ($any_deleted_via_filter_yet) {
								...
								$prune_it = true;
							} elseif ($file_entities_backups_found[$entity] < $syllab_retain) {
								$this->log_with_db_occasionally("This backup set ($backup_datestamp) was an automatic backup, and we have not yet reached any retain limits, so it will not be counted or pruned. Skipping.");
								continue;
							} else {
								$this->log_with_db_occasionally("This backup set ($backup_datestamp) was an automatic backup, and we have already reached retain limits, so it will be pruned.");
						...
						// All non-auto backups must be run through this filter (in date order) regardless of the current state of $prune_it - so that filters are able to track state.
						$prune_it_before_filter = $prune_it;
						if (!$is_autobackup) $prune_it = apply_filters('syllabplus_prune_or_not', $prune_it, 'files', $backup_datestamp, $entity, $file_entities_backups_found[$entity], $rule, $group_id);
						...
						if (!$prune_it && !$is_autobackup) {
							if ($file_entities_backups_found[$entity] >= $syllab_retain) {
								$this->log_with_db_occasionally("$entity: over retain limit ($syllab_retain); will delete this file entity");
								$prune_it = true;
							}
						}
						...
						if ($prune_it) {
							...
							foreach ($prune_this as $k => $prune_file) {
								if ($remote_sent) {
									$syllabplus->log("$entity: $backup_datestamp: was sent to remote site; will remove from local record (only)");
								}
...
// $functional_backup_history[$backup_datestamp] = $backup_to_examine;
		...
		$syllabplus->log("Retain: saving new backup history (sets now: ".count($backup_history).") and finishing retain operation");
		SyllabPlus_Backup_History::save_history($backup_history, false);

		do_action('syllabplus_prune_retained_backups_finished');
		
		$syllabplus->jobdata_set('prune', 'finished');
	...
	private function maybe_save_backup_history_and_reschedule($backup_history) {
		...
		if (time() - $last_saved_at >= 10) {
			global $syllabplus;
			$syllabplus->log("Retain: saving new backup history, because at least 10 seconds have passed since the last save (sets now: ".count($backup_history).")");
			SyllabPlus_Backup_History::save_history($backup_history, false);
			SyllabPlus_Job_Scheduler::something_useful_happened();
			$last_saved_at = time();
		}
	}
	...
	private function remove_backup_set_if_empty($backup_to_examine, $backupable_entities) {
		...
		if (!$contains_files && !$contains_db) {
			$syllabplus->log("This backup set is now empty; will remove from history");
			if (isset($backup_to_examine['nonce'])) {
				$fullpath = $this->syllab_dir."/log.".$backup_to_examine['nonce'].".txt";
				if (is_file($fullpath)) {
					$syllabplus->log("Deleting log file (log.".$backup_to_examine['nonce'].".txt)");
					@unlink($fullpath);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
				} else {
					$syllabplus->log("Corresponding log file (log.".$backup_to_examine['nonce'].".txt) not found - must have already been deleted");
				}
			} else {
				$syllabplus->log("No nonce record found in the backup set, so cannot delete any remaining log file");
			}
			return false;
		} else {
			$syllabplus->log("This backup set remains non-empty (f=$contains_files/d=$contains_db); will retain in history");
	...
	private function prune_file($service, $dofiles, $method_object = null, $object_passback = null, $file_sizes = array()) {
		...
		if (!apply_filters('syllabplus_prune_file', true, $dofiles, $service, $method_object, $object_passback, $file_sizes)) {
			$syllabplus->log("Prune: service=$service: skipped via filter");
		}
		
		foreach ($dofiles as $dofile) {
			if (empty($dofile)) continue;
			$syllabplus->log("Delete file: $dofile, service=$service");
			$fullpath = $this->syllab_dir.'/'.$dofile;
			// delete it if it's locally available
			if (file_exists($fullpath)) {
				$syllabplus->log("Deleting local copy ($dofile)");
				unlink($fullpath);
			}
		}
	...
	private function backup_db_sorttables($a_arr, $b_arr) {
		...
		global $syllabplus;
		$na = SyllabPlus_Manipulation_Functions::str_replace_once($our_table_prefix, '', $a);
		$nb = SyllabPlus_Manipulation_Functions::str_replace_once($our_table_prefix, '', $b);
	...
	private function log_account_space() {
		...
		global $syllabplus;
		$hosting_bytes_free = $syllabplus->get_hosting_disk_quota_free();
		if (is_array($hosting_bytes_free)) {
			$perc = round(100*$hosting_bytes_free[1]/(max($hosting_bytes_free[2], 1)), 1);
			$syllabplus->log(sprintf('Free disk space in account: %s (%s used)', round($hosting_bytes_free[3]/1048576, 1)." MB", "$perc %"));
		}
	}
	...
	private function get_backup_file_basename_from_time($use_time) {
		global $syllabplus;
		return apply_filters('syllabplus_get_backup_file_basename_from_time', 'backup_'.get_date_from_gmt(gmdate('Y-m-d H:i:s', $use_time), 'Y-m-d-Hi').'_'.$this->site_name.'_'.$syllabplus->file_nonce, $use_time, $this->site_name);
	}
	...
	private function backup_dirs($job_status) {
		...
		if (!$syllabplus->backup_time) $syllabplus->backup_time_nonce();

		$use_time = $syllabplus->backup_time;
		...
		if ('finished' != $job_status && $syllabplus->current_resumption >= 2) {
			...
			// 03-Sep-2015 - came across a case (HS#2052) where there apparently was a check-in 'last time', but no resumption was scheduled because the 'useful_checkin' jobdata was *not* last time - which must indicate dying at a very unfortunate/unlikely point in the code. As a result, the split was not auto-reduced. Consequently, we've added !$syllabplus->newresumption_scheduled as a condition on the first check here (it was already on the second), as if no resumption is scheduled then whatever checkin there was last time was only partial. This was on GoDaddy, for which a number of curious I/O event combinations have been seen in recent months - their platform appears to have some odd behaviour when PHP is killed off.
			// 04-Sep-2015 - move the '$syllabplus->current_resumption<=10' check to the inner loop (instead of applying to this whole section), as I see no reason for that restriction (case seen in HS#2064 where it was required on resumption 15)
			if (!empty($syllabplus->no_checkin_last_time) || !$syllabplus->newresumption_scheduled) {
				// Apr 2015: !$syllabplus->newresumption_scheduled added after seeing a log where there was no activity on resumption 9, and extra resumption 10 then tried the same operation.
				if ($syllabplus->current_resumption - $syllabplus->last_successful_resumption > 2 || !$syllabplus->newresumption_scheduled) {
					$this->try_split = true;
				} elseif ($syllabplus->current_resumption <= 10) {
					$maxzipbatch = $syllabplus->jobdata_get('maxzipbatch', 26214400);
					...
					if ($new_maxzipbatch < $maxzipbatch) {
						$syllabplus->log("No check-in was detected on the previous run - as a result, we are reducing the batch amount (old=$maxzipbatch, new=$new_maxzipbatch)");
						$syllabplus->jobdata_set('maxzipbatch', $new_maxzipbatch);
						$syllabplus->jobdata_set('maxzipbatch_ceiling', $new_maxzipbatch);
					}
				}
			}
		}

		if ('finished' != $job_status && !SyllabPlus_Filesystem_Functions::really_is_writable($this->syllab_dir)) {
			$syllabplus->log("Backup directory (".$this->syllab_dir.") is not writable, or does not exist");
			$syllabplus->log(sprintf(__("Backup directory (%s) is not writable, or does not exist.", 'syllabplus'), $this->syllab_dir), 'error');
			return array();
		}

		$this->job_file_entities = $syllabplus->jobdata_get('job_file_entities');
		...
		$existing_zips = $this->find_existing_zips($this->syllab_dir, $syllabplus->file_nonce);

		$possible_backups = $syllabplus->get_backupable_file_entities(true);

		foreach ($possible_backups as $youwhat => $whichdir) {

			if (!isset($this->job_file_entities[$youwhat])) {
				$syllabplus->log("No backup of $youwhat: excluded by user's options");
				continue;
			}
			...
			$zip_file = $this->syllab_dir.'/'.$backup_file_basename.'-'.$youwhat.$indextext.'.zip';

			// Split needed?
			$split_every = max((int) $syllabplus->jobdata_get('split_every'), 250);

			if (false != ($existing_file = $this->file_exists($existing_zips, $youwhat, $index)) && filesize($this->syllab_dir.'/'.$existing_file) > $split_every*1048576) {
				$index++;
				$this->job_file_entities[$youwhat]['index'] = $index;
				$syllabplus->jobdata_set('job_file_entities', $this->job_file_entities);
			}
			...
			if ('finished' == $job_status) {
				...
				$which_entity++;
				$syllabplus->jobdata_set('filecreating_substatus', array('e' => $youwhat, 'i' => $which_entity, 't' => count($this->job_file_entities)));

				if ('others' == $youwhat) $syllabplus->log("Beginning backup of other directories found in the content directory (index: $index)");

				// Apply a filter to allow add-ons to provide their own method for creating a zip of the entity
				$created = apply_filters('syllabplus_backup_makezip_'.$youwhat, $whichdir, $backup_file_basename, $index);
				...
				if ($created === $whichdir) {
					...
					*/
					...
					if ('others' == $youwhat) {
						$dirlist = $syllabplus->backup_others_dirlist(true);
					} elseif ('uploads' == $youwhat) {
						$dirlist = $syllabplus->backup_uploads_dirlist(true);
					} else {
						$dirlist = $whichdir;
					...
					if (!empty($dirlist)) {
						...
						if (!is_string($created) && !is_array($created)) $syllabplus->log("$youwhat: create_zip returned an error");
					} else {
						$syllabplus->log("No backup of $youwhat: there was nothing found to backup");
					}
				}
				...
				if ($created != $whichdir && (is_string($created) || is_array($created))) {
					...
					foreach ($created as $fname) {
						...
						$itext = (0 == $index) ? '' : $index;
						$index++;
						$backup_array[$youwhat.$itext.'-size'] = filesize($this->syllab_dir.'/'.$fname);
					}
				}
	...
	public function resumable_backup_of_files($resumption_no) {
		...
		$bfiles_status = $syllabplus->jobdata_get('backup_files');
		$this->backup_files_array = $syllabplus->jobdata_get('backup_files_array');

		if (!is_array($this->backup_files_array)) $this->backup_files_array = array();
		if ('finished' == $bfiles_status) {
			$syllabplus->log("Creation of backups of directories: already finished");
			// Check for recent activity
			foreach ($this->backup_files_array as $files) {
				if (!is_array($files)) $files =array($files);
				foreach ($files as $file) $syllabplus->check_recent_modification($this->syllab_dir.'/'.$file);
			}
		} elseif ('begun' == $bfiles_status) {
			$this->first_run = apply_filters('syllabplus_filerun_firstrun', 0);
			if ($resumption_no > $this->first_run) {
				$syllabplus->log("Creation of backups of directories: had begun; will resume");
			} else {
				$syllabplus->log("Creation of backups of directories: beginning");
			}
			$syllabplus->jobdata_set('jobstatus', 'filescreating');
			$this->backup_files_array = $this->backup_dirs($bfiles_status);
			$syllabplus->jobdata_set('backup_files_array', $this->backup_files_array);
			$syllabplus->jobdata_set('backup_files', 'finished');
			$syllabplus->jobdata_set('jobstatus', 'filescreated');
		} else {
			// This is not necessarily a backup run which is meant to contain files at all
			$syllabplus->log('This backup run is not intended for files - skipping');
			return array();
		}
		...
		foreach ($backup_array as $entity => $files) {
			...
			foreach ($files as $ind => $file) {
				...
				if (preg_match('/^(backup_[\-0-9]{15}_.*_[0-9a-f]{12}-[\-a-z]+)([0-9]+)?\.zip$/i', $file, $matches)) {
					...
					if (file_exists($this->syllab_dir.'/'.$file)) {
						if (@rename($this->syllab_dir.'/'.$file, $this->syllab_dir.'/'.$new)) {
							$syllabplus->log(sprintf("Renaming: %s to %s", $file, $new));
							$nval = $new;
						}
					} elseif (file_exists($this->syllab_dir.'/'.$new)) {
						$nval = $new;
					}
				}
	...
	public function backup_db($already_done = 'begun', $whichdb = 'wp', $dbinfo = array()) {
		...
		if (!$syllabplus->backup_time) $syllabplus->backup_time_nonce();
		if (!$syllabplus->opened_log_time) $syllabplus->logfile_open($syllabplus->nonce);

		if ('wp' == $this->whichdb) {
			$this->wpdb_obj = $wpdb;
			// The table prefix after being filtered - i.e. what filters what we'll actually backup
			$this->table_prefix = $syllabplus->get_table_prefix(true);
			// The unfiltered table prefix - i.e. the real prefix that things are relative to
			$this->table_prefix_raw = $syllabplus->get_table_prefix(false);
			$dbinfo['host'] = DB_HOST;
			$dbinfo['name'] = DB_NAME;
			$dbinfo['user'] = DB_USER;
			...
			$this->wpdb_obj = new SyllabPlus_WPDB_OtherDB($dbinfo['user'], $dbinfo['pass'], $dbinfo['name'], $dbinfo['host']);
			if (!empty($this->wpdb_obj->error)) {
				$syllabplus->log($dbinfo['user'].'@'.$dbinfo['host'].'/'.$dbinfo['name'].' : database connection attempt failed');
				$syllabplus->log($dbinfo['user'].'@'.$dbinfo['host'].'/'.$dbinfo['name'].' : '.__('database connection attempt failed.', 'syllabplus').' '.__('Connection failed: check your access details, that the database server is up, and that the network connection is not firewalled.', 'syllabplus'), 'error');
				return $syllabplus->log_wp_error($this->wpdb_obj->error);
			}
		...
		do_action('syllabplus_backup_db_begin', $whichdb, $dbinfo, $already_done, $this);

		SyllabPlus_Database_Utility::set_sql_mode(array(), array('ANSI_QUOTES'), $this->wpdb_obj);

		$errors = 0;

		$use_time = apply_filters('syllabplus_base_backup_timestamp', $syllabplus->backup_time);
		$file_base = $this->get_backup_file_basename_from_time($use_time);
		$backup_file_base = $this->syllab_dir.'/'.$file_base;
		...
		$syllabplus->jobdata_set('jobstatus', 'dbcreating'.$this->whichdb_suffix);

		$binsqldump = $syllabplus->find_working_sqldump();
		...
		if ('wp' == $whichdb && method_exists($this->wpdb_obj, 'check_connection') && (!defined('SYLLABPLUS_SUPPRESS_CONNECTION_CHECKS') || !SYLLABPLUS_SUPPRESS_CONNECTION_CHECKS)) {
		...
		if ('wp' == $whichdb && 0 == count($all_tables)) {
			$extra = ($syllabplus->newresumption_scheduled) ? ' - '.__('please wait for the rescheduled attempt', 'syllabplus') : '';
			$syllabplus->log("Error: No WordPress database tables found (SHOW TABLES returned nothing)".$extra);
			$syllabplus->log(__("No database tables found", 'syllabplus').$extra, 'error');
			die;
		}
		...
		if (!SyllabPlus_Filesystem_Functions::really_is_writable($this->syllab_dir)) {
			$syllabplus->log("The backup directory (".$this->syllab_dir.") could not be written to (could be account/disk space full, or wrong permissions).");
			$syllabplus->log($this->syllab_dir.": ".__('The backup directory is not writable (or disk space is full) - the database backup is expected to shortly fail.', 'syllabplus'), 'warning');
		...
		foreach ($all_table_names as $table) {
			if (strtolower($table) != $table && in_array(strtolower($table), $all_table_names)) {
				...
				$syllabplus->log("Tables with names differing only based on case-sensitivity exist in the MySQL database: $table / ".strtolower($table));
			}
		}
		...
		$anonymisation_options = $syllabplus->jobdata_get('anonymisation_options', array());

		if (!empty($anonymisation_options)) {
			$syllabplus->log("Anonymisation options have been set, so mysqldump (which does not support them) will be disabled.");
		}
		...
		if (false !== ($dir_handle = opendir($this->syllab_dir))) {
			while (false !== ($e = readdir($dir_handle))) {
				...
				if (!is_file($this->syllab_dir.'/'.$e)) continue;
			...
			$syllabplus->log("Error: Failed to open directory for reading");
			$syllabplus->log(__("Failed to open directory for reading:", 'syllabplus').' '.$this->syllab_dir, 'error');
		}
		
		$errors_at_all_tables_start = $syllabplus->error_count();
		...
		foreach ($all_tables as $ti) {
			...
			$table_type = $ti['type'];
			$errors_at_table_start = $syllabplus->error_count();
			...
			// Already finished?
			if (file_exists($this->syllab_dir.'/'.$table_file_prefix.'.gz')) {
				$stitched = count($stitch_files, COUNT_RECURSIVE);
				$skip_dblog = (($stitched > 10 && 0 != $stitched % 20) || ($stitched > 100 && 0 != $stitched % 100));
				$syllabplus->log("Table $table: corresponding file already exists; moving on", 'notice', false, $skip_dblog);
			...
			if (empty($this->table_prefix) || (!$this->duplicate_tables_exist && 0 === stripos($table, $this->table_prefix)) || ($this->duplicate_tables_exist && 0 === strpos($table, $this->table_prefix))) {
				...
				if (!apply_filters('syllabplus_backup_table', true, $table, $this->table_prefix, $whichdb, $dbinfo)) {
					$syllabplus->log("Skipping table (filtered): $table");
				...
				add_filter('syllabplus_backup_table_sql_where', array($this, 'backup_exclude_jobdata'), 3, 10);

				$syllabplus->jobdata_set('dbcreating_substatus', array('t' => $table, 'i' => $total_tables, 'a' => $how_many_tables));
				...
				$db_temp_file = $this->syllab_dir.'/'.$table_file_prefix.'.tmp.gz';
				$syllabplus->check_recent_modification($db_temp_file);
				...
				$this->stow("# " . sprintf('Table: %s', SyllabPlus_Manipulation_Functions::backquote($table)) . "\n");
				if (isset($table_status->Rows)) {
					$rows = $table_status->Rows;
					$syllabplus->log("Table $table: Total expected rows (approximate): ".$rows);
					$this->stow("# Approximate rows expected in table: $rows\n");
					if ($rows > SYLLABPLUS_WARN_DB_ROWS) {
						$manyrows_warning = true;
						$syllabplus->log(sprintf(__("Table %s has very many rows (%s) - we hope your web hosting company gives you enough resources to dump out that table in the backup", 'syllabplus'), $table, $rows).' '.__('If not, you will need to either remove data from this table, or contact your hosting company to request more resources.', 'syllabplus'), 'warning', 'manyrows_'.$this->whichdb_suffix.$table);
					}
				}
				...
				$bindump_threshold = (!$syllabplus->something_useful_happened && !empty($syllabplus->current_resumption) && (2 == $syllabplus->current_resumption - $syllabplus->last_successful_resumption)) ? 1000 : 8000;

				$bindump = (isset($table_status->Rows) && ($table_status->Rows>$bindump_threshold || (defined('SYLLABPLUS_ALWAYS_TRY_MYSQLDUMP') && SYLLABPLUS_ALWAYS_TRY_MYSQLDUMP)) && is_string($binsqldump) && empty($anonymisation_options)) ? $this->backup_table_bindump($binsqldump, $table) : false;
				
				// Means "start of table". N.B. The meaning of an integer depends upon whether the table has a usable primary key or not.
				$start_record = true;
				$can_use_primary_key = apply_filters('syllabplus_can_use_primary_key_default', true, $table);
				foreach ($potential_stitch_files as $e) {
					// The 'r' in 'tmpr' indicates that the new scheme is being used. N.B. That does *not* imply that the table has a usable primary key.
					...
					while (!is_array($start_record) && !is_wp_error($start_record)) {
						...
						if (is_integer($start_record) || is_array($start_record)) {
							...
							if (false !== $use_record) {
								...
								rename($db_temp_file, $this->syllab_dir.'/'.$rename_base);
							...
							$syllabplus->log(__("Failed to backup database table:", 'syllabplus').' '.$start_record->get_error_message().' ('.$start_record->get_error_code().')', $level);
						}
				...
				if (!empty($manyrows_warning)) $syllabplus->log_remove_warning('manyrows_'.$this->whichdb_suffix.$table);

				$this->backup_db_close();

				if ($syllabplus->error_count() > $errors_at_table_start) {
					
					$syllabplus->log('Errors occurred during backing up the table; therefore the open file will be removed');
					...
					rename($db_temp_file, $this->syllab_dir.'/'.$table_file_prefix.'.gz');
					SyllabPlus_Job_Scheduler::something_useful_happened();
					...
					foreach ($stitch_files[$table] as $basename) {
						$total_db_size += filesize($this->syllab_dir.'/'.$basename);
					}
					
					$syllabplus->log("Table $table: finishing file(s) (".count($stitch_files[$table]).', '.round($total_db_size/1024, 1).' KB)', 'notice', false, false);
				}
				
			} else {
				$total_tables--;
				$syllabplus->log("Skipping table (lacks our prefix (".$this->table_prefix.")): $table");
				if (empty($this->skipped_tables)) $this->skipped_tables = array();
				// whichdb could be an int in which case to get the name of the database and the array key use the name from dbinfo
				$key = ('wp' === $whichdb) ? 'wp' : $dbinfo['name'];
				if (empty($this->skipped_tables[$key])) $this->skipped_tables[$key] = array();
		...
		if ('wp' == $whichdb) {
			if (!$found_options_table) {
				if ($is_multisite) {
					$syllabplus->log(__('The database backup appears to have failed', 'syllabplus').' - '.__('no options or sitemeta table was found', 'syllabplus'), 'warning', 'optstablenotfound');
				} else {
					$syllabplus->log(__('The database backup appears to have failed', 'syllabplus').' - '.__('the options table was not found', 'syllabplus'), 'warning', 'optstablenotfound');
				}
				$time_this_run = time()-$syllabplus->opened_log_time;
				if ($time_this_run > 2000) {
					...
					SyllabPlus_Job_Scheduler::reschedule(60);
					$syllabplus->log("Have been running very long, and it seems the database went away; scheduling a resumption and terminating for now");
					SyllabPlus_Job_Scheduler::record_still_alive();
					die;
				}
			} else {
				$syllabplus->log_remove_warning('optstablenotfound');
			}
		}

		if ($syllabplus->error_count() > $errors_at_all_tables_start) {
			$syllabplus->log('Errors occurred whilst backing up the tables; will cease and wait for resumption');
			die;
		}
		...
		if (file_exists($backup_final_file_name) && $time_mod>100 && ($time_now-$time_mod)<30) {
			SyllabPlus_Job_Scheduler::terminate_due_to_activity($backup_final_file_name, $time_now, $time_mod);
		}
		
		if (file_exists($backup_final_file_name)) {
			$syllabplus->log("The final database file ($backup_final_file_name) exists, but was apparently not modified within the last 30 seconds (time_mod=$time_mod, time_now=$time_now, diff=".($time_now-$time_mod)."). Thus we assume that another SyllabPlus terminated; thus we will continue.");
		}

		// Finally, stitch the files together
		if (!function_exists('gzopen')) {
			$syllabplus->log("PHP function is disabled; abort expected: gzopen()");
		}
		...
		foreach ($stitch_files as $table => $table_stitch_files) {
			...
			foreach ($table_stitch_files as $table_file) {
				$syllabplus->log("{$table_file} ($sind/$how_many_tables): adding to final database dump");
				if (!$handle = gzopen($this->syllab_dir.'/'.$table_file, "r")) {
					$syllabplus->log("Error: Failed to open database file for reading: ${table_file}.gz");
					$syllabplus->log(__("Failed to open database file for reading:", 'syllabplus').' '.$table_file, 'error');
					$errors++;
				} else {
					...
					gzclose($handle);
					$unlink_files[] = $this->syllab_dir.'/'.$table_file;
				}
				$sind++;
				...
				if (0 == $sind % 100) SyllabPlus_Job_Scheduler::something_useful_happened();
			}
		}

		// DB triggers
		if ($this->wpdb_obj->get_results("SHOW TRIGGERS")) {
			// N.B. DELIMITER is not a valid SQL command; you cannot pass it to the server. It has to be interpreted by the interpreter - e.g. /usr/bin/mysql, or SyllabPlus, and used to interpret what follows. The effect of this is that using it means that some SQL clients will stumble; but, on the other hand, failure to use it means that others that don't have special support for CREATE TRIGGER may stumble, because they may feed incomplete statements to the SQL server. Since /usr/bin/mysql uses it, we choose to support it too (both reading and writing).
			...
			foreach ($all_tables as $ti) {
				...
				if ($table_triggers) {
					$this->stow("\n\n# Triggers of  ".SyllabPlus_Manipulation_Functions::backquote($table)."\n\n");
					foreach ($table_triggers as $trigger) {
						...
						$quoted_escaped_trigger_name = SyllabPlus_Manipulation_Functions::backquote(str_replace('`', '``', $trigger_name));
						$this->stow("DROP TRIGGER IF EXISTS $quoted_escaped_trigger_name;;\n");
						$trigger_query = "CREATE TRIGGER $quoted_escaped_trigger_name $trigger_time $trigger_event ON ".SyllabPlus_Manipulation_Functions::backquote($table)." FOR EACH ROW $trigger_statement;;";
		...
		// DB Stored Routines
		$stored_routines = SyllabPlus_Database_Utility::get_stored_routines();
		if (is_array($stored_routines) && !empty($stored_routines)) {
			$syllabplus->log("Dumping routines for database {$this->dbinfo['name']}");
			$this->stow("\n\n# Dumping routines for database ".SyllabPlus_Manipulation_Functions::backquote($this->dbinfo['name'])."\n\n");
			$this->stow("DELIMITER ;;\n\n");
			foreach ($stored_routines as $routine) {
				...
				$quoted_escaped_routine_name = SyllabPlus_Manipulation_Functions::backquote(str_replace('`', '``', $routine_name));
				...
				$syllabplus->log("Dumping routine: {$routine['Name']}");
			}
			$this->stow("DELIMITER ;\n\n");
		} elseif (is_wp_error($stored_routines)) {
			$syllabplus->log($stored_routines->get_error_message());
		}

		$this->stow("/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;\n/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;\n/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;\n");

		$syllabplus->log($file_base.'-db'.$this->whichdb_suffix.'.gz: finished writing out complete database file ('.round(filesize($backup_final_file_name)/1024, 1).' KB)');
		if (!$this->backup_db_close()) {
			$syllabplus->log('An error occurred whilst closing the final database file');
			$syllabplus->log(__('An error occurred whilst closing the final database file', 'syllabplus'), 'error');
			$errors++;
		}
		...
		$syllabplus->jobdata_set('jobstatus', 'dbcreated'.$this->whichdb_suffix);
		...
		foreach ($checksums as $checksum) {
			...
			$syllabplus->jobdata_set($checksum.'-db'.(('wp' == $whichdb) ? '0' : $whichdb.'0'), $cksum);
			if ($checksum_description) $checksum_description .= ', ';
			$checksum_description .= "$checksum: $cksum";
		
		}
		
		$syllabplus->log("Total database tables backed up: $total_tables (".basename($backup_final_file_name).", size: ".filesize($backup_final_file_name).", $checksum_description)");
	...
	/**
	 * This function will return a SQL WHERE clause to exclude syllab jobdata
	 *
	...
	public function backup_exclude_jobdata($where, $table) {
		...
		if ('wp' == $this->whichdb && (!empty($this->table_prefix) && strtolower($this->table_prefix.'sitemeta') == strtolower($table))) {
			$where[] = 'meta_key NOT LIKE "syllab_jobdata_%"';
			...
			if ('win' == strtolower(substr(PHP_OS, 0, 3))) {
				$syllab_jobdata = "'syllab_jobdata_%'";
			...
			$where[] = 'option_name NOT LIKE '.$syllab_jobdata.' AND option_name NOT LIKE '.$site_transient_update.'';
		}
	...
	private function backup_table_bindump($potsql, $table_name) {
		...
		file_put_contents($this->syllab_dir.'/'.$pfile, "[mysqldump]\npassword=\"".addslashes($this->dbinfo['pass'])."\"\n");

		$where_array = apply_filters('syllabplus_backup_table_sql_where', array(), $table_name, $this);
		$where = '';
		...
		if (strtolower(substr(PHP_OS, 0, 3)) == 'win') {
			$exec = "cd ".escapeshellarg(str_replace('/', '\\', $this->syllab_dir))." & ";
		} else {
			$exec = "cd ".escapeshellarg($this->syllab_dir)."; ";
		}
		...
		if ($handle) {
			...
			$ret = pclose($handle);
			if (0 != $ret) {
				$syllabplus->log("Binary mysqldump: error (code: $ret)");
				// Keep counter of failures? Change value of binsqldump?
			} else {
				if ($any_output) {
					$syllabplus->log("Table $table_name: binary mysqldump finished (writes: $writes) in ".sprintf("%.02f", max(microtime(true)-$microtime, 0.00001))." seconds");
					$ret = true;
				}
			}
		} else {
			$syllabplus->log("Binary mysqldump error: bindump popen failed");
		}

		// Clean temporary files
		@unlink($this->syllab_dir.'/'.$pfile);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
	...
	private function write_table_backup_beginning($table, $dump_as_table, $table_type, $table_structure) {
		...
		$this->stow("\n# Delete any existing table ".SyllabPlus_Manipulation_Functions::backquote($table)."\n\nDROP TABLE IF EXISTS " . SyllabPlus_Manipulation_Functions::backquote($dump_as_table).";\n");
		
		if ('VIEW' == $table_type) {
			$this->stow("DROP VIEW IF EXISTS " . SyllabPlus_Manipulation_Functions::backquote($dump_as_table) . ";\n");
		}
		
		$description = ('VIEW' == $table_type) ? 'view' : 'table';
		
		$this->stow("\n# Table structure of $description ".SyllabPlus_Manipulation_Functions::backquote($table)."\n\n");
		
		$create_table = $this->wpdb_obj->get_results("SHOW CREATE TABLE ".SyllabPlus_Manipulation_Functions::backquote($table), ARRAY_N);
		if (false === $create_table) {
			$this->stow("#\n# Error with SHOW CREATE TABLE for $table\n#\n");
		}
		$create_line = SyllabPlus_Manipulation_Functions::str_lreplace('TYPE=', 'ENGINE=', $create_table[0][1]);
		...
		if ($dump_as_table !== $table) $create_line = SyllabPlus_Manipulation_Functions::str_replace_once($table, $dump_as_table, $create_line);
		...
		$this->stow("\n\n# ".sprintf("Data contents of $description %s", SyllabPlus_Manipulation_Functions::backquote($table))."\n\n");
	...
	private function number_of_rows_to_fetch($table, $allow_further_reductions, $is_first_fetch_for_table) {
		...
		$fetch_rows = $is_first_fetch_for_table ? $default_on_first_fetch : $syllabplus->jobdata_get('fetch_rows', $default_on_first_fetch);
		...
		if (!$syllabplus->something_useful_happened && !empty($syllabplus->current_resumption) && $resumptions_since_last_successful > 1) {
			...
			$syllabplus->log("Last successful resumption was $resumptions_since_last_successful runs ago; fetch_rows will thus be: $fetch_rows (allow_further_reductions=$allow_further_reductions, is_first_fetch=$is_first_fetch_for_table)");
		}
		
		// If it has changed, then preserve it in the job for the next resumption (of this table)
		if ($fetch_rows_at_start !== $fetch_rows || $is_first_fetch_for_table) $syllabplus->jobdata_set('fetch_rows', $fetch_rows);
	...
	private function get_oversized_rows($table) {
		...
		$oversized_list = $syllabplus->jobdata_get($jobdata_key);
		...
		$oversized_row_size = defined('SYLLABPLUS_OVERSIZED_ROW_SIZE') ? SYLLABPLUS_OVERSIZED_ROW_SIZE : 2048576;
		
		$sql = $this->wpdb_obj->prepare("SELECT id FROM ".SyllabPlus_Manipulation_Functions::backquote($table)." WHERE LENGTH(post_content) > %d ORDER BY id ASC", $oversized_row_size);
		...
		$syllabplus->jobdata_set($jobdata_key, $oversized_rows);
	...
	private function backup_table($table, $table_type = 'BASE TABLE', $start_record = true, $can_use_primary_key = true) {
		...
		$table_structure = $this->wpdb_obj->get_results("DESCRIBE ".SyllabPlus_Manipulation_Functions::backquote($table));
		if (!$table_structure) {
			// $syllabplus->log(__('Error getting table details', 'syllabplus') . ": $table", 'error');
			$error_message = '';
		...
		$data_optional_tables = ('wp' == $this->whichdb) ? apply_filters('syllabplus_data_optional_tables', explode(',', SYLLABPLUS_DATA_OPTIONAL_TABLES)) : array();
		if (in_array($table_sans_prefix, $data_optional_tables)) {
			if (!$syllabplus->something_useful_happened && !empty($syllabplus->current_resumption) && ($syllabplus->current_resumption - $syllabplus->last_successful_resumption > 2)) {
				$syllabplus->log("Table $table: Data skipped (previous attempts failed, and table is marked as non-essential)");
				return array();
			}
		}
		...
		if ('VIEW' != $table_type) {
			...
			foreach ($table_structure as $struct) {
				...
				if (preg_match('/^bit(?:\(([0-9]+)\))?$/i', trim($struct->Type), $matches)) {
					...
					$struct->Field = "CAST(".SyllabPlus_Manipulation_Functions::backquote(str_replace('`', '``', $struct->Field))." AS BINARY) AS ".SyllabPlus_Manipulation_Functions::backquote(str_replace('`', '``', $struct->Field));
					$fields[] = $struct->Field;
				} else {
					$fields[] = SyllabPlus_Manipulation_Functions::backquote(str_replace('`', '``', $struct->Field));
				}
			}
			...
			if ($can_use_primary_key && is_string($primary_key) && preg_match('#^(small|medium|big)?int\(#i', $primary_key_type)) {
				...
				if (preg_match('# unsigned$#i', $primary_key_type)) {
					...
					if (true === $start_record) {
						$min_value = $this->wpdb_obj->get_var('SELECT MIN('.SyllabPlus_Manipulation_Functions::backquote($primary_key).') FROM '.SyllabPlus_Manipulation_Functions::backquote($table));
			...
			$where_array = apply_filters('syllabplus_backup_table_sql_where', array(), $table, $this);
			$where = '';
			if (!empty($where_array) && is_array($where_array)) {
			...
			do {
				...
				if ($use_primary_key) {
					...
					$final_where .= SyllabPlus_Manipulation_Functions::backquote($primary_key).((-1 === $start_record) ? ' >= 0' : " > $start_record");
					...
					if ($oversized_last) {
						$final_where .= " AND ". SyllabPlus_Manipulation_Functions::backquote($primary_key)." <= $oversized_last";
					}
				
					$limit_statement = sprintf('LIMIT %d', $fetch_rows);
					
					$order_by = 'ORDER BY '.SyllabPlus_Manipulation_Functions::backquote($primary_key).' ASC';
				...
				$select_sql = "SELECT $select FROM ".SyllabPlus_Manipulation_Functions::backquote($table)." $final_where $order_by $limit_statement";

				// Allow the data to be filtered (e.g. anonymisation)
				$table_data = apply_filters('syllabplus_backup_table_results', $this->wpdb_obj->get_results($select_sql, ARRAY_A), $table, $this->table_prefix, $this->whichdb);
				
				if (null === $table_data) {
					$syllabplus->log("Database fetch error (null returned) when running: $select_sql");
				}
				...
				if (!$table_data) {
					...
					if ($oversized_changes) $syllabplus->jobdata_set('oversized_rows_'.$table, $oversized_rows);
					continue;
				}
				$entries = 'INSERT INTO '.SyllabPlus_Manipulation_Functions::backquote($dump_as_table).' VALUES ';
		...
		$syllabplus->log("Table $table: Rows added in this batch (next record: $start_record): $total_rows (uncompressed bytes in this segment=".$this->db_current_raw_bytes.") in ".sprintf("%.02f", max(microtime(true)-$microtime, 0.00001))." seconds");

		// If all data has been fetched, then write out the closing comment
		if (-1 == $process_pages || 0 == count($table_data)) {
			$this->stow("\n# End of data contents of table ".SyllabPlus_Manipulation_Functions::backquote($table)."\n\n");
			// Keep the keyname here in sync with what is in self::get_oversized_rows()
			$syllabplus->jobdata_delete('oversized_rows_'.$table);
	...
	public function encrypt_file($file) {
		global $syllabplus;
		$encryption = $syllabplus->get_job_option('syllab_encryptionphrase');
		if (strlen($encryption) > 0) {
			$syllabplus->log("Attempting to encrypt backup file");
			try {
				$result = apply_filters('syllab_encrypt_file', null, $file, $encryption, $this->whichdb, $this->whichdb_suffix);
			} catch (Exception $e) {
	...
	public function backup_db_open($file, $allow_gz = true) {
		...
		if (false === $this->dbhandle) {
			global $syllabplus;
			$syllabplus->log("ERROR: $file: Could not open the backup file for writing");
			$syllabplus->log($file.": ".__("Could not open the backup file for writing", 'syllabplus'), 'error');
		}
	...
	public function stow($query_line) {
		if ($this->dbhandle_isgz) {
			if (false == ($ret = @gzwrite($this->dbhandle, $query_line))) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
				// $syllabplus->log(__('There was an error writing a line to the backup script:', 'syllabplus').'  '.$query_line.'  '.$php_errormsg, 'error');
			}
		} else {
	...
	private function backup_db_header() {
		...
		if ('wp' == $this->whichdb) {
			...
			$this->stow("# Created by SyllabPlus version ".$syllabplus->version." (https://syllabplus.com)\n");
		...
		$label = $syllabplus->jobdata_get('label');
		if (!empty($label)) $this->stow("# Label: $label\n");

		$this->stow("\n# Generated: ".date("l j. F Y H:i T")."\n");
		$this->stow("# Hostname: ".$this->dbinfo['host']."\n");
		$this->stow("# Database: ".SyllabPlus_Manipulation_Functions::backquote($this->dbinfo['name'])."\n");
		...
		$this->stow("/*!40101 SET NAMES ".$syllabplus->get_connection_charset($this->wpdb_obj)." */;\n");
...
// $zipfile = $this->zip_basename.(($this->index == 0) ? '' : ($this->index+1)).'.zip.tmp';
		...
		if (is_link($fullpath) && is_dir($fullpath) && 'more' == $this->whichone) {
			$syllabplus->log("Directory symlink encounted in more files backup: $use_path_when_storing -> ".readlink($fullpath).": skipping");
			return true;
		}
		...
		if (($fullpath !== $original_fullpath && strpos($original_fullpath, $fullpath) === 0) || ($original_fullpath == $fullpath && ((1== $startlevels && strpos($use_path_when_storing, '/') !== false) || (2 == $startlevels && substr_count($use_path_when_storing, '/') >1)))) {
			$syllabplus->log("Infinite recursion: symlink led us to $fullpath, which is within $original_fullpath");
			$syllabplus->log(__("Infinite recursion: consult your log for more information", 'syllabplus'), 'error');
			return false;
		}
		...
		if (false !== ($fkey = array_search($stripped_storage_path, $exclude))) {
			$syllabplus->log("Entity excluded by configuration option: $stripped_storage_path");
			unset($exclude[$fkey]);
			return true;
		}
		...
		if (is_file($fullpath)) {
			if (!empty($this->excluded_extensions) && $this->is_entity_excluded_by_extension($fullpath)) {
				$syllabplus->log("Entity excluded by configuration option (extension): ".basename($fullpath));
			} elseif (!empty($this->excluded_prefixes) && $this->is_entity_excluded_by_prefix($fullpath)) {
				$syllabplus->log("Entity excluded by configuration option (prefix): ".basename($fullpath));
			} elseif (apply_filters('syllabplus_exclude_file', false, $fullpath)) {
				$syllabplus->log("Entity excluded by filter: ".basename($fullpath));
				...
				$syllabplus->log("$fullpath: unreadable file");
				$syllabplus->log(sprintf(__("%s: unreadable file - could not be backed up (check the file permissions and ownership)", 'syllabplus'), $fullpath), 'warning');
			}
		} elseif (is_dir($fullpath)) {
			if ($fullpath == $this->syllab_dir_realpath) {
				$syllabplus->log("Skip directory (SyllabPlus backup directory): $use_path_when_storing");
				return true;
			}
			
			if (apply_filters('syllabplus_exclude_directory', false, $fullpath, $use_path_when_storing)) {
				$syllabplus->log("Skip filtered directory: $use_path_when_storing");
				return true;
			}
			
			if (file_exists($fullpath.'/.donotbackup')) {
				$syllabplus->log("Skip directory (.donotbackup file found): $use_path_when_storing");
				return true;
			}
			...
			if (!$dir_handle = @opendir($fullpath)) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
				$syllabplus->log("Failed to open directory: $fullpath");
				$syllabplus->log(sprintf(__("Failed to open directory (check the file permissions and ownership): %s", 'syllabplus'), $fullpath), 'error');
				return false;
			}
				...
				if (is_link($fullpath.'/'.$e)) {
					...
					if (is_file($deref)) {
						...
						if (false !== ($fkey = array_search($use_stripped, $exclude))) {
							$syllabplus->log("Entity excluded by configuration option: $use_stripped");
							unset($exclude[$fkey]);
						} elseif (!empty($this->excluded_extensions) && $this->is_entity_excluded_by_extension($e)) {
							$syllabplus->log("Entity excluded by configuration option (extension): $use_stripped");
						} elseif (!empty($this->excluded_prefixes) && $this->is_entity_excluded_by_prefix($e)) {
							$syllabplus->log("Entity excluded by configuration option (prefix): $use_stripped");
						} elseif (apply_filters('syllabplus_exclude_file', false, $deref, $use_stripped)) {
							$syllabplus->log("Entity excluded by filter: $use_stripped");
							...
							$syllabplus->log("$deref: unreadable file");
							$syllabplus->log(sprintf(__("%s: unreadable file - could not be backed up"), $deref), 'warning');
						}
					} elseif (is_dir($deref)) {
					
// $link_target = readlink($deref);
// $syllabplus->log("Symbolic link $use_path_when_storing/$e -> $link_target");
					...
					if (false !== ($fkey = array_search($use_stripped, $exclude))) {
						...
						$syllabplus->log("Entity excluded by configuration option (prefix): $use_stripped");
					} elseif (apply_filters('syllabplus_exclude_file', false, $fullpath.'/'.$e)) {
						$syllabplus->log("Entity excluded by filter: $use_stripped");
						...
						$syllabplus->log("$fullpath/$e: unreadable file");
						$syllabplus->log(sprintf(__("%s: unreadable file - could not be backed up", 'syllabplus'), $use_path_when_storing.'/'.$e), 'warning', "unrfile-$e");
					}
				} elseif (is_dir($fullpath.'/'.$e)) {
					if ('wpcore' == $this->whichone && 'syllab' == $e && basename($use_path_when_storing) == 'wp-content' && (!defined('SYLLABPLUS_WPCORE_INCLUDE_SYLLAB_DIRS') || !SYLLABPLUS_WPCORE_INCLUDE_SYLLAB_DIRS)) {
						// This test, of course, won't catch everything - it just aims to make things better by default
						$syllabplus->log("Directory excluded for looking like a sub-site's internal SyllabPlus directory (enable by defining SYLLABPLUS_WPCORE_INCLUDE_SYLLAB_DIRS): ".$use_path_when_storing.'/'.$e);
					} else {
			...
			$syllabplus->log("Unexpected: path ($use_path_when_storing) fails both is_file() and is_dir()");
		}
	...
	private function get_excluded_extensions($exclude) {
		...
		if (defined('SYLLABPLUS_EXCLUDE_EXTENSIONS')) {
			$exclude_from_define = explode(',', SYLLABPLUS_EXCLUDE_EXTENSIONS);
	...
	private function unserialize_gz_cache_file($file) {
		...
		while (!gzeof($whandle)) {
			...
			if (empty($bytes)) {
				$emptimes++;
				$syllabplus->log("Got empty gzread ($emptimes times)");
	...
	private function make_zipfile($source, $backup_file_basename, $whichone, $retry_on_error = true) {
		...
		$destination = $this->syllab_dir.'/'.$destination_base;
		...
		$backupable_entities = $syllabplus->get_backupable_file_entities(true, false);
		...
		for ($j = $this->first_linked_index; $j <= $this->index; $j++) {
			...
			$examine_zip = $this->syllab_dir.'/'.$backup_file_basename.'-'.$whichone.$jtext.'.zip'.(($j == $this->index) ? '.tmp' : '');

			// This comes from https://wordpress.org/support/topic/syllabplus-not-moving-all-files-to-remote-server - where it appears that the jobdata's record of the split was done (i.e. database write), but the *earlier* rename of the .tmp file was not done (i.e. I/O lost). i.e. In theory, this should be impossible; but, the sychnronicity apparently cannot be fully relied upon in some setups. The check for the index being one behind is being conservative - there's no inherent reason why it couldn't be done for other indexes.
			...
			if ($j != $this->index && !file_exists($examine_zip)) {
				$alt_examine_zip = $this->syllab_dir.'/'.$backup_file_basename.'-'.$whichone.$jtext.'.zip'.(($j == $this->index - 1) ? '.tmp' : '');
				if ($alt_examine_zip != $examine_zip && file_exists($alt_examine_zip) && is_readable($alt_examine_zip) && filesize($alt_examine_zip)>0) {
					$syllabplus->log("Looked-for zip file not found; but non-zero .tmp zip was, despite not being current index ($j != ".$this->index." - renaming zip (assume previous resumption's IO was lost before kill)");
					if (rename($alt_examine_zip, $examine_zip)) {
						clearstatcache();
					} else {
						$syllabplus->log("Rename failed - backup zips likely to not have sequential numbers (does not affect backup integrity, but can cause user confusion)");
					}
				}
			}
			...
			if (file_exists($examine_zip) && is_readable($examine_zip) && filesize($examine_zip)>0) {
				...
				if (true !== $zip->open($examine_zip)) {
					$syllabplus->log("Could not open zip file to examine (".$zip->last_error."); will remove: ".basename($examine_zip));
				...
				$syllabplus->log(basename($examine_zip).": Zip file already exists, with ".count($this->existing_files)." files");
					...
					if (isset($this->try_split)) {
						if (filesize($examine_zip) > 50*1048576) {
							...
							$this->zip_split_every = max(
								...
								SYLLABPLUS_SPLIT_MIN*1048576,
								min(filesize($examine_zip)-1048576, $this->zip_split_every)
							);
							$syllabplus->jobdata_set('split_every', (int) ($this->zip_split_every/1048576));
							$syllabplus->log("No check-in on last two runs; bumping index and reducing zip split to: ".round($this->zip_split_every/1048576, 1)." MB");
							$do_bump_index = true;
						}
						unset($this->try_split);
					} elseif (filesize($examine_zip) > $this->zip_split_every) {
						$syllabplus->log(sprintf("Zip size is at/near split limit (%s MB / %s MB) - bumping index (from: %d)", filesize($examine_zip), round($this->zip_split_every/1048576, 1), $this->index));
						$do_bump_index = true;
					}
				}

			} elseif (file_exists($examine_zip)) {
				$syllabplus->log("Zip file already exists, but is not readable or was zero-sized; will remove: ".basename($examine_zip));
				@unlink($examine_zip);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			}
		}
		...
		$this->zip_basename = $this->syllab_dir.'/'.$backup_file_basename.'-'.$whichone;
		...
		$exclude = $syllabplus->get_exclude($whichone);

		$files_enumerated_at = $syllabplus->jobdata_get('files_enumerated_at');
		if (!is_array($files_enumerated_at)) $files_enumerated_at = array();
		$files_enumerated_at[$whichone] = time();
		$syllabplus->jobdata_set('files_enumerated_at', $files_enumerated_at);
		...
		if (('uploads' == $whichone || 'others' == $whichone) && function_exists('gzopen') && function_exists('gzread')) {
			...
			if (file_exists($cache_file_base.'-zfd.gz.tmp') && file_exists($cache_file_base.'-zfb.gz.tmp') && file_exists($cache_file_base.'-info.tmp')) {
				...
				if ($use_cache_files) {
					...
					if ($any_failures) {
						$syllabplus->log("Failed to recover file lists from existing cache files");
						// Reset it all
						$this->zipfiles_skipped_notaltered = array();
						$this->makezip_recursive_batchedbytes = 0;
						$this->zipfiles_batched = array();
						...
						$syllabplus->log("File lists recovered from cache files; sizes: ".count($this->zipfiles_batched).", ".count($this->zipfiles_batched).", ".count($this->zipfiles_skipped_notaltered).")");
		...
		if (('uploads' == $whichone || 'others' == $whichone) && !$error_occurred && function_exists('gzopen') && function_exists('gzwrite')) {
			...
			if ($time_counting_ended-$time_counting_began > 20 && $syllabplus->verify_free_memory($memory_needed_estimate*0.15) && $whandle = gzopen($cache_file_base.'-zfb.gz.tmp', 'w')) {
				$syllabplus->log("File counting took a long time (".($time_counting_ended - $time_counting_began)."s); will attempt to cache results (memory_limit: $memory_limit (used: ${memory_usage}M | ${memory_usage2}M), estimated uncompressed bytes: ".round($memory_needed_estimate/1024, 1)." Kb)");
...
/*
		...
		*/
		...
		if ($retry_on_error) $syllabplus->check_recent_modification($destination);
		...
		if (count($this->zipfiles_dirbatched) > 0 || count($this->zipfiles_batched) > 0) {
			...
			$syllabplus->log(sprintf("Total entities for the zip file: %d directories, %d files (%d skipped as non-modified), %s MB", count($this->zipfiles_dirbatched), count($this->zipfiles_batched), count($this->zipfiles_skipped_notaltered), round($this->makezip_recursive_batchedbytes/1048576, 1)));
			...
			if (is_wp_error($add_them)) {
				foreach ($add_them->get_error_messages() as $msg) {
					$syllabplus->log("Error returned from makezip_addfiles: ".$msg);
				}
				$error_occurred = true;
			} elseif (false === $add_them) {
				$syllabplus->log("Error: makezip_addfiles returned false");
				$error_occurred = true;
			}
		...
		if (((file_exists($destination) || $this->index == $original_index) && @filesize($destination) < 90 && 'SyllabPlus_ZipArchive' == $this->use_zip_object) || ($error_occurred && $retry_on_error)) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			...
			$syllabplus->log("makezip_addfiles(".$this->use_zip_object.") apparently failed (file=".basename($destination).", type=$whichone, size=".filesize($destination).") - retrying with PclZip");
		...
		if (false == $error_occurred || $this->zipfiles_added > 0) {
			...
			$syllabplus->log("makezip failure: zipfiles_added=".$this->zipfiles_added.", error_occurred=".$error_occurred." (method=".$this->use_zip_object.")");
			return false;
		}
	 ...
	 */
	private function syllabplus_include_manifest($whichone) {
		global $syllabplus;

		$manifest_name = "syllabplus-manifest.json";
		$manifest = trailingslashit($this->syllab_dir).$manifest_name;

		$syllabplus->log(sprintf("Creating file manifest ($manifest_name) for incremental backup (included: %d, skipped: %d)", count($this->zipfiles_batched), count($this->zipfiles_skipped_notaltered)));
		
		if (false === ($handle = fopen($manifest, 'w+'))) return $syllabplus->log("Failed to open manifest file ($manifest_name)");
		...
		$go_to_levels = apply_filters('syllabplus_manifest_go_to_level', $go_to_levels, $whichone);
		...
		if (false === fwrite($handle, '{"version":'.$version.',"type":"'.$whichone.'",'.$directory.'"listed_levels":"'.$go_to_level.'","contents":{"directories":[')) $syllabplus->log("First write to manifest file failed ($manifest_name)");
		...
		$syllabplus->log("Successfully created file manifest (size: ".filesize($manifest).")");
	...
	private function makezip_addfiles($warn_on_failures) {
		...
		if (0 == $this->index && $this->makezip_recursive_batchedbytes < $this->zip_split_every) {
			...
			if ('SyllabPlus_PclZip' == $this->use_zip_object && ($this->makezip_recursive_batchedbytes < 512*1048576 || (defined('SYLLABPLUS_PCLZIP_FORCEALLINONE') && SYLLABPLUS_PCLZIP_FORCEALLINONE == true && 'SyllabPlus_PclZip' == $this->use_zip_object))) {
				$syllabplus->log("Only one archive required (".$this->use_zip_object.") - will attempt to do in single operation (data: ".round($this->makezip_recursive_batchedbytes/1024, 1)." KB, split: ".round($this->zip_split_every/1024, 1)." KB)");
// $syllabplus->log("PclZip, and only one archive required - will attempt to do in single operation (data: ".round($this->makezip_recursive_batchedbytes/1024, 1)." KB, split: ".round($this->zip_split_every/1024, 1)." KB)");
				$force_allinone = true;
...
// }
// if ($zipcode == 0) {
// $syllabplus->log("PclZip Error: ".$zip->errorInfo(true), 'warning');
...
// }
		...
		// $max_data_added_any_resumption = $syllabplus->jobdata_get('max_data_added_any_resumption', 0);
		...
		if (true !== $opencode) return new WP_Error('no_open', sprintf(__('Failed to open the zip file (%s) - %s', 'syllabplus'), $zipfile, $zip->last_error));

		if (apply_filters('syllabplus_include_manifest', false, $this->whichone, $this)) {
			$this->syllabplus_include_manifest($this->whichone);
		}
		...
		foreach ($this->zipfiles_batched as $file => $add_as) {
			...
			if (!file_exists($file)) {
				$syllabplus->log("File has vanished from underneath us; dropping: $add_as");
				continue;
			}
			...
			if (defined('SYLLABPLUS_SKIP_FILE_OVER_SIZE') && SYLLABPLUS_SKIP_FILE_OVER_SIZE && $fsize > SYLLABPLUS_SKIP_FILE_OVER_SIZE) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
				$syllabplus->log("File is larger than the user-configured (SYLLABPLUS_SKIP_FILE_OVER_SIZE) maximum (is: ".round($fsize/1024, 1)." KB); will skip: ".$add_as);
				continue;
			} elseif ($fsize > SYLLABPLUS_WARN_FILE_SIZE) {
			
				$log_msg = __('A very large file was encountered: %s (size: %s Mb)', 'syllabplus');
			
				// Was this warned about on a previous run?
				if ($syllabplus->warning_exists($large_file_warning_key)) {
					$syllabplus->log_remove_warning($large_file_warning_key);
					$large_file_warning_key .= '-2';
					$log_msg .= ' - '.__('a second attempt is being made (upon further failure it will be skipped)', 'syllabplus');
				} elseif ($syllabplus->warning_exists($large_file_warning_key.'-2') || $syllabplus->warning_exists($large_file_warning_key.'-final')) {
					$syllabplus->log_remove_warning($large_file_warning_key.'-2');
					$large_file_warning_key .= '-final';
					$log_msg .= ' - '.__('two unsuccessful attempts were made to include it, and it will now be omitted from the backup', 'syllabplus');
				}
			
				$syllabplus->log(sprintf($log_msg, $add_as, round($fsize/1048576, 1)), 'warning', $large_file_warning_key);
			...
			if (!isset($this->existing_files[$add_as]) || $this->existing_files[$add_as] != $fsize) {
				...
				if (method_exists($zip, 'setCompressionName') && $this->file_should_be_stored_without_compression($add_as)) {
					if (false == ($set_compress = $zip->setCompressionName($add_as, ZipArchive::CM_STORE))) {// phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
						$syllabplus->log("Zip: setCompressionName failed on: $add_as");
					}
				}
				...
				*/
				...
				if (!$force_allinone && ($zipfiles_added_thisbatch > SYLLABPLUS_MAXBATCHFILES || $reaching_split_limit || $data_added_since_reopen > $maxzipbatch || (time() - $this->zipfiles_lastwritetime) > 2)) {
					...
					if (apply_filters('syllabplus_include_manifest', false, $this->whichone, $this)) {
						...
						foreach ($files_zipadded_since_open as $info) {
							if ('syllabplus-manifest.json' == $info['file']) $manifest = true;
						}

						if (!$manifest) {
							@touch($zipfile);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
							$path = array_search('syllabplus-manifest.json', $this->zipfiles_batched);
							$zip->addFile($path, 'syllabplus-manifest.json');
							$zipfiles_added_thisbatch++;
							...
							if (method_exists($zip, 'setCompressionName') && $this->file_should_be_stored_without_compression($this->zipfiles_batched[$path])) {
								if (false == ($set_compress = $zip->setCompressionName($this->zipfiles_batched[$path], ZipArchive::CM_STORE))) {
									$syllabplus->log("Zip: setCompressionName failed on: $this->zipfiles_batched[$path]");
								}
							}

							// N.B., Since makezip_addfiles() can get called more than once if there were errors detected, potentially $zipfiles_added_thisrun can exceed the total number of batched files (if they get processed twice).
							$this->zipfiles_added_thisrun++;
							$files_zipadded_since_open[] = array('file' => $path, 'addas' => 'syllabplus-manifest.json');
					...
					if ($data_added_since_reopen > $maxzipbatch) {
						...
						$syllabplus->log("Adding batch to zip file (".$this->use_zip_object."): over ".round($maxzipbatch/1048576, 1)." MB added on this batch (".round($data_added_since_reopen/1048576, 1)." MB, ".count($this->zipfiles_batched)." files batched, $zipfiles_added_thisbatch (".$this->zipfiles_added_thisrun.") added so far); re-opening (prior size: ".round($original_size/1024, 1).' KB)');
					} elseif ($zipfiles_added_thisbatch > SYLLABPLUS_MAXBATCHFILES) {
						$syllabplus->log("Adding batch to zip file (".$this->use_zip_object."): over ".SYLLABPLUS_MAXBATCHFILES." files added on this batch (".round($data_added_since_reopen/1048576, 1)." MB, ".count($this->zipfiles_batched)." files batched, $zipfiles_added_thisbatch (".$this->zipfiles_added_thisrun.") added so far); re-opening (prior size: ".round($original_size/1024, 1).' KB)');
					} elseif (!$reaching_split_limit) {
						$syllabplus->log("Adding batch to zip file (".$this->use_zip_object."): over 2.0 seconds have passed since the last write (".round($data_added_since_reopen/1048576, 1)." MB, $zipfiles_added_thisbatch (".$this->zipfiles_added_thisrun.") files added so far); re-opening (prior size: ".round($original_size/1024, 1).' KB)');
					} else {
						$syllabplus->log("Adding batch to zip file (".$this->use_zip_object."): possibly approaching split limit (".round($data_added_since_reopen/1048576, 1)." MB, $zipfiles_added_thisbatch (".$this->zipfiles_added_thisrun.") files added so far); last ratio: ".round($this->zip_last_ratio, 4)."; re-opening (prior size: ".round($original_size/1024, 1).' KB)');
					}
					...
					// $syllabplus->jobdata_set('max_data_added_any_resumption', $max_data_added_any_resumption);
					// }
					...
					if (filesize($zipfile) > $original_size) {
						...
						if (!$something_useful_sizetest) {
							...
							$syllabplus->log(sprintf("A useful amount of data was added after this amount of zip processing: %s s (normalised: %s s, rate: %s KB/s)", round($time_since_began, 1), round($normalised_time_since_began, 1), round($rate/1024, 1)));
							...
							*/
							...
							if ($syllabplus->current_resumption >= 1) {
								$time_passed = $syllabplus->jobdata_get('run_times');
								if (!is_array($time_passed)) $time_passed = array();
								list($max_time, $timings_string, $run_times_known) = SyllabPlus_Manipulation_Functions::max_time_passed($time_passed, $syllabplus->current_resumption-1, $this->first_run);// phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
							} else {
								// $run_times_known = 0;
								// $max_time = -1;
								$run_times_known = 1;
								$max_time = microtime(true)-$syllabplus->opened_log_time;
							}

							if ($normalised_time_since_began < 6 || ($syllabplus->current_resumption >= 1 && $run_times_known >= 1 && $time_since_began < 0.6*$max_time) || (0 == $syllabplus->current_resumption && $max_time > 240)) {

								// How much can we increase it by?
								if ($normalised_time_since_began < 6 || 0 == $syllabplus->current_resumption) {
								...
								$maxzipbatch_ceiling = $syllabplus->jobdata_get('maxzipbatch_ceiling');
								if (is_numeric($maxzipbatch_ceiling) && $maxzipbatch_ceiling > 20*1048576 && $new_maxzipbatch > $maxzipbatch_ceiling) {
									$syllabplus->log("Was going to raise maxzipbytes to $new_maxzipbatch, but this is too high: a previous failure led to the ceiling being set at $maxzipbatch_ceiling, which we will use instead");
									$new_maxzipbatch = $maxzipbatch_ceiling;
								}

								// Final sanity check
								if ($new_maxzipbatch > 1048576) $syllabplus->jobdata_set('maxzipbatch', $new_maxzipbatch);
								
								if ($new_maxzipbatch <= 1048576) {
									$syllabplus->log("Unexpected new_maxzipbatch value obtained (time=$time_since_began, normalised_time=$normalised_time_since_began, max_time=$max_time, data points known=$run_times_known, old_max_bytes=$maxzipbatch, new_max_bytes=$new_maxzipbatch)");
								} elseif ($new_maxzipbatch > $maxzipbatch) {
									$syllabplus->log("Performance is good - will increase the amount of data we attempt to batch (time=$time_since_began, normalised_time=$normalised_time_since_began, max_time=$max_time, data points known=$run_times_known, old_max_bytes=$maxzipbatch, new_max_bytes=$new_maxzipbatch)");
								} elseif ($new_maxzipbatch < $maxzipbatch) {
									// Ironically, we thought we were speedy...
									$syllabplus->log("Adjust: Reducing maximum amount of batched data (time=$time_since_began, normalised_time=$normalised_time_since_began, max_time=$max_time, data points known=$run_times_known, new_max_bytes=$new_maxzipbatch, old_max_bytes=$maxzipbatch)");
								} else {
									$syllabplus->log("Performance is good - but we will not increase the amount of data we batch, as we are already at the present limit (time=$time_since_began, normalised_time=$normalised_time_since_began, max_time=$max_time, data points known=$run_times_known, max_bytes=$maxzipbatch)");
								}
							...
							if (!$syllabplus->something_useful_happened && $syllabplus->current_resumption >= 7) {
								...
								if ($run_times_known >= 5 && ($time_since_began > 0.8 * $max_time || $time_since_began + 7 > $max_time)) {
									...
									if ($new_maxzipbatch < $maxzipbatch) {
										...
										$syllabplus->jobdata_set("maxzipbatch", $new_maxzipbatch);
										$syllabplus->log("We are within a small amount of the expected maximum amount of time available; the zip-writing thresholds will be reduced (time_passed=$time_since_began, normalised_time_passed=$normalised_time_since_began, max_time=$max_time, data points known=$run_times_known, old_max_bytes=$maxzipbatch, new_max_bytes=$new_maxzipbatch)");
									} else {
										$syllabplus->log("We are within a small amount of the expected maximum amount of time available, but the zip-writing threshold is already at its lower limit (20MB), so will not be further reduced (max_time=$max_time, data points known=$run_times_known, max_bytes=$maxzipbatch)");
									}
								}
			...
			if (0 == $this->zipfiles_added % 100) {
				...
				$syllabplus->log("Zip: ".basename($zipfile).": ".$this->zipfiles_added." files added (on-disk size: ".round(@filesize($zipfile)/1024, 1)." KB)", 'notice', false, $skip_dblog);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			}

			if ($bump_index) {
				$syllabplus->log(sprintf("Zip size is at/near split limit (%s MB / %s MB) - bumping index (from: %d)", $bumped_at, round($this->zip_split_every/1048576, 1), $this->index));
				$bump_index = false;
				$this->bump_index();
		...
		if (apply_filters('syllabplus_include_manifest', false, $this->whichone, $this)) {
			if (!empty($this->manifest_path) && file_exists($this->manifest_path)) {
				$syllabplus->log('Removing manifest file: '.basename($this->manifest_path).': '.(@unlink($this->manifest_path) ? 'OK' : 'failed'));// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			}
		}

		$this->zipfiles_lastwritetime = time();
		// May not exist if the last thing we did was bump
		if (file_exists($zipfile) && filesize($zipfile) > $original_size) SyllabPlus_Job_Scheduler::something_useful_happened();

		// Move on to next archive?
		if (file_exists($zipfile) && filesize($zipfile) > $this->zip_split_every) {
			$syllabplus->log(sprintf("Zip size has gone over split limit (%s, %s) - bumping index (%d)", round(filesize($zipfile)/1048576, 1), round($this->zip_split_every/1048576, 1), $this->index));
			$this->bump_index();
		}
	...
	private function record_zip_error($files_zipadded_since_open, $msg, $warn = true) {
		...
		if (!empty($syllabplus->cpanel_quota_readable)) {
			$hosting_bytes_free = $syllabplus->get_hosting_disk_quota_free();
			if (is_array($hosting_bytes_free)) {
				$perc = round(100*$hosting_bytes_free[1]/(max($hosting_bytes_free[2], 1)), 1);
				$quota_free_msg = sprintf('Free disk space in account: %s (%s used)', round($hosting_bytes_free[3]/1048576, 1)." MB", "$perc %");
				$syllabplus->log($quota_free_msg);
				if ($hosting_bytes_free[3] < 1048576*50) {
					$quota_low = true;
					$quota_free_mb = round($hosting_bytes_free[3]/1048576, 1);
					$syllabplus->log(sprintf(__('Your free space in your hosting account is very low - only %s Mb remain', 'syllabplus'), $quota_free_mb), 'warning', 'lowaccountspace'.$quota_free_mb);
				}
			}
		}

		// Always warn of this
		if (strpos($msg, 'File Size Limit Exceeded') !== false && 'SyllabPlus_BinZip' == $this->use_zip_object) {
			$syllabplus->log(sprintf(__('The zip engine returned the message: %s.', 'syllabplus'), 'File Size Limit Exceeded'). __('Go here for more information.', 'syllabplus').' https://syllabplus.com/what-should-i-do-if-i-see-the-message-file-size-limit-exceeded/', 'warning', 'zipcloseerror-filesizelimit');
		} elseif ($warn) {
			$warn_msg = __('A zip error occurred', 'syllabplus').' - ';
			if (!empty($quota_low)) {
				$warn_msg = sprintf(__('your web hosting account appears to be full; please see: %s', 'syllabplus'), 'https://syllabplus.com/faqs/how-much-free-disk-space-do-i-need-to-create-a-backup/');
			} else {
				$warn_msg .= __('check your log for more details.', 'syllabplus');
			}
			$syllabplus->log($warn_msg, 'warning', 'zipcloseerror-'.$this->whichone);
		}

		$syllabplus->log("The attempt to close the zip file returned an error ($msg). List of files we were trying to add follows (check their permissions).");

		foreach ($files_zipadded_since_open as $ffile) {
			$syllabplus->log("File: ".$ffile['addas']." (exists: ".(int) @file_exists($ffile['file']).", is_readable: ".(int) @is_readable($ffile['file'])." size: ".@filesize($ffile['file']).')', 'notice', false, true);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
		}
	}
	...
	private function bump_index() {
		...
		foreach ($checksums as $checksum) {
			...
			$cksum = hash_file($checksum, $full_path.'.tmp');
			$syllabplus->jobdata_set($checksum.'-'.$youwhat.$this->index, $cksum);
			if ($checksum_description) $checksum_description .= ', ';
			$checksum_description .= "$checksum: $cksum";
		...
		if (file_exists($full_path.'.tmp') && filesize($full_path.'.tmp') > 0) {
			if (!rename($full_path.'.tmp', $full_path)) {
				$syllabplus->log("Rename failed for $full_path.tmp");
			} else {
				SyllabPlus_Job_Scheduler::something_useful_happened();
			}
		}
		...
		$kbsize = filesize($full_path)/1024;
		$rate = round($kbsize/$timetaken, 1);
		$syllabplus->log("Created ".$this->whichone." zip (".$this->index.") - ".round($kbsize, 1)." KB in ".round($timetaken, 1)." s ($rate KB/s) (checksums: $checksum_description)");
		$this->zip_microtime_start = microtime(true);

		// No need to add $itext here - we can just delete any temporary files for this zip
		SyllabPlus_Filesystem_Functions::clean_temporary_files('_'.$syllabplus->file_nonce."-".$youwhat, 600);
...
class SyllabPlus_WPDB_OtherDB extends wpdb {
	/**
	...
	public function bail($message, $error_code = '500') {
		...
		$syllabplus->log("WPDB_OtherDB error: $message ($error_code)");
