...
class SyllabPlus {
	...
	public $plugin_title = 'SylLab Backup – HIPAA, GDPR, CCPA Framework';

	// Choices will be shown in the admin menu in the order used here
	public $backup_methods = array(
		'syllabvault' => 'SylLab Vault',
	/*	'dropbox' => 'Dropbox',
		's3' => 'Amazon S3',
	...
	public function __construct() {
		...
		if ($fp = fopen(SYLLABPLUS_DIR.'/syllabplus.php', 'r')) {
		...
		$load_classes = array(
			'SyllabPlus_Backup_History' => 'includes/class-backup-history.php',
			'SyllabPlus_Encryption' => 'includes/class-syllabplus-encryption.php',
			'SyllabPlus_Manipulation_Functions' => 'includes/class-manipulation-functions.php',
			'SyllabPlus_Filesystem_Functions' => 'includes/class-filesystem-functions.php',
			'SyllabPlus_Storage_Methods_Interface' => 'includes/class-storage-methods-interface.php',
			'SyllabPlus_Job_Scheduler' => 'includes/class-job-scheduler.php',
		);
		
		foreach ($load_classes as $class => $relative_path) {
			if (!class_exists($class)) include_once(SYLLABPLUS_DIR.'/'.$relative_path);
		}
		
		// Create admin page
		add_action('init', array($this, 'handle_url_actions'));
		add_action('init', array($this, 'syllabplus_single_site_maintenance_init'));
		// Run earlier than default - hence earlier than other components
		// admin_menu runs earlier, and we need it because options.php wants to use $syllabplus_admin before admin_init happens
		add_action(apply_filters('syllab_admin_menu_hook', 'admin_menu'), array($this, 'admin_menu'), 9);
		...
		add_action('syllab_backup', array($this, 'backup_files'));
		add_action('syllab_backup_database', array($this, 'backup_database'));

		// The three actions that can be called from "Backup Now"
		add_action('syllab_backupnow_backup', array($this, 'backupnow_files'));
		add_action('syllab_backupnow_backup_database', array($this, 'backupnow_database'));
		add_action('syllab_backupnow_backup_all', array($this, 'backup_all'));

		// backup_all as an action is legacy (Oct 2013) - there may be some people who wrote cron scripts to use it
		add_action('syllab_backup_all', array($this, 'backup_all'));

		// This is our runs-after-backup event, whose purpose is to see if it succeeded or failed, and resume/mom-up etc.
		add_action('syllab_backup_resume', array($this, 'backup_resume'), 10, 3);
		...
		add_action('syllab_report_remotestorage_extrainfo', array($this, 'report_remotestorage_extrainfo'), 10, 3);
		...
		if (!empty($_POST) && !empty($_GET['udm_action']) && 'vault_disconnect' == $_GET['udm_action'] && !empty($_POST['slrpc_message']) && !empty($_POST['reset_hash'])) {
	...
	public function maybe_auto_update_plugin($update, $item) {
		if (!isset($item->plugin) || basename(SYLLABPLUS_DIR).'/syllabplus.php' !== $item->plugin) return $update;
	...
	/**
	 * Called by the WP action syllab_report_remotestorage_extrainfo
	 *
	...
	public function upgrader_source_selection($source, $remote_source, $upgrader_object, $hook_extra = array()) {// phpcs:ignore Generic.CodeAnalysis.UnusedFunctionParameter.Found -- Filter use
		...
		if ($been_here_already || !is_array($hook_extra) || empty($hook_extra['type']) || 'plugin' !== $hook_extra['type'] || empty($hook_extra['action']) || 'install' !== $hook_extra['action'] || empty($source) || 'syllabplus' !== basename(untrailingslashit($source)) || !class_exists('ReflectionObject')) return $source;
		...
		$upgrader_object->strings['folder_exists'] .= ' '.__('A version of SyllabPlus is already installed. WordPress will only allow you to install your new version after first de-installing the existing one. That is safe - all your settings and backups will be retained. So, go to the "Plugins" page, de-activate and de-install SyllabPlus, and then try again.', 'syllabplus');
	...
	public function itsec_scheduled_external_backup() {
		return wp_next_scheduled('syllab_backup') ? true : false;
	}
	...
	public function itsec_external_backup_link() {
			return SyllabPlus_Options::admin_page_url().'?page=syllabplus';
	}

	/**
	 * This method will disconnect SyllabVault accounts.
	 *
	...
	public function wp_loaded_vault_disconnect() {
		...
		if (is_wp_error($opts)) {
			if ('recursion' !== $opts->get_error_code()) {
				$msg = "SyllabVault (".$opts->get_error_code()."): ".$opts->get_error_message();
				$this->log($msg);
			...
			foreach ($opts['settings'] as $storage_options) {
				if (!empty($storage_options['token']) && $storage_options['token']) {
					...
					if ($hash == $_POST['reset_hash']) {
						$this->log('This site has been remotely disconnected from SyllabPlus Vault');
						...
						$this->log('An invalid request was received to disconnect this site from SyllabPlus Vault');
					}
				}
	 ...
	 * @return array
	 */
	public function get_slrpc($indicator_name = 'migrator.syllabplus.com') {
		if (!class_exists('SyllabPlus_Remote_Communications')) include_once(apply_filters('syllabplus_class_slrpc_path', SYLLABPLUS_DIR.'/includes/class-slrpc.php', $this->version));
		$ud_rpc = new SyllabPlus_Remote_Communications($indicator_name);
	...
	public function ensure_phpseclib($classes = array()) {
		...
		$phpseclib_dir = SYLLABPLUS_DIR.'/vendor/phpseclib/phpseclib/phpseclib';
	...
	public function get_hosting_disk_quota_free() {
		if (!@is_dir('/usr/local/cpanel') || $this->detect_safe_mode() || !function_exists('popen') || (!@is_executable('/usr/local/bin/perl') && !@is_executable('/usr/local/cpanel/3rdparty/bin/perl')) || (defined('SYLLABPLUS_SKIP_CPANEL_QUOTA_CHECK') && SYLLABPLUS_SKIP_CPANEL_QUOTA_CHECK)) return false;// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
		...
		$exec = "SYLLABPLUSKEY=syllabplus $perl ".SYLLABPLUS_DIR."/includes/get-cpanel-quota-usage.pl";
	...
	public function last_modified_log() {
		$syllab_dir = $this->backups_dir_location();

		$log_file = '';
		$mod_time = false;
		$nonce = '';

		if ($handle = @opendir($syllab_dir)) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			while (false !== ($entry = readdir($handle))) {
				// The latter match is for files created internally by zipArchive::addFile
				if (preg_match('/^log\.([a-z0-9]+)\.txt$/i', $entry, $matches)) {
					$mtime = filemtime($syllab_dir.'/'.$entry);
					if ($mtime > $mod_time) {
						$mod_time = $mtime;
						$log_file = $syllab_dir.'/'.$entry;
						$nonce = $matches[1];
					}
				}
			}
	...
	public function admin_menu() {
		...
		if (empty($syllabplus_admin)) include_once(SYLLABPLUS_DIR.'/admin.php');

		if (isset($_GET['wpnonce']) && isset($_GET['page']) && isset($_GET['action']) && 'syllabplus' == $_GET['page'] && 'downloadlatestmodlog' == $_GET['action'] && wp_verify_nonce($_GET['wpnonce'], 'syllabplus_download')) {
	...
	public function http_api_curl($handle) {
		if (defined('SYLLABPLUS_IPV4_ONLY') && SYLLABPLUS_IPV4_ONLY) {
	...
	public function modify_http_options($opts) {
		...
		if (!SyllabPlus_Options::get_syllab_option('syllab_ssl_useservercerts')) $opts['sslcertificates'] = SYLLABPLUS_DIR.'/includes/cacert.pem';

		$opts['sslverify'] = SyllabPlus_Options::get_syllab_option('syllab_ssl_disableverify') ? false : true;
	...
	/**
	 * Handle actions passed on to method plugins; e.g. Google OAuth 2.0 - ?action=syllabmethod-googledrive-auth&page=syllabplus
	...
	public function handle_url_actions() {
		...
		if (isset($_SERVER['REQUEST_METHOD']) && ('GET' == $_SERVER['REQUEST_METHOD'] || 'POST' == $_SERVER['REQUEST_METHOD']) && isset($_GET['action'])) {
			if (preg_match("/^syllabmethod-([a-z]+)-([a-z]+)$/", $_GET['action'], $matches) && file_exists(SYLLABPLUS_DIR.'/methods/'.$matches[1].'.php') && SyllabPlus_Options::user_can_manage()) {
				$_GET['page'] = 'syllabplus';
				$_REQUEST['page'] = 'syllabplus';
				$method = $matches[1];
				...
				$instance_id = isset($_GET['syllabplus_instance']) ? sanitize_text_field($_GET['syllabplus_instance']) : '';
		
				if ("POST" == $_SERVER['REQUEST_METHOD'] && isset($_POST['state'])) {
					$state = urldecode(sanitize_text_field($_POST['state']));
				...
				if (isset($storage_objects_and_ids[$method]['instance_settings'][$instance_id])) {
					...
					include_once(SYLLABPLUS_DIR.'/methods/'.$method.'.php');
					$call_class = "SyllabPlus_BackupModule_".$method;
				...
				try {
					...
					$this->log(sprintf(__("%s error: %s", 'syllabplus'), $method, $e->getMessage().' ('.$e->getCode().')', 'error'));
				}
				$this->register_wp_http_option_hooks(false);
			} elseif (isset($_GET['page']) && 'syllabplus' == $_GET['page'] && 'downloadlog' == $_GET['action'] && isset($_GET['syllabplus_backup_nonce']) && preg_match("/^[0-9a-f]{12}$/", $_GET['syllabplus_backup_nonce']) && SyllabPlus_Options::user_can_manage()) {
				// No WordPress nonce is needed here or for the next, since the backup is already nonce-based
				$syllab_dir = $this->backups_dir_location();
				$log_file = $syllab_dir.'/log.'.sanitize_text_field($_GET['syllabplus_backup_nonce']).'.txt';
				if (is_readable($log_file)) {
					header('Content-type: text/plain');
			...
			} elseif (isset($_GET['page']) && 'syllabplus' == $_GET['page'] && 'downloadfile' == $_GET['action'] && isset($_GET['syllabplus_file']) && preg_match('/^backup_([\-0-9]{15})_.*_([0-9a-f]{12})-db([0-9]+)?+\.(gz\.crypt)$/i', $_GET['syllabplus_file']) && SyllabPlus_Options::user_can_manage()) {
				...
				$syllab_dir = $this->backups_dir_location();
				$file = sanitize_text_field($_GET['syllabplus_file']);
				$spool_file = $syllab_dir.'/'.basename($file);
				if (is_readable($spool_file)) {
					$dkey = isset($_GET['decrypt_key']) ? stripslashes(sanitize_text_field($_GET['decrypt_key'])) : '';
					$this->spool_file($spool_file, $dkey);
					exit;
				} else {
					add_action('all_admin_notices', array($this, 'show_admin_warning_unreadablefile'));
				}
			} elseif ('syllabplus_spool_file' == $_GET['action'] && !empty($_GET['what']) && !empty($_GET['backup_timestamp']) && is_numeric($_GET['backup_timestamp']) && SyllabPlus_Options::user_can_manage()) {
				// At some point, it may be worth merging this with the previous section
				$syllab_dir = $this->backups_dir_location();
				
				$findex = isset($_GET['findex']) ? (int) sanitize_text_field($_GET['findex']) : 0;
				$backup_timestamp = sanitize_text_field($_GET['backup_timestamp']);
				$what = sanitize_text_field($_GET['what']);
				
				$backup_set = SyllabPlus_Backup_History::get_history($backup_timestamp);
				...
				if (empty($filename) || !is_readable($syllab_dir.'/'.basename($filename))) {
					echo json_encode(array('result' => __('SyllabPlus notice:', 'syllabplus').' '.__('The given file was not found, or could not be read.', 'syllabplus')));
					exit;
				}
				
				$dkey = isset($_GET['decrypt_key']) ? stripslashes(sanitize_text_field($_GET['decrypt_key'])) : "";
				
				$this->spool_file($syllab_dir.'/'.basename($filename), $dkey);
				exit;
	 ...
	 * @return void
	 */
	public function syllabplus_single_site_maintenance_init() {
		...
		wp_die('<h1>'.__('Under Maintenance', 'syllabplus') .'</h1><p>'.__('Briefly unavailable for scheduled maintenance. Check back in a minute.', 'syllabplus').'</p>');
	}
	...
	public function get_table_prefix($allow_override = false) {
		...
		return $allow_override ? apply_filters('syllabplus_get_table_prefix', $prefix) : $prefix;
	}

	public function siteid() {
		$sid = get_site_option('syllabplus-addons_siteid');
		if (!is_string($sid) || empty($sid)) {
			$sid = md5(rand().microtime(true).home_url());
			update_site_option('syllabplus-addons_siteid', $sid);
		}
		return $sid;
	}
	...
	public function show_admin_warning_unreadablelog() {
		global $syllabplus_admin;
		$syllabplus_admin->show_admin_warning('<strong>'.__('SyllabPlus notice:', 'syllabplus').'</strong> '.__('The log file could not be read.', 'syllabplus'));
	}

	public function show_admin_warning_nolog() {
		global $syllabplus_admin;
		$syllabplus_admin->show_admin_warning('<strong>'.__('SyllabPlus notice:', 'syllabplus').'</strong> '.__('No log files were found.', 'syllabplus'));
	}

	public function show_admin_warning_unreadablefile() {
		global $syllabplus_admin;
		$syllabplus_admin->show_admin_warning('<strong>'.__('SyllabPlus notice:', 'syllabplus').'</strong> '.__('The given file was not found, or could not be read.', 'syllabplus'));
	}
	...
	public function plugins_loaded() {
		...
		load_plugin_textdomain('syllabplus', false, basename(dirname(__FILE__)).'/languages/');
		...
		if ((defined('DOING_CRON') && DOING_CRON) || (defined('DOING_AJAX') && DOING_AJAX && isset($_REQUEST['subaction']) && 'backupnow' == $_REQUEST['subaction']) || (isset($_GET['page']) && 'syllabplus' == $_GET['page'] )) {
		...
		add_filter('syllabcentral_remotecontrol_command_classes', array($this, 'syllabcentral_remotecontrol_command_classes'));
		add_action('syllabcentral_command_class_wanted', array($this, 'syllabcentral_command_class_wanted'));
		add_action('syllabcentral_listener_pre_slrpc_action', array($this, 'syllabcentral_listener_pre_slrpc_action'));
		add_action('syllabcentral_listener_post_slrpc_action', array($this, 'syllabcentral_listener_post_slrpc_action'));

		if (file_exists(SYLLABPLUS_DIR.'/central/bootstrap.php')) {
			if (file_exists(SYLLABPLUS_DIR.'/central/factory.php')) include_once(SYLLABPLUS_DIR.'/central/factory.php');
			include_once(SYLLABPLUS_DIR.'/central/bootstrap.php');
		}

		$load_classes = array();

		if (defined('SYLLABPLUS_THIS_IS_CLONE')) {
			$load_classes['SyllabPlus_Temporary_Clone_Dash_Notice'] = 'includes/syllabclone/temporary-clone-dash-notice.php';
			$load_classes['SyllabPlus_Temporary_Clone_User_Notice'] = 'includes/syllabclone/temporary-clone-user-notice.php';
			$load_classes['SyllabPlus_Temporary_Clone_Restore'] = 'includes/syllabclone/temporary-clone-restore.php';
			$load_classes['SyllabPlus_Temporary_Clone_Auto_Login'] = 'includes/syllabclone/temporary-clone-auto-login.php';
			$load_classes['SyllabPlus_Temporary_Clone_Status'] = 'includes/syllabclone/temporary-clone-status.php';
		}
	...
	/**
	 * Runs upon the action syllabcentral_listener_pre_slrpc_action
	 */
	public function syllabcentral_listener_pre_slrpc_action() {
		$this->register_wp_http_option_hooks();
	}
	
	/**
	 * Runs upon the action syllabcentral_listener_post_slrpc_action
	 */
	public function syllabcentral_listener_post_slrpc_action() {
		$this->register_wp_http_option_hooks(false);
	}
	
	/**
	 * Register our class. WP filter syllabcentral_remotecontrol_command_classes.
	 *
	 * @param Array $command_classes sends across the command class
	 *
	 * @return Array - filtered value
	 */
	public function syllabcentral_remotecontrol_command_classes($command_classes) {
		if (is_array($command_classes)) $command_classes['syllabplus'] = 'SyllabCentral_SyllabPlus_Commands';
		if (is_array($command_classes)) $command_classes['syllabvault'] = 'SyllabCentral_SyllabVault_Commands';
	 ...
	 */
	public function syllabcentral_command_class_wanted($command_php_class) {
		if ('SyllabCentral_SyllabPlus_Commands' == $command_php_class) {
			include_once(SYLLABPLUS_DIR.'/includes/class-syllabcentral-syllabplus-commands.php');
		} elseif ('SyllabCentral_SyllabVault_Commands' == $command_php_class) {
			include_once(SYLLABPLUS_DIR.'/includes/syllabvault.php');
		}
	}
	...
	public function backup_time_nonce($nonce = false, $timestamp = false) {
		...
		$this->file_nonce = apply_filters('syllabplus_incremental_backup_file_nonce', $nonce);
		$this->nonce = $nonce;
		return $nonce;
	}
	...
	/**
	 * Get the SyllabPlus version and convert it to the correct format to be used in filenames
	 *
	 * @return String - the file version number
	 */
	public function get_syllabplus_file_version() {
	...
	public function get_logfile_name($nonce) {
		$syllab_dir = $this->backups_dir_location();
		return $syllab_dir."/log.$nonce.txt";
	}

	/**
	 * Writes a standardised header to the log file, using the specified logging function, which needs to be compatible with (or to be) SyllabPlus::log()
	 *
	...
	public function write_log_header($logging_function) {
		...
		if (0 === $this->current_resumption) {
			...
			if ($memlim<65 && $memlim>0) {
				$this->log(sprintf(__('The amount of memory (RAM) allowed for PHP is very low (%s Mb) - you should increase it to avoid failures due to insufficient memory (consult your web hosting company for more help)', 'syllabplus'), round($memlim, 1)), 'warning', 'lowram');
			}
			if ($max_execution_time>0 && $max_execution_time<20) {
				call_user_func($logging_function, sprintf(__('The amount of time allowed for WordPress plugins to run is very low (%s seconds) - you should increase it to avoid backup failures due to time-outs (consult your web hosting company for more help - it is the max_execution_time PHP setting; the recommended value is %s seconds or more)', 'syllabplus'), $max_execution_time, 90), 'warning', 'lowmaxexecutiontime');
			}
		...
		if (is_array($hosting_bytes_free)) {
			...
			if ($hosting_bytes_free[3] < 1048576*50) {
				...
				call_user_func($logging_function, sprintf(__('Your free space in your hosting account is very low - only %s Mb remain', 'syllabplus'), $quota_free_mb), 'warning', 'lowaccountspace'.$quota_free_mb);
			}
		} else {
			$quota_free = '';
		}
		...
		if (false == $disk_free_space) {
			call_user_func($logging_function, "Free space on disk containing Syllab's temporary directory: Unknown".$quota_free);
		} else {
			call_user_func($logging_function, "Free space on disk containing Syllab's temporary directory: ".round($disk_free_space/1048576, 1)." MB".$quota_free);
			$disk_free_mb = round($disk_free_space/1048576, 1);
			if ($disk_free_space < 50*1048576) call_user_func($logging_function, sprintf(__('Your free disk space is very low - only %s Mb remain', 'syllabplus'), round($disk_free_space/1048576, 1)), 'warning', 'lowdiskspace'.$disk_free_mb);
		}
	 ...
	 *
	 * @param string $nonce - the SyllabPlus file nonce
	 *
	...
	public function log($line, $level = 'notice', $uniq_id = false, $skip_dblog = false) {
		...
		if (false === ($line = apply_filters('syllabplus_logline', $line, $this->nonce, $level, $uniq_id, $destination))) return;
		...
		switch ($this->jobdata_get('job_type')) {
			...
			$findex = empty(sanitize_text_field($_POST['findex'])) ? 0 : sanitize_text_field($_POST['findex']);

			if (!empty(sanitize_text_field($_POST['timestamp'])) && !empty(sanitize_text_field($_POST['type']))) $this->jobdata_set('dlmessage_'.sanitize_text_field($_POST['timestamp']).'_'.sanitize_text_field($_POST['type']).'_'.$findex, $line);
				break;
			...
			default:
			if (!$skip_dblog && 'debug' != $level) SyllabPlus_Options::update_syllab_option('syllab_lastmessage', $line." (".date_i18n('M d H:i:s').")", false);
				break;
		}

		if (defined('SYLLABPLUS_CONSOLELOG') && SYLLABPLUS_CONSOLELOG) echo $line."\n";
		if (defined('SYLLABPLUS_BROWSERLOG') && SYLLABPLUS_BROWSERLOG) echo htmlentities($line)."<br>\n";
	}
	 ...
	 * @param  Boolean				   $logerror - whether to pass errors to SyllabPlus::log()
	...
	public function log_wp_error($err, $echo = false, $logerror = false) {
		...
		if (is_string($err)) {
			...
			if ($echo) $this->log(sprintf(__('Error: %s', 'syllabplus'), esc_html($err)), 'notice-warning');
			if ($logerror) $this->log($err, 'error');
			return false;
		}
		foreach ($err->get_error_messages() as $msg) {
			$this->log("Error message: $msg");
			if ($echo) $this->log(sprintf(__('Error: %s', 'syllabplus'), esc_html($msg)), 'notice-warning');
	...
	public function output_to_browser($line) {
		...
		if ((time() - $time) >= 8) {
			...
			echo esc_html(str_repeat(" ", ($buffer_size-$strcount)));
			// reset values
			$time = time();
			$strcount = 0;
		}
	 ...
	 * @param  Boolean $log_it	  - whether to pass the message to SyllabPlus::log()
	 * @return Void
	 */
	public function record_uploaded_chunk($percent, $extra = '', $file_path = false, $log_it = true) {
		...
		if ($percent > 0.7 * ($this->current_resumption - max($this->jobdata_get('uploaded_lastreset'), 9))) SyllabPlus_Job_Scheduler::something_useful_happened();

		// Log it
		global $syllabplus_backup;
		$log = empty($syllabplus_backup->current_service) ? '' : ucfirst($syllabplus_backup->current_service)." chunked upload: $percent % uploaded";
	...
	public function chunked_upload($caller, $file, $cloudpath, $logname, $chunk_size, $uploaded_size, $singletons = false) {
		...
		if ($uploaded_size < $orig_file_size) {
			...
			if (false == ($fp = @fopen($fullpath, 'rb'))) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
				...
				$this->log("$file: ".sprintf(__('%s Error: Failed to open local file', 'syllabplus'), $logname), 'error');
				return false;
			}
		...
		if (method_exists($caller, 'chunked_upload_finish')) {
			do_action('syllabplus_pre_chunked_upload_finish', $file, $caller);
			$ret = $caller->chunked_upload_finish($file);
			if (!$ret) {
				$this->log("$logname - failed to re-assemble chunks");
				$this->log(sprintf(__('%s error - failed to re-assemble chunks', 'syllabplus'), $logname), 'error');
			}
			do_action('syllabplus_post_chunked_upload_finish', $file, $caller, $ret);
		}
		...
		if ($ret) {
			...
			$ret = true;
			// SyllabPlus_RemoteStorage_Addons_Base calls this itself
			if (!is_a($caller, 'SyllabPlus_RemoteStorage_Addons_Base_v2')) $this->uploaded_file($file);
		}
	...
	public function chunked_download($file, $method, $remote_size, $manually_break_up = false, $passback = null, $chunk_size = 1048576) {
		...
		try {
			...
			if (!($fh = fopen($fullpath, 'c+'))) {
				...
				$this->log($file.": ".__("Error", 'syllabplus').": ".__('Error opening local file: Failed to download', 'syllabplus'), 'error');
				return false;
			}
			...
			$this->log("$file: ".__('Error - failed to download the file', 'syllabplus').' ('.$e->getCode().', '.$e->getMessage().')', 'error');
			return false;
		}
	...
	public function find_working_sqldump($log_it = true, $cacheit = true) {
		...
		file_put_contents($syllab_dir.'/'.$pfile, "[mysqldump]\npassword=\"".addslashes(DB_PASSWORD)."\"\n");

		$result = false;
		foreach (explode(',', SYLLABPLUS_MYSQLDUMP_EXECUTABLE) as $potsql) {
			...
			if ('win' == strtolower(substr(PHP_OS, 0, 3))) {
				$exec = "cd ".escapeshellarg(str_replace('/', '\\', $syllab_dir))." & ";
				$siteurl = "'siteurl'";
				if (false !== strpos($potsql, ' ')) $potsql = '"'.$potsql.'"';
			} else {
				$exec = "cd ".escapeshellarg($syllab_dir)."; ";
				$siteurl = "\\'siteurl\\'";
				if (false !== strpos($potsql, ' ')) $potsql = "'$potsql'";
			}
			...
			$msqld_max_allowed_packet = (defined('SYLLABPLUS_MYSQLDUMP_MAX_ALLOWED_PACKET') && (is_int(SYLLABPLUS_MYSQLDUMP_MAX_ALLOWED_PACKET) || is_string(SYLLABPLUS_MYSQLDUMP_MAX_ALLOWED_PACKET))) ? SYLLABPLUS_MYSQLDUMP_MAX_ALLOWED_PACKET : '1M';
...
// $dumped = file_get_contents($syllab_dir.'/'.$tmp_file, false, null, 0, 4096);
		...
		@unlink($syllab_dir.'/'.$pfile);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
		@unlink($syllab_dir.'/'.$tmp_file);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
	...
	public function get_zip_object_name() {
		...
		if (!class_exists('SyllabPlus_BinZip')) include_once(SYLLABPLUS_DIR . '/includes/class-zip.php');

		$zip_object = 'SyllabPlus_ZipArchive';

		// In tests, PclZip was found to be 25% slower than ZipArchive
		if (((defined('SYLLABPLUS_PREFERPCLZIP') && SYLLABPLUS_PREFERPCLZIP == true) || !class_exists('ZipArchive') || !class_exists('SyllabPlus_ZipArchive') || (!extension_loaded('zip') && !method_exists('ZipArchive', 'AddFile')))) {
			$zip_object = 'SyllabPlus_PclZip';
		}
	 ...
	 *
	 * @param  Boolean $log_it	- whether to record the results with SyllabPlus::log()
	...
	public function find_working_bin_zip($log_it = true, $cacheit = true) {
		...
		$syllab_dir = $this->backups_dir_location();
		foreach (explode(',', SYLLABPLUS_ZIP_EXECUTABLE) as $potzip) {
			...
			@mkdir($syllab_dir.'/binziptest/subdir1/subdir2', 0777, true);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged

			if (!file_exists($syllab_dir.'/binziptest/subdir1/subdir2')) return false;
			
			file_put_contents($syllab_dir.'/binziptest/subdir1/subdir2/test.html', '<html><body><a href="https://syllabplus.com">SyllabPlus is a great backup and restoration plugin for WordPress.</a></body></html>');
			@unlink($syllab_dir.'/binziptest/test.zip');// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			if (is_file($syllab_dir.'/binziptest/subdir1/subdir2/test.html')) {

				$exec = "cd ".escapeshellarg($syllab_dir)."; $potzip";
				if (defined('SYLLABPLUS_BINZIP_OPTS') && SYLLABPLUS_BINZIP_OPTS) $exec .= ' '.SYLLABPLUS_BINZIP_OPTS;
				...
				// Now test -@
				if (true == $all_ok) {
					file_put_contents($syllab_dir.'/binziptest/subdir1/subdir2/test2.html', '<html><body><a href="https://syllabplus.com">SyllabPlus is a really great backup and restoration plugin for WordPress.</a></body></html>');
					...
					$handle = proc_open($exec, $descriptorspec, $pipes, $syllab_dir);
					if (is_resource($handle)) {
				...
				$found_first = false;
				$found_second = false;
				if ($all_ok && file_exists($syllab_dir.'/binziptest/test.zip')) {
					if (function_exists('gzopen')) {
						if (!class_exists('PclZip')) include_once(ABSPATH.'/wp-admin/includes/class-pclzip.php');
						$zip = new PclZip($syllab_dir.'/binziptest/test.zip');
						...
						if (filesize($syllab_dir.'/binziptest/test.zip') > 0) {
							$found_first = true;
							$found_second = true;
						}
					}
				}
				$this->remove_binzip_test_files($syllab_dir);
	 ...
	 *
	 * @param String $syllab_dir - directory to find the files in
	 */
	private function remove_binzip_test_files($syllab_dir) {
		@unlink($syllab_dir.'/binziptest/subdir1/subdir2/test.html');// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
		@unlink($syllab_dir.'/binziptest/subdir1/subdir2/test2.html');// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
		@rmdir($syllab_dir.'/binziptest/subdir1/subdir2');// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
		@rmdir($syllab_dir.'/binziptest/subdir1');// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
		@unlink($syllab_dir.'/binziptest/test.zip');// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
		@rmdir($syllab_dir.'/binziptest');// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
	}
	...
	public function which_checksums() {
		return apply_filters('syllabplus_which_checksums', array('sha1', 'sha256'));
	}
	...
	public function printfile($description, $history, $entity, $checksums, $jobdata, $smaller = false) {
		...
		if ($smaller) {
			$pfiles = "<strong>".$description." (".sprintf(__('files: %s', 'syllabplus'), $how_many).")</strong><br>\n";
		} else {
			$pfiles = "<h3>".$description." (".sprintf(__('files: %s', 'syllabplus'), $how_many).")</h3>\n\n";
		}
		...
		if ($is_incremental) {
			...
			$backup_history = SyllabPlus_Backup_History::get_history($backup_timestamp);
			$pfiles .= "<dl>";
	...
	public function get_entity_row($file, $history, $entity, $checksums, $jobdata, $ind) {
		...
		$op = apply_filters('syllab_report_downloadable_file_link', $op, $entity, $ind, $jobdata);
		...
		if ('db' == substr($entity, 0, 2) && 'db' != $entity) {
			...
			if (is_array($jobdata) && !empty($jobdata['backup_database']) && is_array($jobdata['backup_database']) && !empty($jobdata['backup_database'][$dind]) && is_array($jobdata['backup_database'][$dind]['dbinfo']) && !empty($jobdata['backup_database'][$dind]['dbinfo']['host'])) {
				...
				$meta .= sprintf(__('External database (%s)', 'syllabplus'), $dbinfo['user'].'@'.$dbinfo['host'].'/'.$dbinfo['name'])."<br>";
			}
		}
		if (isset($history[$skey])) $meta .= sprintf(__('Size: %s MB', 'syllabplus'), round($history[$skey]/1048576, 1));
		$ckey = $entity.$ind;
		foreach ($checksums as $ck) {
			$ck_plain = false;
			if (isset($history['checksums'][$ck][$ckey])) {
				$meta .= (($meta) ? ', ' : '').sprintf(__('%s checksum: %s', 'syllabplus'), strtoupper($ck), $history['checksums'][$ck][$ckey]);
				$ck_plain = true;
			}
			if (isset($history['checksums'][$ck][$ckey.'.crypt'])) {
				if ($ck_plain) $meta .= ' '.__('(when decrypted)');
				$meta .= (($meta) ? ', ' : '').sprintf(__('%s checksum: %s', 'syllabplus'), strtoupper($ck), $history['checksums'][$ck][$ckey.'.crypt']);
			}
		}

		$fileinfo = apply_filters("syllabplus_fileinfo_$entity", array(), $ind);
	...
	public function get_backupable_file_entities($include_others = true, $full_info = false) {
		...
		if ($full_info) {
			$arr = array(
				'plugins' => array('path' => untrailingslashit(WP_PLUGIN_DIR), 'description' => __('Plugins', 'syllabplus')),
				'themes' => array('path' => WP_CONTENT_DIR.'/themes', 'description' => __('Themes', 'syllabplus')),
				'uploads' => array('path' => untrailingslashit($wp_upload_dir['basedir']), 'description' => __('Uploads', 'syllabplus'))
			);
		} else {
			$arr = array(
		...
		$arr = apply_filters('syllab_backupable_file_entities', $arr, $full_info);

		// We then add 'others' on to the end
		if ($include_others) {
			if ($full_info) {
				$arr['others'] = array('path' => WP_CONTENT_DIR, 'description' => __('Others', 'syllabplus'));
			} else {
		...
		$arr = apply_filters('syllab_backupable_file_entities_final', $arr, $full_info);
	...
	public function backup_resume($resumption_no, $bnonce) {
		...
		SyllabPlus_Backup_History::always_get_from_db();
		...
		if (0 == $resumption_no) {
			...
			if ($label) $resumption_extralog = apply_filters('syllabplus_autobackup_extralog', ", label=$label");
		} else {
			$this->nonce = $bnonce;
			...
			foreach ($time_passed as $run => $passed) {
				if (isset($runs_started[$run]) && $runs_started[$run] + $time_passed[$run] + 30 > $time_now) {
					...
					SyllabPlus_Job_Scheduler::terminate_due_to_activity('check-in', round($time_now, 1), round($runs_started[$run] + $time_passed[$run], 1), $increase_resumption);
				}
			}
			...
			if ($time_now - $this->backup_time > 172800 && true == apply_filters('syllabplus_check_obsolete_backup', true, $time_now, $this)) {
				...
				if (empty($this->backup_time) && empty($this->backup_is_already_complete) && !empty($this->logfile_name) && is_readable($this->logfile_name)) {
					...
					if (preg_match('/\(0\) Opened log file at time: (.*) on /', $first_log_bit, $matches)) {
						...
						if (time() - $first_opened < 1000) {
							...
							SyllabPlus_Job_Scheduler::reschedule(120);
							die;
						}
					}
				}
		...
		do_action('syllabplus_resume_backup_'.$job_type);
		...
		if ($resumption_no > 0 && isset($runs_started[$prev_resumption])) {
			...
			$our_expected_start = apply_filters('syllabplus_expected_start', $our_expected_start, $job_type);
			...
			if ($time_now > $our_expected_start + 720) {
				...
				$this->log(__('Your website is visited infrequently and SyllabPlus is not getting the resources it hoped for; please read this page:', 'syllabplus').' https://syllabplus.com/faqs/why-am-i-getting-warnings-about-my-site-not-having-enough-visitors/', 'warning', 'infrequentvisits');
			}
		}

		$this->jobdata_set('current_resumption', $resumption_no);

		$first_run = apply_filters('syllabplus_filerun_firstrun', 0);
		...
		if ($resumption_no >= $first_run + 8 && $resumption_no < $first_run + 15 && $resume_interval >= 300) {
			...
			list($max_time, $timings_string, $run_times_known) = SyllabPlus_Manipulation_Functions::max_time_passed($time_passed, $resumption_no - 1, $first_run);
		...
		if ($next_resumption < $first_run + 10) {
			...
			if (!empty($fail_on_resume) && $fail_on_resume == $this->current_resumption) {
				$this->log('The backup is being aborted for a repeated failure to progress.', 'syllabplus');
				$this->log(__('The backup is being aborted for a repeated failure to progress.', 'syllabplus'), 'error');
		...
		if (!empty($schedule_resumption)) {
			...
			wp_schedule_single_event($schedule_for, 'syllab_backup_resume', array($next_resumption, $bnonce));
		...
		include_once(SYLLABPLUS_DIR.'/backup.php');
		$syllabplus_backup = new SyllabPlus_Backup($backup_files, apply_filters('syllabplus_files_altered_since', -1, $job_type));
		...
		if ('no' == $backup_files) {
			...
			try {
				...
				$backup_array = $syllabplus_backup->resumable_backup_of_files($resumption_no);
				...
				$this->log(sprintf(__('A PHP exception (%s) has occurred: %s', 'syllabplus'), get_class($e), $e->getMessage()), 'error');
				die();
			// @codingStandardsIgnoreLine
			} catch (Error $e) {
				...
				if (function_exists('wp_debug_backtrace_summary')) $log_message .= ' Backtrace: '.wp_debug_backtrace_summary();
				$this->log($log_message);
				$this->log(sprintf(__('A PHP fatal error (%s) has occurred: %s', 'syllabplus'), get_class($e), $e->getMessage()), 'error');
				die();
			}
		...
		foreach ($backup_databases as $whichdb => $backup_database) {
			...
			if ('begun' == $backup_database || 'finished' == $backup_database || 'encrypted' == $backup_database) {
				...
				try {
					$db_backup = $syllabplus_backup->backup_db($backup_database, $whichdb, $dbinfo);
				} catch (Exception $e) {
					$log_message = 'Exception ('.get_class($e).') occurred during files backup: '.$e->getMessage().' (Code: '.$e->getCode().', line '.$e->getLine().' in '.$e->getFile().')';
					$this->log($log_message);
					error_log($log_message);
			...
			if ('no' != $backup_database && isset($our_files[$tindex]) && !preg_match("/\.crypt$/", $our_files[$tindex]) && 'incremental' != $job_type) {
				$our_files[$tindex] = $syllabplus_backup->encrypt_file($our_files[$tindex]);
			...
			if ('no' != $backup_database && isset($our_files[$tindex]) && file_exists($syllab_dir.'/'.$our_files[$tindex])) {
				$our_files[$tindex.'-size'] = filesize($syllab_dir.'/'.$our_files[$tindex]);
				$this->save_backup_to_history($our_files);
			}
		...
		foreach ($our_files as $key => $files) {
			...
			foreach ($files as $findex => $file) {
				...
				if ($this->is_uploaded($file)) {
				...
				} elseif (is_file($syllab_dir.'/'.$file)) {
					...
					$this->log("$file: $key: Note: This file was not marked as successfully uploaded, but does not exist on the local filesystem; now marking as uploaded ($syllab_dir/$file)");
		...
		$size_description = (false === $total_size) ? 'Unknown' : SyllabPlus_Manipulation_Functions::convert_numeric_size_to_text($total_size);
		$this->log("Saving backup history. Total backup size: $size_description");
		$this->save_backup_to_history($our_files);
		do_action('syllab_final_backup_history', $our_files);
		...
		if (0 == count($undone_files)) {
			...
			if ('begun' == $this->jobdata_get('prune')) {
				...
				$syllabplus_backup->do_prune_standalone();
				$allow_email = true;
			}
		...
		if (empty($this->newresumption_scheduled) && 0 == $resumption_no && 0 == $this->error_count_before_cloud_backup && true === $this->jobdata_get('reschedule_before_upload')) {
			...
			SyllabPlus_Job_Scheduler::reschedule(60);
			SyllabPlus_Job_Scheduler::record_still_alive();
		}
		...
		$syllabplus_backup->cloud_backup($undone_files);
	...
	public function jobdata_getarray($job_id) {
		return get_site_option('syllab_jobdata_'.$job_id, array());
	}

	public function jobdata_set_from_array($array) {
		$this->jobdata = $array;
		if (!empty($this->nonce)) update_site_option("syllab_jobdata_".$this->nonce, $this->jobdata);
	}
	...
	public function jobdata_set($key, $value) {
		if (empty($this->jobdata)) {
			$this->jobdata = empty($this->nonce) ? array() : get_site_option('syllab_jobdata_'.$this->nonce);
			if (!is_array($this->jobdata)) $this->jobdata = array();
		}
		$this->jobdata[$key] = $value;
		if ($this->nonce) update_site_option('syllab_jobdata_'.$this->nonce, $this->jobdata);
	}
	...
	public function get_job_option($opt) {
		...
		if (empty($this->jobdata['option_cache']) || !is_array($this->jobdata['option_cache'])) {
			if (!is_array($this->jobdata) && $this->nonce) $this->jobdata = get_site_option("syllab_jobdata_".$this->nonce, array());
			$this->jobdata['option_cache'] = array();
		}
		return isset($this->jobdata['option_cache'][$opt]) ? $this->jobdata['option_cache'][$opt] : SyllabPlus_Options::get_syllab_option($opt);
	}
	...
	public function jobdata_get($key, $default = null) {
		if (empty($this->jobdata)) {
			$this->jobdata = empty($this->nonce) ? array() : get_site_option('syllab_jobdata_'.$this->nonce, array());
	...
	/**
	 * Gets an instance of the "SyllabPlus_Clone" class which will be
	 * used to login the user to SyllabPlus.com
	 *
	 * @return object
	 */
	public function get_syllabplus_clone() {
		if (!class_exists('SyllabPlus_Clone')) include_once(SYLLABPLUS_DIR.'/includes/syllabplus-clone.php');
		return new SyllabPlus_Clone();
	}
	 ...
	 */
	public function syllabplus_clone_backup_options($options, $request) {
	...
	/**
	 * This function will set up the backup job data for when we are starting a clone backup job. It changes the initial jobdata so that SyllabPlus knows it's a clone job and adds the needed information for to lookup the clone and if we have it the URL and migration key for the clone.
	 *
	 ...
	 */
	public function syllabplus_clone_backup_jobdata($jobdata, $options, $split_every) {
		...
		$option_cache['syllab_encryptionphrase'] = '';
		...
		*/
		foreach ($db_backups as $key => $db_info) {
			...
			if (isset($backup['db'.$db_index])) {
				...
				if (SyllabPlus_Encryption::is_file_encrypted($db_backup_name)) $status = 'encrypted';
	 ...
	 *
	 * @param array $options
	 * @return Boolean|Void - as for SyllabPlus::boot_backup()
	 */
	...
	public function get_semaphore_lock($backup_files, $backup_database) {
		...
		if (!class_exists('SyllabPlus_Semaphore')) include_once(SYLLABPLUS_DIR.'/includes/class-semaphore.php');
		
		SyllabPlus_Semaphore::ensure_semaphore_exists($semaphore);
		...
		if (function_exists('doing_action') && (doing_action('init') || (defined('DOING_CRON') && DOING_CRON)) && (doing_action('syllab_backup_database') || doing_action('syllab_backup'))) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			$last_scheduled_action_called_at = get_option("syllab_last_scheduled_$semaphore");
			...
			if ($last_scheduled_action_called_at && $seconds_ago < 660 && apply_filters('syllab_check_repeated_scheduled_backups', true)) {
		...
		update_option("syllab_last_scheduled_$semaphore", time());
		
		$this->semaphore = SyllabPlus_Semaphore::factory();
	...
	public function get_backup_job_semaphore_lock($job_nonce, $resumption_no) {
		...
		if (!class_exists('Syllab_Semaphore_3_0')) include_once(SYLLABPLUS_DIR.'/includes/class-syllab-semaphore.php');

		if (empty($this->backup_semaphore)) {
			$this->backup_semaphore = new Syllab_Semaphore_3_0($semaphore, 30, array($this));
		}
	...
	public function is_backup_running() {
		...
		foreach ($backup_history as $backup) {
			...
			if (!empty($jobdata) && 'finished' != $jobdata['jobstatus']) {
				...
				if (wp_next_scheduled('syllab_backup_resume')) return "job_resumption_scheduled";
	...
	public function incremental_backup_file_nonce($nonce) {
		if (apply_filters('syllabplus_incremental_addon_installed', false) && !empty($this->file_nonce)) return $this->file_nonce;
		return $nonce;
	}
	...
	public function boot_backup($backup_files, $backup_database, $restrict_files_to_override = false, $one_shot = false, $service = false, $options = array()) {
		...
		if (!is_file($this->logfile_name)) {
			$this->log('Failed to open log file ('.$this->logfile_name.') - you need to check your SyllabPlus settings (your chosen directory for creating files in is not writable, or you ran out of disk space). Backup aborted.');
			$this->log(__('Could not create files in the backup directory. Backup aborted - check your SyllabPlus settings.', 'syllabplus'), 'error');
			return false;
		}

		// Some house-cleaning
		SyllabPlus_Filesystem_Functions::clean_temporary_files();
		
		// Log some information that may be helpful
		$this->log("Tasks: Backup files: $backup_files (schedule: ".SyllabPlus_Options::get_syllab_option('syllab_interval', 'unset').") Backup DB: $backup_database (schedule: ".SyllabPlus_Options::get_syllab_option('syllab_interval_database', 'unset').")");
		...
		if (false === $one_shot && is_bool($backup_database)) {
			...
			$files_schedule = SyllabPlus_Options::get_syllab_option('syllab_interval');
			$db_schedule = SyllabPlus_Options::get_syllab_option('syllab_interval_database');
			
			$sched_log_extra = '';
			
			if ('manual' != $files_schedule && false !== $files_schedule) {
				if ($files_schedule == $db_schedule || SyllabPlus_Options::get_syllab_option('syllab_interval_database', 'xyz') == 'xyz') {
					...
					if (preg_match('/^(cancel:)?(\d+)$/', $combine_around, $matches)) {
						...
						SyllabPlus_Options::update_syllab_option('syllab_combine_jobs_around', 'cancel:'.$this->combine_jobs_around);
					
						$margin = (defined('SYLLABPLUS_COMBINE_MARGIN') && is_numeric(SYLLABPLUS_COMBINE_MARGIN)) ? SYLLABPLUS_COMBINE_MARGIN : 600;
						...
						if ($time_now >= $combine_around && $time_now <= $combine_around + 2*$margin) {
							...
							if ('cancel:' == $matches[1]) {
								...
								// We want them both to happen on whichever run is first (since, afterwards, the syllab_combine_jobs_around option will have been removed when the event is rescheduled).
		...
		if (false == apply_filters('syllabplus_boot_backup', true, $backup_files, $backup_database, $one_shot)) {
		...
		if (!is_string($service) && !is_array($service)) {
			$all_services = !empty($options['remote_storage_instances']) ? array_keys($options['remote_storage_instances']) : SyllabPlus_Options::get_syllab_option('syllab_service');
			if (is_string($all_services)) $all_services = (array) $all_services;
			
			$enabled_storage_objects_and_ids = SyllabPlus_Storage_Methods_Interface::get_enabled_storage_objects_and_ids($all_services);
				...
				foreach ($enabled_storage_objects_and_ids as $method_id => $method_info) {
					...
					if ($method_info['object']->supports_feature('multi_options')) {
						foreach ($method_info['instance_settings'] as $instance_id => $instance_settings) {
							...
							if (!apply_filters('syllab_boot_backup_remote_storage_instance_include', true, $instance_settings, $method_id, $instance_id, $is_scheduled_backup)) continue;
		...
		foreach ($service as $serv) {
			include_once(SYLLABPLUS_DIR.'/methods/'.$serv.'.php');
			$cclass = 'SyllabPlus_BackupModule_'.$serv;
			if (!class_exists($cclass)) {
				error_log("SyllabPlus: backup class does not exist: $cclass");
				continue;
			}
			$obj = new $cclass;
			...
			if (is_callable(array($obj, 'get_credentials'))) {
				...
				if (is_array($opts)) {
					foreach ($opts as $opt) $option_cache[$opt] = SyllabPlus_Options::get_syllab_option($opt);
				}
			}
		}
		$option_cache = apply_filters('syllabplus_job_option_cache', $option_cache);
		...
		if (!$backup_files && !$backup_database) {
			...
			if (!SyllabPlus_Options::get_syllab_option('syllab_debug_mode') && !empty($this->logfile_name) && file_exists($this->logfile_name)) {
		...
		if (defined('SYLLABPLUS_INITIAL_RESUME_INTERVAL') && is_numeric(SYLLABPLUS_INITIAL_RESUME_INTERVAL)) {
			$resume_interval = SYLLABPLUS_INITIAL_RESUME_INTERVAL;
		} else {
			$resume_interval = (int) min(max(300, get_site_transient('syllab_initial_resume_interval')), 600);
		}
		...
		delete_site_transient('syllab_initial_resume_interval');

		$job_file_entities = array();
		if ($backup_files) {
			$possible_backups = $this->get_backupable_file_entities(true);
			foreach ($possible_backups as $youwhat => $whichdir) {
				if ((false === $restrict_files_to_override && SyllabPlus_Options::get_syllab_option("syllab_include_$youwhat", apply_filters("syllabplus_defaultoption_include_$youwhat", true))) || (is_array($restrict_files_to_override) && in_array($youwhat, $restrict_files_to_override))) {
		...
		$followups_allowed = (((!$one_shot && defined('DOING_CRON') && DOING_CRON)) || (defined('SYLLABPLUS_FOLLOWUPS_ALLOWED') && SYLLABPLUS_FOLLOWUPS_ALLOWED));

		$split_every = max((int) SyllabPlus_Options::get_syllab_option('syllab_split_every', 400), SYLLABPLUS_SPLIT_MIN);
		...
		if ($one_shot) update_site_option('syllab_oneshotnonce', $this->nonce);
		...
		if ($backup_database) {
			$dbs = apply_filters('syllab_backup_databases', array('wp' => 'begun'));
			if (is_array($dbs)) {
		...
		try {
			...
			call_user_func_array(array($this, 'jobdata_set_multi'), apply_filters('syllabplus_initial_jobdata', $initial_jobdata, $options, $split_every));
		} catch (Exception $e) {
		...
		if ($one_shot) delete_site_option('syllab_oneshotnonce');
	...
	public function get_canonical_service_list($services = null) {
		...
		if (null === $services) $services = SyllabPlus_Options::get_syllab_option('syllab_service');
	...
	public function backup_finish($do_cleanup, $allow_email, $force_abort = false) {
		...
		if (0 == $this->error_count() || $force_abort) {
			if ($do_cleanup) {
				...
				$this->jobdata_set('jobstatus', 'finished');
				wp_clear_scheduled_hook('syllab_backup_resume', array($cancel_event, $this->nonce));
				...
				wp_clear_scheduled_hook('syllab_backup_resume', array($cancel_event+1, $this->nonce));
				wp_clear_scheduled_hook('syllab_backup_resume', array($cancel_event+2, $this->nonce));
				wp_clear_scheduled_hook('syllab_backup_resume', array($cancel_event+3, $this->nonce));
				wp_clear_scheduled_hook('syllab_backup_resume', array($cancel_event+4, $this->nonce));
				$delete_jobdata = true;
			}
		} else {
			...
			if (isset($this->error_count_before_cloud_backup) && 0 === $this->error_count_before_cloud_backup) {
				if (0 == $this->current_resumption) {
					...
					if ($resume_interval > $max_interval) {
						SyllabPlus_Job_Scheduler::reschedule($max_interval);
					}
				}
			}
		}
		...
		if ($force_abort) {
			$send_an_email = true;
			$final_message = __('The backup was aborted by the user', 'syllabplus');
			if (!empty($clone_job)) $this->get_syllabplus_clone()->clone_failed_delete(array('clone_id' => $clone_id, 'secret_token' => $secret_token));
			...
			if (0 == $this->error_count('warning')) {
				$final_message = __('The backup apparently succeeded and is now complete', 'syllabplus');
				...
				$final_message = __('The backup apparently succeeded (with warnings) and is now complete', 'syllabplus');
			...
			if ($remote_sent && !$force_abort) {
				$final_message .= empty($clone_job) ? '. '.__('To complete your migration/clone, you should now log in to the remote site and restore the backup set.', 'syllabplus') : '. '.__('Your clone will now deploy this data to re-create your site.', 'syllabplus');
			}
			if ($do_cleanup) $delete_jobdata = apply_filters('syllabplus_backup_complete', $delete_jobdata);
			...
			if ($this->current_resumption + 1 == $this->jobdata_get('fail_on_resume')) {
				...
				wp_clear_scheduled_hook('syllab_backup_resume', array($this->current_resumption + 1, $this->nonce));
			}
		
			$send_an_email = true;
			$final_message = __('The backup attempt has finished, apparently unsuccessfully', 'syllabplus');
			if (!empty($clone_job)) $this->get_syllabplus_clone()->clone_failed_delete(array('clone_id' => $clone_id, 'secret_token' => $secret_token));
		} else {
			// There are errors, but a resumption will be attempted
			$final_message = __('The backup has not finished; a resumption is scheduled', 'syllabplus');
		}
		...
		$email = SyllabPlus_Options::get_syllab_option('syllab_email');
		...
		if ($delete_jobdata) delete_site_option('syllab_jobdata_'.$this->nonce);
	...
	private function send_results_email($final_message, $jobdata) {
		...
		$sendmail_to = $this->just_one_email(SyllabPlus_Options::get_syllab_option('syllab_email'));
		...
		$backup_type = ('backup' == $jobdata['job_type']) ? __('Full backup', 'syllabplus') : __('Incremental', 'syllabplus');

		$was_aborted = !empty($jobdata['aborted']);
		
		if ($was_aborted) {
			$backup_contains = __('The backup was aborted by the user', 'syllabplus');
		} elseif ('finished' == $backup_files && ('finished' == $backup_db || 'encrypted' == $backup_db)) {
			$backup_contains = __('Files and database', 'syllabplus')." ($backup_type)";
		} elseif ('finished' == $backup_files) {
			$backup_contains = ('begun' == $backup_db) ? __("Files (database backup has not completed)", 'syllabplus') : __('Files only (database was not part of this particular schedule)', 'syllabplus');
			$backup_contains .= " ($backup_type)";
		} elseif ('finished' == $backup_db || 'encrypted' == $backup_db) {
			$backup_contains = ('begun' == $backup_files) ? __("Database (files backup has not completed)", 'syllabplus') : __('Database only (files were not part of this particular schedule)', 'syllabplus');
		} elseif ('begun' == $backup_db || 'begun' == $backup_files) {
			$backup_contains = __('Incomplete', 'syllabplus');
		} else {
			$this->log('Unknown/unexpected status: '.serialize($backup_files).'/'.serialize($backup_db));
			$backup_contains = __("Unknown/unexpected error - please raise a support request", 'syllabplus');
		}
		...
		if ($this->error_count() > 0) {
			$append_log .= __('Errors encountered:', 'syllabplus')."\r\n";
		...
		if (is_array($warnings) && count($warnings) >0) {
			$append_log .= __('Warnings encountered:', 'syllabplus')."\r\n";
			$attachments[0] = $this->logfile_name;
		...
		if ($debug_mode && '' != $this->logfile_name && !in_array($this->logfile_name, $attachments)) {
			$append_log .= "\r\n".__('The log file has been attached to this email.', 'syllabplus');
			$attachments[0] = $this->logfile_name;
		}
		...
		$subject = apply_filters('syllab_report_subject', sprintf(__('Backed up: %s', 'syllabplus'), wp_specialchars_decode(get_option('blogname'), ENT_QUOTES)).' (SyllabPlus '.$this->version.') '.get_date_from_gmt(gmdate('Y-m-d H:i:s', time()), 'Y-m-d H:i'), $error_count, count($warnings));
		...
		$feed = '';
		if (!class_exists('SyllabPlus_Addon_Reporting') && !defined('SYLLABPLUS_NOADS_B') && !defined('SYLLABPLUS_NONEWSFEED')) {
			$this->log('Fetching RSS news feed');
			$rss = $this->get_syllabplus_rssfeed();
			$this->log('Fetched RSS news feed; result is a: '.get_class($rss));
			if (is_a($rss, 'SimplePie')) {
				$feed .= __('Email reports created by SyllabPlus (free edition) bring you the latest SyllabPlus.com news', 'syllabplus')." - ".sprintf(__('read more at %s', 'syllabplus'), 'https://syllabplus.com/news/')."\r\n\r\n";
		...
		$extra_messages = apply_filters('syllabplus_report_extramessages', array());
		$extra_msg = '';
		...
		$ws_advert = $syllabplus_notices->do_notice(false, 'report-plain', true);
		
		$body = apply_filters('syllab_report_body',
			__('Backup of:', 'syllabplus').' '.site_url()."\r\n".
			"SyllabPlus ".__('WordPress backup is complete', 'syllabplus').".\r\n".
			__('Backup contains:', 'syllabplus')." $backup_contains\r\n".
			__('Latest status:', 'syllabplus').' '.$final_message."\r\n".
			$extra_msg.
			"\r\n".
			$feed.
			$ws_advert."\r\n".
		...
		$this->attachments = apply_filters('syllab_report_attachments', $attachments);
		...
		foreach ($sendmail_to as $ind => $mailto) {
			...
			if (false === apply_filters('syllab_report_sendto', true, $mailto, $error_count, count($warnings), $ind)) continue;
			...
			foreach (explode(',', $mailto) as $sendmail_addr) {
				...
				try {
					wp_mail(trim($sendmail_addr), $subject, $body, array("X-SyllabPlus-Backup-ID: ".$this->nonce));
				} catch (Exception $e) {
		...
		do_action('syllab_report_finished');
	...
	public function set_sender_email_address($phpmailer) {
		...
		if (trim(strtolower($sitename)) === trim(strtolower($admin_email_domain))) {
			...
			$phpmailer->setFrom(get_bloginfo('admin_email'), sprintf(__('SyllabPlus on %s', 'syllabplus'), $sitename), false);
		}
	}
	...
	private function save_last_backup($backup_array) {
		...
		$last_backup = SyllabPlus_Options::get_syllab_option('syllab_last_backup', array());
		...
		$last_backup = apply_filters('syllabplus_save_last_backup', $last_backup);
		SyllabPlus_Options::update_syllab_option('syllab_last_backup', $last_backup, false);
	}
	...
	public function check_db_connection($handle = false, $log_it = false, $reschedule = false, $allow_bail = false) {
		...
		if ('mysql link' == $type || 'mysqli' == $type) {
			...
			if (method_exists($handle, 'check_connection') && (!defined('SYLLABPLUS_SUPPRESS_CONNECTION_CHECKS') || !SYLLABPLUS_SUPPRESS_CONNECTION_CHECKS)) {
				if (!$handle->check_connection($allow_bail)) {
					...
					if ($reschedule) SyllabPlus_Job_Scheduler::reschedule(60);
					$db_connected = false;
				} else {
					$db_connected = true;
				}
	...
	public function uploaded_file($file, $force = false) {
		...
		$service = empty($syllabplus_backup->current_service) ? '' : $syllabplus_backup->current_service;
		$instance_id = empty($syllabplus_backup->current_instance) ? '' : $syllabplus_backup->current_instance;
		$shash = $service.(('' == $service) ? '' : '-').$instance_id.(('' == $instance_id) ? '' : '-').md5($file);

		if ($force || !empty($syllabplus_backup->last_storage_instance)) {
			...
			$this->log("Recording as successfully uploaded: $file (".$syllabplus_backup->current_service.", more services to follow)");
		}
		...
		$service = $this->jobdata_get('service');
		if (!empty($syllabplus_backup->last_storage_instance) && ('' !== $service && ((is_array($service) && count($service)>0 && (count($service) > 1 || (array('') !== $service && array('none') !== $service))) || (is_string($service) && 'none' !== $service)))) {
	...
	private function check_upload_completed() {
		...
		$storage_objects_and_ids = SyllabPlus_Storage_Methods_Interface::get_storage_objects_and_ids($services);
	...
	private function delete_local($file) {
		...
		if (SyllabPlus_Options::get_syllab_option('syllab_delete_local', 1)) {
			$fullpath = $this->backups_dir_location().'/'.$file;
			...
			$this->log($log."skipped: user has unchecked syllab_delete_local option");
		}
		return true;
	}
	...
	public function check_recent_modification($file) {
		if (file_exists($file)) {
			...
			if ($time_mod > 100 && ($time_now - $time_mod) < 30) {
				SyllabPlus_Job_Scheduler::terminate_due_to_activity($file, $time_now, $time_mod);
			}
		}
	}
	...
	public function get_exclude($whichone) {
		if ('uploads' == $whichone) {
			$exclude = explode(',', SyllabPlus_Options::get_syllab_option('syllab_include_uploads_exclude', SYLLAB_DEFAULT_UPLOADS_EXCLUDE));
		} elseif ('others' == $whichone) {
			$exclude = explode(',', SyllabPlus_Options::get_syllab_option('syllab_include_others_exclude', SYLLAB_DEFAULT_OTHERS_EXCLUDE));
		} else {
			$exclude = apply_filters('syllabplus_include_'.$whichone.'_exclude', array());
		}
	...
	public function backup_uploads_dirlist($log_it = false) {
		...
		$exclude = SyllabPlus_Options::get_syllab_option('syllab_include_uploads_exclude', SYLLAB_DEFAULT_UPLOADS_EXCLUDE);
	...
	public function backup_others_dirlist($log_it = false) {
		// Create an array of directories to be skipped
		// Make the values into the keys
		$exclude = SyllabPlus_Options::get_syllab_option('syllab_include_others_exclude', SYLLAB_DEFAULT_OTHERS_EXCLUDE);
	...
	public function compile_folder_list_for_backup($backup_from_inside_dir, $avoid_these_dirs, $skip_these_dirs) {
		...
		if (is_file($backup_from_inside_dir)) {
			...
			while (false !== ($entry = readdir($handle))) {
				...
				} elseif ($candidate == $syllab_dir) {
					$this->log("finding files: $entry: skipping: this is the syllab directory");
			...
			$this->log(__('Could not read the directory', 'syllabplus').': '.$backup_from_inside_dir, 'error');
		}
	...
	private function save_backup_to_history($backup_array) {
		...
		if (!is_array($backup_array)) {
			...
			$this->log(__('Could not save backup history because we have no backup array. Backup probably failed.', 'syllabplus'), 'error');
			return;
		}
		...
		$storage_objects_and_ids = SyllabPlus_Storage_Methods_Interface::get_enabled_storage_objects_and_ids($backup_array['service'], $remote_storage_instances);
		...
		SyllabPlus_Backup_History::save_backup(apply_filters('syllabplus_save_backup_history_timestamp', $this->backup_time), $backup_array);
	}
	...
	public function schedule_event($event) {
		...
		if (is_object($event) && ('syllab_backup' == $event->hook || 'syllab_backup_database' == $event->hook)) {
			...
			$this->combine_jobs_around = SyllabPlus_Options::get_syllab_option('syllab_combine_jobs_around');
			
			SyllabPlus_Options::delete_syllab_option('syllab_combine_jobs_around');
			...
			$backup_scheduled_for = ('syllab_backup' == $event->hook) ? $event->timestamp : wp_next_scheduled('syllab_backup');
			$db_scheduled_for = ('syllab_backup_database' == $event->hook) ? $event->timestamp : wp_next_scheduled('syllab_backup_database');
			...
			if ($backup_scheduled_for && $db_scheduled_for && $diff < $margin) {
				...
				SyllabPlus_Options::update_syllab_option('syllab_combine_jobs_around', min($backup_scheduled_for, $db_scheduled_for));
			}
	...
	/**
	 * This function is both the backup scheduler and a filter callback for saving the option. It is called in the register_setting for the syllab_interval, which means when the admin settings are saved it is called.
	 *
	 * @param  String $interval
	 * @return String - filtered value
	 */
	public function schedule_backup($interval) {
		$previous_time = wp_next_scheduled('syllab_backup');

		// Clear schedule so that we don't stack up scheduled backups
		wp_clear_scheduled_hook('syllab_backup');
		if ('manual' == $interval) {
			// Clear increments schedule as the file schedule is manual
			wp_clear_scheduled_hook('syllab_backup_increments');
			return 'manual';
		}
		$previous_interval = SyllabPlus_Options::get_syllab_option('syllab_interval');
		...
		$first_time = apply_filters('syllabplus_schedule_firsttime_files', $default_time);

		wp_schedule_event($first_time, $interval, 'syllab_backup');

		return $interval;
	}

	/**
	 * This function is both the database backup scheduler and a filter callback for saving the option. It is called in the register_setting for the syllab_interval_database, which means when the admin settings are saved it is called.
	 *
	 * @param  String $interval
	 * @return String - filtered value
	 */
	public function schedule_backup_database($interval) {
		$previous_time = wp_next_scheduled('syllab_backup_database');

		// Clear schedule so that we don't stack up scheduled backups
		wp_clear_scheduled_hook('syllab_backup_database');
		if ('manual' == $interval) return 'manual';

		$previous_interval = SyllabPlus_Options::get_syllab_option('syllab_interval_database');
		...
		$first_time = apply_filters('syllabplus_schedule_firsttime_db', $default_time);
		wp_schedule_event($first_time, $interval, 'syllab_backup_database');

		return $interval;
	}
	
	/**
	 * This function is both the increments backup scheduler and a filter callback for saving the option. It is called in the register_setting for the syllab_interval_increments, which means when the admin settings are saved it is called.
	 *
	 * @param  String $interval
	 * @return String - filtered value
	 */
	public function schedule_backup_increments($interval) {
		$previous_time = wp_next_scheduled('syllab_backup_increments');

		// Clear schedule so that we don't stack up scheduled backups
		wp_clear_scheduled_hook('syllab_backup_increments');
		if ('none' == $interval || empty($interval)) return 'none';
		$previous_interval = SyllabPlus_Options::get_syllab_option('syllab_interval_increments');
		...
		$first_time = apply_filters('syllabplus_schedule_firsttime_increments', $default_time);

		wp_schedule_event($first_time, $interval, 'syllab_backup_increments');
	...
	public function storage_options_filter($options, $option_name) {
		if ('syllab_' !== substr($option_name, 0, 8)) return $options;
		$method = substr($option_name, 8);
		
		$storage = SyllabPlus_Storage_Methods_Interface::get_storage_object($method);
		
		if (!is_a($storage, 'SyllabPlus_BackupModule') || !is_callable(array($storage, 'options_filter'))) return $options;
	...
	public function backups_dir_location($allow_cache = true) {
		...
		$syllab_dir = untrailingslashit(SyllabPlus_Options::get_syllab_option('syllab_dir'));
		// When newly installing, if someone had (e.g.) wp-content/syllab in their database from a previous, deleted pre-1.7.18 install but had removed the syllab directory before re-installing, without this fix they'd end up with wp-content/wp-content/syllab.
		if (preg_match('/^wp-content\/(.*)$/', $syllab_dir, $matches) && ABSPATH.'wp-content' === WP_CONTENT_DIR) {
			SyllabPlus_Options::update_syllab_option('syllab_dir', $matches[1]);
			$syllab_dir = WP_CONTENT_DIR.'/'.$matches[1];
		}

		// Default
		if (!$syllab_dir) $syllab_dir = WP_CONTENT_DIR.'/syllab';

		// Do a test for a relative path
		if ('/' != substr($syllab_dir, 0, 1) && "\\" != substr($syllab_dir, 0, 1) && !preg_match('/^[a-zA-Z]:/', $syllab_dir)) {
			// Legacy - file paths stored related to ABSPATH
			if (is_dir(ABSPATH.$syllab_dir) && is_file(ABSPATH.$syllab_dir.'/index.html') && is_file(ABSPATH.$syllab_dir.'/.htaccess') && !is_file(ABSPATH.$syllab_dir.'/index.php') && false !== strpos(file_get_contents(ABSPATH.$syllab_dir.'/.htaccess', false, null, 0, 20), 'deny from all')) {
				$syllab_dir = ABSPATH.$syllab_dir;
			} else {
				// File paths stored relative to WP_CONTENT_DIR
				$syllab_dir = trailingslashit(WP_CONTENT_DIR).$syllab_dir;
			}
		}
		...
		if ((!is_dir($syllab_dir) || !is_file($syllab_dir.'/index.html') || !is_file($syllab_dir.'/.htaccess')) && !is_file($syllab_dir.'/index.php') || !is_file($syllab_dir.'/web.config')) {
			@mkdir($syllab_dir, 0775, true);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			@file_put_contents($syllab_dir.'/index.html', "<html><body><a href=\"https://syllabplus.com\" target=\"_blank\">WordPress backups by SyllabPlus</a></body></html>");// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			if (!is_file($syllab_dir.'/.htaccess')) @file_put_contents($syllab_dir.'/.htaccess', 'deny from all');// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			if (!is_file($syllab_dir.'/web.config')) @file_put_contents($syllab_dir.'/web.config', "<configuration>\n<system.webServer>\n<authorization>\n<deny users=\"*\" />\n</authorization>\n</system.webServer>\n</configuration>\n");// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
		}

		$this->backup_dir = $syllab_dir;
	...
	public function spool_file($fullpath, $encryption = '') {
		...
		if (SyllabPlus_Encryption::is_file_encrypted($fullpath)) {
			if (ob_get_level()) {
			...
			SyllabPlus_Encryption::spool_crypted_file($fullpath, (string) $encryption);
			return;
		}

		$content_type = SyllabPlus_Manipulation_Functions::get_mime_type_from_filename($fullpath, false);
		
		include_once(SYLLABPLUS_DIR.'/includes/class-partialfileservlet.php');
		...
		try {
			$range_header = SyllabPlus_RangeHeader::createFromHeaderString($range_header);
			$servlet = new SyllabPlus_PartialFileServlet($range_header);
			$servlet->sendFile($fullpath, $content_type);
		} catch (SyllabPlus_InvalidRangeHeaderException $e) {
			header("HTTP/1.1 400 Bad Request");
			error_log("SyllabPlus: SyllabPlus_InvalidRangeHeaderException: ".$e->getMessage());
		} catch (SyllabPlus_UnsatisfiableRangeException $e) {
			header("HTTP/1.1 416 Range Not Satisfiable");
		} catch (SyllabPlus_NonExistentFileException $e) {
			header("HTTP/1.1 404 Not Found");
		} catch (SyllabPlus_UnreadableFileException $e) {
	...
	public function just_one($input, $filter = 'savestorage', $rinput = false) {
		...
		return apply_filters('syllabplus_'.$filter, $rinput, $oinput);
	}
	...
	public function enqueue_select2() {
		...
		wp_enqueue_script('select2', SYLLABPLUS_URL."/includes/select2/select2".$min_or_not.".js", array('jquery'), $select2_version);
		wp_enqueue_style('select2', SYLLABPLUS_URL."/includes/select2/select2".$min_or_not.".css", array(), $select2_version);
	}
	...
	/**
	 * Get the SyllabPlus RSS feed
	 *
	 * @uses fetch_feed()
	 *
	 * @return WP_Error|SimplePie WP_Error object on failure or SimplePie object on success
	 */
	public function get_syllabplus_rssfeed() {
		if (!function_exists('fetch_feed')) include(ABSPATH.WPINC.'/feed.php');
		return fetch_feed('http://feeds.feedburner.com/syllabplus/');
	}

	/**
	 * Sets up the nonce, basic job data, opens a log file for a new restore job, and makes sure that the Syllab_Restorer class is available
	 *
	...
	public function initiate_restore_job($nonce = false) {
		...
		if (!class_exists('Syllab_Restorer')) include_once(SYLLABPLUS_DIR.'/restorer.php');
	}
	 ...
	 * @param Array			 $res		  - accompanying data. The key 'syllab_encryptionphrase' will be used for decryption if relevant.
	...
	public function analyse_db_file($timestamp, $res, $db_file = false, $header_only = false) {
		...
		if (!class_exists('SyllabPlus_Database_Utility')) include_once(SYLLABPLUS_DIR.'/includes/class-database-utility.php');
		...
		if (false === $db_file) {
			...
			$backup = SyllabPlus_Backup_History::get_history($timestamp);
			if (!isset($backup['nonce']) || !isset($backup['db'])) return array($mess, $warn, $err, $info);

			$db_file = is_string($backup['db']) ? $syllab_dir.'/'.$backup['db'] : $syllab_dir.'/'.$backup['db'][0];
		}

		if (!is_readable($db_file)) return array($mess, $warn, $err, $info);

		// Encrypted - decrypt it
		if (SyllabPlus_Encryption::is_file_encrypted($db_file)) {

			$encryption = empty($res['syllab_encryptionphrase']) ? SyllabPlus_Options::get_syllab_option('syllab_encryptionphrase') : $res['syllab_encryptionphrase'];

			if (!$encryption) {
				if (class_exists('SyllabPlus_Addon_MoreDatabase')) {
					$err[] = sprintf(__('Error: %s', 'syllabplus'), __('Decryption failed. The database file is encrypted, but you have no encryption key entered.', 'syllabplus'));
				} else {
					$err[] = sprintf(__('Error: %s', 'syllabplus'), __('Decryption failed. The database file is encrypted.', 'syllabplus'));
				}
				return array($mess, $warn, $err, $info);
			}

			$decrypted_file = SyllabPlus_Encryption::decrypt($db_file, $encryption);

			if (is_array($decrypted_file)) {
				$db_file = $decrypted_file['fullpath'];
			} else {
				$err[] = __('Decryption failed. The most likely cause is that you used the wrong key.', 'syllabplus');
				return array($mess, $warn, $err, $info);
			}
		}
		...
		if (filesize($db_file) < 1000) {
			$err[] = sprintf(__('The database is too small to be a valid WordPress database (size: %s Kb).', 'syllabplus'), round(filesize($db_file)/1024, 1));
			return array($mess, $warn, $err, $info);
		}

		// If the backup is not from SyllabPlus and it's not a simple SQL file then we don't want to scan
		...
		$dbhandle = $is_plain ? fopen($db_file, 'r') : SyllabPlus_Filesystem_Functions::gzopen_for_read($db_file, $warn, $err);
		if (!is_resource($dbhandle)) {
			$err[] = __('Failed to open database file.', 'syllabplus');
			return array($mess, $warn, $err, $info);
		}
		...
		$dbscan_timeout = (defined('SYLLABPLUS_DBSCAN_TIMEOUT') && is_numeric(SYLLABPLUS_DBSCAN_TIMEOUT)) ? SYLLABPLUS_DBSCAN_TIMEOUT : $default_dbscan_timeout;
		...
		$db_charset_collate_scan_timeout = (defined('SYLLABPLUS_DB_CHARSET_COLLATE_SCAN_TIMEOUT') && is_numeric(SYLLABPLUS_DB_CHARSET_COLLATE_SCAN_TIMEOUT)) ? SYLLABPLUS_DB_CHARSET_COLLATE_SCAN_TIMEOUT : 10;
		...
		while ((($is_plain && !feof($dbhandle)) || (!$is_plain && !gzeof($dbhandle))) && ($line<100 || (!$header_only && count($wanted_tables)>0) || ((microtime(true) - $charset_scan_start_time) < $db_charset_collate_scan_timeout && !empty($db_supported_character_sets)))) {
			...
			if (substr($buffer, 0, 1) == '#') {
				...
				if ('' == $old_siteurl && preg_match('/^\# Backup of: (http(.*))$/', $buffer, $matches)) {
					...
					$mess[] = __('Backup of:', 'syllabplus').' '.esc_html($old_siteurl).((!empty($old_wp_version)) ? ' '.sprintf(__('(version: %s)', 'syllabplus'), $old_wp_version) : '');
					// Check for should-be migration
					if (untrailingslashit(site_url()) != $old_siteurl) {
						if (!$migration_warning) {
							$migration_warning = true;
							$info['migration'] = true;
							// && !class_exists('SyllabPlus_Addons_Migrator')
							if (SyllabPlus_Manipulation_Functions::normalise_url($old_siteurl) == SyllabPlus_Manipulation_Functions::normalise_url(site_url())) {
								...
								if ((stripos($old_siteurl_parsed['host'], 'www.') === 0 && stripos($actual_siteurl_parsed['host'], 'www.') !== 0) || (stripos($old_siteurl_parsed['host'], 'www.') !== 0 && stripos($actual_siteurl_parsed['host'], 'www.') === 0)) {
									$powarn = sprintf(__('The website address in the backup set (%s) is slightly different from that of the site now (%s). This is not expected to be a problem for restoring the site, as long as visits to the former address still reach the site.', 'syllabplus'), $old_siteurl, site_url()).' ';
								} else {
									$powarn = '';
								}
								if (('https' == $old_siteurl_parsed['scheme'] && 'http' == $actual_siteurl_parsed['scheme']) || ('http' == $old_siteurl_parsed['scheme'] && 'https' == $actual_siteurl_parsed['scheme'])) {
									$powarn .= sprintf(__('This backup set is of this site, but at the time of the backup you were using %s, whereas the site now uses %s.', 'syllabplus'), $old_siteurl_parsed['scheme'], $actual_siteurl_parsed['scheme']);
									if ('https' == $old_siteurl_parsed['scheme']) {
										$powarn .= ' '.apply_filters('syllabplus_https_to_http_additional_warning', sprintf(__('This restoration will work if you still have an SSL certificate (i.e. can use https) to access the site. Otherwise, you will want to use %s to search/replace the site address so that the site can be visited without https.', 'syllabplus'), '<a href="https://syllabplus.com/shop/migrator/" target="_blank">'.__('the migrator add-on', 'syllabplus').'</a>'));
									} else {
										$powarn .= ' '.apply_filters('syllabplus_http_to_https_additional_warning', sprintf(__('As long as your web hosting allows http (i.e. non-SSL access) or will forward requests to https (which is almost always the case), this is no problem. If that is not yet set up, then you should set it up, or use %s so that the non-https links are automatically replaced.', 'syllabplus'), apply_filters('syllabplus_migrator_addon_link', '<a href="https://syllabplus.com/shop/migrator/" target="_blank">'.__('the migrator add-on', 'syllabplus').'</a>')));
									}
								} else {
									$powarn .= apply_filters('syllabplus_dbscan_urlchange_www_append_warning', '');
								}
								$warn[] = $powarn;
							} else {
								...
								$warn[] = apply_filters('syllabplus_dbscan_urlchange', '<a href="https://syllabplus.com/shop/migrator/" target="_blank">'.sprintf(__('This backup set is from a different site (%s) - this is not a restoration, but a migration. You need the Migrator add-on in order to make this work.', 'syllabplus'), esc_html($old_siteurl.' / '.untrailingslashit(site_url()))).'</a>', $old_siteurl, $res);
							}
							if (!class_exists('SyllabPlus_Addons_Migrator')) {
								$warn[] .= '<strong><a href="'.apply_filters('syllabplus_com_link', "https://syllabplus.com/faqs/tell-me-more-about-the-search-and-replace-site-location-in-the-database-option/").'" target="_blank">'.__('You can search and replace your database (for migrating a website to a new location/URL) with the Migrator add-on - follow this link for more information', 'syllabplus').'</a></strong>';
							}
						}

						if ($this->mod_rewrite_unavailable(false)) {
							$warn[] = sprintf(__('You are using the %s webserver, but do not seem to have the %s module loaded.', 'syllabplus'), 'Apache', 'mod_rewrite').' '.sprintf(__('You should enable %s to make any pretty permalinks (e.g. %s) work', 'syllabplus'), 'mod_rewrite', 'http://example.com/my-page/');
						}
					...
					// Check for should-be migration
					if (!$migration_warning && SyllabPlus_Manipulation_Functions::normalise_url(home_url()) != SyllabPlus_Manipulation_Functions::normalise_url($old_home)) {
						$migration_warning = true;
						$powarn = apply_filters('syllabplus_dbscan_urlchange', '<a href="https://syllabplus.com/shop/migrator/" target="_blank">'.sprintf(__('This backup set is from a different site (%s) - this is not a restoration, but a migration. You need the Migrator add-on in order to make this work.', 'syllabplus'), esc_html($old_home.' / '.home_url())).'</a>', $old_home, $res);
						if (!empty($powarn)) $warn[] = $powarn;
					}
				} elseif (!isset($info['created_by_version']) && preg_match('/^\# Created by SyllabPlus version ([\d\.]+)/', $buffer, $matches)) {
					...
					if (version_compare($old_wp_version, $wp_version, '>')) {
						// $mess[] = sprintf(__('%s version: %s', 'syllabplus'), 'WordPress', $old_wp_version);
						$warn[] = sprintf(__('You are importing from a newer version of WordPress (%s) into an older one (%s). There are no guarantees that WordPress can handle this.', 'syllabplus'), $old_wp_version, $wp_version);
					}
					if (preg_match('/running on PHP ([0-9]+\.[0-9]+)(\s|\.)/', $matches[4], $nmatches) && preg_match('/^([0-9]+\.[0-9]+)(\s|\.)/', PHP_VERSION, $cmatches)) {
						...
						if (version_compare($old_php_version, $current_php_version, '>')) {
							// $mess[] = sprintf(__('%s version: %s', 'syllabplus'), 'WordPress', $old_wp_version);
							$warn[] = sprintf(__('The site in this backup was running on a webserver with version %s of %s. ', 'syllabplus'), $old_php_version, 'PHP').' '.sprintf(__('This is significantly newer than the server which you are now restoring onto (version %s).', 'syllabplus'), PHP_VERSION).' '.sprintf(__('You should only proceed if you cannot update the current server and are confident (or willing to risk) that your plugins/themes/etc. are compatible with the older %s version.', 'syllabplus'), 'PHP').' '.sprintf(__('Any support requests to do with %s should be raised with your web hosting company.', 'syllabplus'), 'PHP');
						} elseif (version_compare($old_php_version, $current_php_version, '<')) {
							$warn[] = sprintf(__('The site in this backup was running on a webserver with version %s of %s. ', 'syllabplus'), $old_php_version, 'PHP').' '.sprintf(__('This is older than the server which you are now restoring onto (version %s).', 'syllabplus'), PHP_VERSION).' '.sprintf(__('You should only proceed if you have checked and are confident (or willing to risk) that your plugins/themes/etc. are compatible with the new %s version.', 'syllabplus'), 'PHP').' '.sprintf(__('Any support requests to do with %s should be raised with your web hosting company.', 'syllabplus'), 'PHP');
						}
					}
				} elseif (null === $old_table_prefix && (preg_match('/^\# Table prefix: ?(\S*)$/', $buffer, $matches) || preg_match('/^-- Table prefix: ?(\S*)$/i', $buffer, $matches))) {
					$old_table_prefix = $matches[1];
					// echo '<strong>'.__('Old table prefix:', 'syllabplus').'</strong> '.esc_html($old_table_prefix).'<br>';
				} elseif (empty($info['label']) && preg_match('/^\# Label: (.*)$/', $buffer, $matches)) {
					$info['label'] = $matches[1];
					$mess[] = __('Backup label:', 'syllabplus').' '.esc_html($info['label']);
					...
					if ('end' == $matches[1]) {
						...
						if (isset($old_siteinfo['multisite']) && !$old_siteinfo['multisite'] && is_multisite()) {
							...
							// if (!defined('SYLLABPLUS_EXPERIMENTAL_IMPORTINTOMULTISITE') || !SYLLABPLUS_EXPERIMENTAL_IMPORTINTOMULTISITE) {
							// $err[] =  sprintf(__('Error: %s', 'syllabplus'), __('You are running on WordPress multisite - but your backup is not of a multisite site.', 'syllabplus'));
							// return array($mess, $warn, $err, $info);
							// } else {
							$warn[] = __('You are running on WordPress multisite - but your backup is not of a multisite site.', 'syllabplus').' '.__('It will be imported as a new site.', 'syllabplus').' <a href="https://syllabplus.com/information-on-importing-a-single-site-wordpress-backup-into-a-wordpress-network-i-e-multisite/" target="_blank">'.__('Please read this link for important information on this process.', 'syllabplus').'</a>';
							// }
							// Got the needed code?
							if (!class_exists('SyllabPlusAddOn_MultiSite') || !class_exists('SyllabPlus_Addons_Migrator')) {
								$err[] = sprintf(__('Error: %s', 'syllabplus'), sprintf(__('To import an ordinary WordPress site into a multisite installation requires %s.', 'syllabplus'), 'SyllabPlus Premium'));
								return array($mess, $warn, $err, $info);
							}
						} elseif (isset($old_siteinfo['multisite']) && $old_siteinfo['multisite'] && !is_multisite()) {
							$warn[] = __('Warning:', 'syllabplus').' '.__('Your backup is of a WordPress multisite install; but this site is not. Only the first site of the network will be accessible.', 'syllabplus').' <a href="https://codex.wordpress.org/Create_A_Network" target="_blank">'.__('If you want to restore a multisite backup, you should first set up your WordPress installation as a multisite.', 'syllabplus').'</a>';
						}
						...
						if ('multisite' == $key) {
							...
							if ($val) $mess[] = '<strong>'.__('Site information:', 'syllabplus').'</strong> '.'backup is of a WordPress Network';
						}
				...
				if (null !== $old_table_prefix) {
					// Remove prefix
					$table = $old_table_prefix ? SyllabPlus_Manipulation_Functions::str_replace_once($old_table_prefix, '', $table) : $table;
				...
				if (!$mysql_version_warned && version_compare($db_version, '5.2.0', '<') && preg_match('/(CHARSET|COLLATE)[= ]utf8mb4/', $buffer)) {
					...
					$err[] = sprintf(__('Error: %s', 'syllabplus'), sprintf(__('The database backup uses MySQL features not available in the old MySQL version (%s) that this site is running on.', 'syllabplus'), $db_version).' '.__('You must upgrade MySQL to be able to use this database.', 'syllabplus'));
				}
		...
		if (!empty($db_supported_character_sets)) {
			...
			if ($db_charset_forbidden) {
				...
				$warn[] = sprintf(_n("The database server that this WordPress site is running on doesn't support the character set (%s) which you are trying to import.", "The database server that this WordPress site is running on doesn't support the character sets (%s) which you are trying to import.", count($db_unsupported_charset_unique), 'syllabplus'), implode(', ', $db_unsupported_charset_unique)).' '.__('You can choose another suitable character set instead and continue with the restoration at your own risk.', 'syllabplus').' <a target="_blank" href="https://syllabplus.com/faqs/implications-changing-tables-character-set/" target="_blank">'.__('Go here for more information.', 'syllabplus').'</a>'.' <a target="_blank" href="https://syllabplus.com/faqs/implications-changing-tables-character-set/" target="_blank">'.__('Go here for more information.', 'syllabplus').'</a>';
				$db_supported_character_sets = array_keys($db_supported_character_sets);
				$similar_type_charset = SyllabPlus_Manipulation_Functions::get_matching_str_from_array_elems($db_unsupported_charset_unique, $db_supported_character_sets, true);
				...
				$charset_select_html = '<label>'.__('Your chosen character set to use instead:', 'syllabplus').'</label> ';
				$charset_select_html .= '<select name="syllab_restorer_charset" id="syllab_restorer_charset">';
			...
			if ($db_collate_forbidden) {
				...
				if (empty($similar_type_collate) && !empty($db_supported_charsets_related_to_unsupported_collations)) {
					...
					if (!empty($db_supported_collations_related_to_charset)) {
						$similar_type_collate = SyllabPlus_Manipulation_Functions::get_matching_str_from_array_elems($db_unsupported_collate_unique, $db_supported_collations_related_to_charset, false);
					}
				}
				...
				if (empty($similar_type_collate)) {
					$similar_type_collate = SyllabPlus_Manipulation_Functions::get_matching_str_from_array_elems($db_unsupported_collate_unique, array_keys($db_supported_collations), false);
				}

				$collate_select_html = '<div class="notice below-h2 syllab-restore-option"><label>'.__('Your chosen replacement collation', 'syllabplus').':</label>';
				$collate_select_html .= '<select name="syllab_restorer_collate" id="syllab_restorer_collate">';
				$db_charsets_found_unique = array_unique($db_charsets_found);
				...
				if (count($db_charsets_found_unique) > 1 && !$db_charset_forbidden) {
					$collate_select_html .= '<option value="choose_a_default_for_each_table" selected="selected">'.__('Choose a default for each table', 'syllabplus').'</option>';
				}
		...
		*/
		...
		if (null !== $old_table_prefix) {
			...
			if ('' === $old_table_prefix) $warn[] = __('This backup is of a site with an empty table prefix, which WordPress does not officially support; the results may be unreliable.', 'syllabplus');
			...
			if (!$header_only) {
				...
				if (count($missing_tables)>0) {
					$warn[] = sprintf(__('This database backup is missing core WordPress tables: %s', 'syllabplus'), implode(', ', $missing_tables));
				}
				if (count($skipped_tables)>0) {
					$warn[] = sprintf(__('This database backup has the following WordPress tables excluded: %s', 'syllabplus'), implode(', ', $skipped_tables));
				}
			}
		} else {
			if (empty($backup['meta_foreign'])) {
				$warn[] = __('SyllabPlus was unable to find the table prefix when scanning the database backup.', 'syllabplus');
			}
		}
		...
		if ($skip_composite_tables) {
			if (!empty($info['migration'])) {
				...
				$warn[] = sprintf(__('This backup was created on a previous SyllabPlus version (%s) which did not correctly backup tables with composite primary keys (such as the term_relationships table, which records tags and product attributes).', 'syllabplus').' '.__('Therefore it is advised that you take a fresh backup on the source site, using a later version.', 'syllabplus'), $info['created_by_version']);
			} else {
				$warn[] = sprintf(__('This backup was created on a previous SyllabPlus version (%s) which did not correctly backup tables with composite primary keys (such as the term_relationships table, which records tags and product attributes).', 'syllabplus').' '.__('Therefore, affected tables on the current site which already exist will not be replaced by default, to avoid corrupting them (you can review this in the list of tables below).', 'syllabplus'), $info['created_by_version']);
			}
		}

		$select_restore_tables = '<div class="notice below-h2 syllab-restore-option">';
		$select_restore_tables .= '<p>'.__('If you do not want to restore all your tables, then choose some to exclude here.', 'syllabplus').'(<a href="#" id="syllabplus_restore_tables_showmoreoptions">...</a>)</p>';

		$select_restore_tables .= '<div class="syllabplus_restore_tables_options_container" style="display:none;">';

		if ($db_scan_timed_out || $php_max_input_vars_exceeded) {
			if ($db_scan_timed_out) $all_other_table_title = __('The database scan was taking too long and consequently the list of all tables in the database could not be completed. This option will ensure all tables not found will be backed up.', 'syllabplus');
			if ($php_max_input_vars_exceeded) $all_other_table_title = __('The amount of database tables scanned is near or over the php_max_input_vars value so some tables maybe truncated. This option will ensure all tables not found will be backed up.', 'syllabplus');
			$select_restore_tables .= '<input class="syllab_restore_table_options" id="syllab_restore_table_slp_all_other_tables" checked="checked" type="checkbox" name="syllab_restore_table_options[]" value="slp_all_other_tables"> ';
			$select_restore_tables .= '<label for="syllab_restore_table_slp_all_other_tables"  title="'.$all_other_table_title.'">'.__('Include all tables not listed below', 'syllabplus').'</label><br>';
		}

		foreach ($tables_found as $table) {
			$checked = $skip_composite_tables && SyllabPlus_Database_Utility::table_has_composite_private_key($table) ? '' : 'checked="checked"';
			$select_restore_tables .= '<input class="syllab_restore_table_options" id="syllab_restore_table_'.$table.'" '. $checked .' type="checkbox" name="syllab_restore_table_options[]" value="'.$table.'"> ';
			$select_restore_tables .= '<label for="syllab_restore_table_'.$table.'">'.$table.'</label><br>';
		}
	...
	public function get_outgoing_ip_address($use_ipv6_service = false) {
		...
		$urls = apply_filters('syllabplus_get_outgoing_ip_address', $urls);
		foreach ($urls as $url) {
	...
	public static function get_current_clean_url() {
		...
		// Within an SyllabCentral context, there should be no prefix on the anchor link
		if (defined('SYLLABCENTRAL_COMMAND') && SYLLABCENTRAL_COMMAND || defined('WP_CLI') && WP_CLI) return '';
		...
		$remove_query_args = array('state', 'action', 'oauth_verifier', 'nonce', 'syllabplus_instance', 'access_token', 'user_id', 'syllabplus_googledriveauth');
		
		return SyllabPlus_Manipulation_Functions::wp_unslash(remove_query_arg($remove_query_args, $current_url));
	}
	...
	public function get_settings_keys() {
		// N.B. syllab_backup_history is not included here, as we don't want that wiped
		return array(
			'syllab_autobackup_default',
			'syllab_dropbox',
			'syllab_googledrive',
			'syllabplus_tmp_googledrive_access_token',
			'syllabplus_dismissedautobackup',
			...
			'syllabplus_dismissedexpiry',
			'syllabplus_dismisseddashnotice',
			'syllab_interval',
			'syllab_interval_increments',
			'syllab_interval_database',
			'syllab_retain',
			'syllab_retain_db',
			'syllab_encryptionphrase',
			'syllab_service',
			'syllab_googledrive_clientid',
			'syllab_googledrive_secret',
			'syllab_googledrive_remotepath',
			'syllab_ftp',
			'syllab_backblaze',
			'syllab_server_address',
			'syllab_dir',
			'syllab_email',
			'syllab_delete_local',
			'syllab_debug_mode',
			'syllab_include_plugins',
			'syllab_include_themes',
			'syllab_include_uploads',
			'syllab_include_others',
			'syllab_include_wpcore',
			'syllab_include_wpcore_exclude',
			'syllab_include_more',
			'syllab_include_blogs',
			'syllab_include_mu-plugins',
			'syllab_auto_updates', // since WordPress 5.5, syllab_auto_updates option is no longer used and has been removed from the code, but the HTML IDs which use the same name that represent the automatic update setting are still zealously preserved so this one cannot be removed
			'syllab_include_others_exclude',
			'syllab_include_uploads_exclude',
			'syllab_lastmessage',
			'syllab_googledrive_token',
			'syllab_dropboxtk_request_token',
			'syllab_dropboxtk_access_token',
			'syllab_adminlocking',
			'syllab_syllabvault',
			'syllab_remotesites',
			'syllab_migrator_localkeys',
			'syllab_central_localkeys',
			'syllab_retain_extrarules',
			'syllab_googlecloud',
			'syllab_include_more_path',
			'syllab_split_every',
			'syllab_ssl_nossl',
			'syllab_backupdb_nonwp',
			'syllab_extradbs',
			'syllab_combine_jobs_around',
			'syllab_last_backup',
			'syllab_starttime_files',
			'syllab_starttime_db',
			'syllab_startday_db',
			'syllab_startday_files',
			'syllab_sftp',
			'syllab_s3',
			'syllab_s3generic',
			'syllab_dreamhost',
			'syllab_s3generic_login',
			'syllab_s3generic_pass',
			'syllab_s3generic_remote_path',
			'syllab_s3generic_endpoint',
			'syllab_webdav',
			'syllab_openstack',
			'syllab_onedrive',
			'syllab_azure',
			'syllab_cloudfiles',
			'syllab_cloudfiles_user',
			'syllab_cloudfiles_apikey',
			'syllab_cloudfiles_path',
			'syllab_cloudfiles_authurl',
			'syllab_ssl_useservercerts',
			'syllab_ssl_disableverify',
			'syllab_s3_login',
			'syllab_s3_pass',
			'syllab_s3_remote_path',
			'syllab_dreamobjects_login',
			'syllab_dreamobjects_pass',
			'syllab_dreamobjects_remote_path',
			'syllab_dreamobjects',
			'syllab_report_warningsonly',
			'syllab_report_wholebackup',
			'syllab_report_dbbackup',
			'syllab_log_syslog',
			'syllab_extradatabases',
			'syllabplus_tour_cancelled_on',
			'syllabplus_version',
		);
	}
	...
	public function get_database_tables($dbsinfo = array('wp' => array())) {
		...
		foreach ($dbsinfo as $key => $value) {
			if ('wp' == $key) {
				...
				$dbhandle = new SyllabPlus_WPDB_OtherDB_Utility($dbsinfo[$key]['user'], $dbsinfo[$key]['pass'], $dbsinfo[$key]['name'], $dbsinfo[$key]['host']);
			...
			$syllabplus_database_utility = new SyllabPlus_Database_Utility($key, $table_prefix_raw, $dbhandle);
			usort($all_tables, array($syllabplus_database_utility, 'backup_db_sorttables'));
	...
	public function get_url($which_page = false) {
		switch ($which_page) {
			case 'my-account':
				return apply_filters('syllabplus_com_myaccount', 'https://syllabplus.com/my-account/');
				break;
			case 'shop':
				return apply_filters('syllabplus_com_shop', 'https://syllabplus.com/shop/');
				break;
			case 'premium':
				return apply_filters('syllabplus_com_premium', 'https://syllabplus.com/shop/syllabplus-premium/');
				break;
			case 'buy-tokens':
				return apply_filters('syllabplus_com_syllabclone_tokens', 'https://syllabplus.com/shop/syllabclone-tokens/');
				break;
			case 'lost-password':
				return apply_filters('syllabplus_com_myaccount_lostpassword', 'https://syllabplus.com/my-account/lost-password/');
				break;
			case 'mothership':
				return apply_filters('syllabplus_com_mothership', 'https://syllabplus.com/plugin-info');
				break;
			case 'shop_premium':
				return apply_filters('syllabplus_com_shop_premium', 'https://syllabplus.com/shop/syllabplus-premium/');
				break;
			case 'shop_vault_5':
				return apply_filters('syllabplus_com_shop_vault_5', 'https://syllabplus.com/shop/syllabplus-vault-storage-5-gb/');
				break;
			case 'shop_vault_15':
				return apply_filters('syllabplus_com_shop_vault_15', 'https://syllabplus.com/shop/syllabplus-vault-storage-15-gb/');
				break;
			case 'shop_vault_50':
				return apply_filters('syllabplus_com_shop_vault_50', 'https://syllabplus.com/shop/syllabplus-vault-storage-50-gb/');
				break;
			case 'anon_backups':
				return apply_filters('syllabplus_com_anon_backups', 'https://syllabplus.com/upcoming-syllabplus-feature-clone-data-anonymisation/');
				break;
			default:
	...
	public function wipe_state_data($include_locks = false) {
		...
		global $wpdb;
		if ($include_locks) {
			$wpdb->query("DELETE FROM $wpdb->options WHERE (option_name LIKE 'syllabplus_unlocked_%' OR option_name LIKE 'syllabplus_locked_%' OR option_name LIKE 'syllabplus_last_lock_time_%' OR option_name LIKE 'syllabplus_semaphore_%' OR option_name LIKE 'syllab_jobdata_%' OR option_name LIKE 'syllab_last_scheduled_%' )");
		} else {
			$sql = "DELETE FROM $wpdb->options WHERE option_name LIKE 'syllab_jobdata_%'";
			if (!empty($this->nonce)) $sql .= " AND option_name != 'syllab_jobdata_".$this->nonce."'";
			$wpdb->query($sql);
		}
	}
	...
	public function use_unminified_scripts() {
		return SyllabPlus_Options::get_syllab_option('syllab_debug_mode') || (defined('SCRIPT_DEBUG') && SCRIPT_DEBUG);
	}
	...
	public function check_restore_progress($job_time_greater_than = 120) {
		...
		if (is_array($restore_jobdata) && !empty($restore_jobdata)) {
			...
			if (isset($restore_jobdata['job_type']) && 'restore' == $restore_jobdata['job_type'] && isset($restore_jobdata['second_loop_entities']) && !empty($restore_jobdata['second_loop_entities']) && isset($restore_jobdata['job_time_ms']) && (time() - $restore_jobdata['job_time_ms'] > $job_time_greater_than || (defined('SYLLABPLUS_RESTORE_PROGRESS_ALWAYS_SHOW') && SYLLABPLUS_RESTORE_PROGRESS_ALWAYS_SHOW)) && time() - $restore_jobdata['job_time_ms'] < 86400 && (empty($_REQUEST['action']) || ('syllab_restore' != $_REQUEST['action'] && 'syllab_restore_continue' != $_REQUEST['action']))) {
	...
	public function get_hosting_info() {
		...
		return apply_filters('syllabplus_get_hosting_info', $hosting_company);
	}
	...
	/**
	 * Maintain compatibility on all versions between WordPress and SyllabPlus, specifically since WordPress 5.5
	 */
	public function wordpress_55_updates_potential_migration() {
		// Due to the new WP's auto-updates interface in WordPress version 5.5, we need to maintain the auto update compatibility on all versions of WordPress and SyllabPlus
		$slp_saved_version = SyllabPlus_Options::get_syllab_option('syllabplus_version');
		$syllab_auto_updates = SyllabPlus_Options::get_syllab_option('syllab_auto_updates');
		if (!$slp_saved_version || version_compare($slp_saved_version, '1.16.34', '<=') || (version_compare($slp_saved_version, '2.0.0', '>=') && version_compare($slp_saved_version, '2.16.34', '<=')) || null !== $syllab_auto_updates) {
			$this->replace_auto_updates_option();
		}
	}

	/**
	 * Remove the use of syllab_auto_updates option/meta (single & multisite) and replace it with auto_update_plugins site option that is used in WordPress's core since version 5.5
	 * This needs to be done in order to maintain auto-updates compatibility between WordPress and Syllabplus and to synchronise the auto-updates setting for both
	 */
	private function replace_auto_updates_option() {
		$old_setting_value = SyllabPlus_Options::get_syllab_option('syllab_auto_updates');
		SyllabPlus_Options::delete_syllab_option('syllab_auto_updates');
		$new_setting_value = (array) get_site_option('auto_update_plugins', array());
		if (!empty($old_setting_value)) $new_setting_value[] = basename(SYLLABPLUS_DIR).'/syllabplus.php';
	...
	public function set_automatic_updates($value) {
		...
		if (!empty($value)) {
			$auto_update_plugins[] = basename(SYLLABPLUS_DIR).'/syllabplus.php';
			$auto_update_plugins = array_unique($auto_update_plugins);
		} else {
			$auto_update_plugins = array_diff($auto_update_plugins, array(basename(SYLLABPLUS_DIR).'/syllabplus.php'));
		}
		update_site_option('auto_update_plugins', $auto_update_plugins);
	}

	/**
	 * Check whether the automatic-updates is set for SyllabPlus
	 *
	 * @return Boolean True if set, false otherwise
	 */
	public function is_automatic_updating_enabled() {
		$auto_update_plugins = (array) get_site_option('auto_update_plugins', array());
		return in_array(basename(SYLLABPLUS_DIR).'/syllabplus.php', $auto_update_plugins, true);
	}
