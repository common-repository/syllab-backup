...
class SyllabPlus_Tour {
	...
	public function load_tour($hook) {
		...
		$tour_data = array(
			...
			'settings_remote_storage' => array(
				...
				'text' => __("Now select a remote storage destination to protect against server-wide threats. If not, your backups remain on the same server as your site.", 'syllabplus')
					...
					.'<p><a href="javascript:void(0);" class="button button-primary">'.__('Try SylLab Vault for only 6 dollars a month!', 'syllabplus').'</a></p>'
					.'</div>'
			),
			'settings_more' => array(
		...
		if (isset($_REQUEST['tab'])) {
			$tour_data['show_tab_on_load'] = '#syllab-navtab-'.esc_attr(sanitize_text_field($_REQUEST['tab']));
		}
		...
		if ($syllabplus_addons2 && method_exists($syllabplus_addons2, 'connection_status')) {
			...
			$tour_data['settings_remote_storage'] = array(
				...
				'text' => __("Now select a remote storage destination to protect against server-wide threats. If not, your backups remain on the same server as your site.", 'syllabplus')
					...
					.' <a href="'.apply_filters('syllabplus_com_link', 'https://syllabplus.com/shop/syllabplus-vault-storage-5-gb/').'" target="_blank" '.$checkout_embed_5gb_attribute.'>'.__('Otherwise, you can try SyllabVault for 1 month for only $1!', 'syllabplus').'</a>'
					.'</div>'
			);
