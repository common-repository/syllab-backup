...
class SyllabPlus_ftp_wrapper {
	...
	public function connect() {
		...
		if (time() - $time_start > 19) {
			...
			if (isset($syllabplus_admin->logged) && is_array($syllabplus_admin->logged)) {
				$syllabplus_admin->logged[] = sprintf(__('The %s connection timed out; if you entered the server correctly, then this is usually caused by a firewall blocking the connection - you should check with your web hosting company.', 'syllabplus'), 'FTP');
			} else {
				global $syllabplus;
				$syllabplus->log(sprintf(__('The %s connection timed out; if you entered the server correctly, then this is usually caused by a firewall blocking the connection - you should check with your web hosting company.', 'syllabplus'), 'FTP'), 'error');
			}
		}
	...
	public function put($local_file_path, $remote_file_path, $mode = FTP_BINARY, $resume = false, $syllabplus = false) {
		...
		if ($resume) {
			...
			if ($existing_size <=0) {
				...
				if (is_a($syllabplus, 'SyllabPlus')) $syllabplus->log("File already exists at remote site: size $existing_size. Will attempt resumption.");
				if ($existing_size >= $file_size) {
					if (is_a($syllabplus, 'SyllabPlus')) $syllabplus->log("File is apparently already completely uploaded");
					return true;
				}
			}
		}
		...
		while (FTP_MOREDATA == $ret) {
			if (is_a($syllabplus, 'SyllabPlus')) {
				...
				if ($new_size - $existing_size > $record_after) {
					...
					$syllabplus->record_uploaded_chunk($percent, '', $local_file_path);
				}
			}
			// Continue upload
		...
		if (FTP_FINISHED != $ret) {
			if (is_a($syllabplus, 'SyllabPlus')) $syllabplus->log("FTP upload: error ($ret)");
			return false;
		}
	...
	public function get($local_file_path, $remote_file_path, $mode = FTP_BINARY, $resume = false, $syllabplus = false) {
		...
		while (FTP_MOREDATA == $ret) {
			...
			if ($syllabplus) {
				...
				if ($file_now_size - $file_last_size > 524288) {
					$syllabplus->log("FTP fetch: file size is now: ".sprintf("%0.2f", filesize($local_file_path)/1048576)." Mb");
		...
		if (FTP_FINISHED == $ret) {
			if ($syllabplus) $syllabplus->log("FTP fetch: fetch complete");
			return true;
		} else {
			if ($syllabplus) $syllabplus->log("FTP fetch: fetch failed");
			return false;
		}
