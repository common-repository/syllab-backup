...
$ud_rpc = new SyllabPlus_Remote_Communications($name_indicator); // $name_indicator is a key indicator - indicating which key is being used.
...
$ud_rpc = new SyllabPlus_Remote_Communications($name_indicator); // $name_indicator is a key indicator - indicating which key is being used.
$ud_rpc->set_option_name('slrpc_remotekey');
if (!$ud_rpc->get_key_remote()) throw new Exception('...');
$encrypted = $ud_rpc->encrypt_message('blah blah');

// Generate a new key
$ud_rpc = new SyllabPlus_Remote_Communications('myindicator.example.com');
$ud_rpc->set_option_name('slrpc_localkey'); // Save as a WP site option
...
add_filter('slrpc_command_somecommand', 'my_function', 10, 3);
// function my_function($response, $data, $name_indicator) { ... ; return array('response' => 'my_reply', 'data' => 'any mixed data'); }
// Or:
// add_filter('slrpc_action', 'some_function', 10, 4); // Function must return something other than false to indicate that it handled the specific command. Any returned value will be sent as the reply.
// function some_function($response, $command, $data, $name_indicator) { ...; return array('response' => 'my_reply', 'data' => 'any mixed data'); }
$ud_rpc->set_option_name('slrpc_local_private_key');
$ud_rpc->activate_replay_protection();
...
class SyllabPlus_Remote_Communications {
	...
	private function ensure_crypto_loaded() {
		if (!class_exists('Crypt_Rijndael') || !class_exists('Crypt_RSA') || !class_exists('Crypt_Hash')) {
			...
			if (is_a($syllabplus, 'SyllabPlus')) {
				// Since May 2019, the second parameter is unused; but, since we don't know the version, we send it.
				$ensure_phpseclib = $syllabplus->ensure_phpseclib(array('Crypt_Rijndael', 'Crypt_RSA', 'Crypt_Hash'), array('Crypt/Rijndael', 'Crypt/RSA', 'Crypt/Hash'));
				if (is_wp_error($ensure_phpseclib)) return $ensure_phpseclib;
			} elseif (defined('SYLLABPLUS_DIR') && file_exists(SYLLABPLUS_DIR.'/vendor/phpseclib/phpseclib/phpseclib')) {
				$pdir = SYLLABPLUS_DIR.'/vendor/phpseclib/phpseclib/phpseclib';
	...
	private function log($message, $level = 'notice') {
		...
		do_action('slrpc_log', $message, $level, $this->key_name_indicator, $this->debug, $this);
		if ('info' != $level) error_log('SLRPC ('.$this->key_name_indicator.", $level): $message");
	}
	...
	public function encrypt_message($plaintext, $use_key = false, $key_length = 32) {
		...
		if (defined('SLRPC_PHPSECLIB_ENCRYPTION_MODE')) $rsa->setEncryptionMode(SLRPC_PHPSECLIB_ENCRYPTION_MODE);
	...
	public function http_post($post_options) {
		...
		if (is_a($this->http_transport, 'GuzzleHttp\Client')) {
			...
			$guzzle_options = array(
				...
				'headers' => array(
					'User-Agent' => 'WordPress/'.$wp_version.'; class-slrpc.php-Guzzle/'.$this->version.'; '.get_bloginfo('url'),
				),
				'exceptions' => false,
			...
			if (defined('SLRPC_GUZZLE_SSL_VERIFY')) {
				$verify = SLRPC_GUZZLE_SSL_VERIFY;
			...
			$guzzle_options['verify'] = apply_filters('slrpc_guzzle_verify', $verify);
			...
			$response = $client->post($this->destination_url, apply_filters('slrpc_guzzle_options', $guzzle_options, $this));
			...
			$post_options['user-agent'] = 'WordPress/'.$wp_version.'; class-slrpc.php/'.$this->version.'; '.get_bloginfo('url');
	...
	public function send_message($command, $data = null, $timeout = 20) {
		...
		$post_options = apply_filters('slrpc_post_options', $post_options, $command, $data, $timeout, $this);
		...
		if (!is_array($decoded) || empty($decoded['slrpc_message'])) return new WP_Error('response_not_understood', 'Response from remote site was not in the expected format ('.$post['body'].')', $decoded);
		...
		if ($this->format >= 2) {
			...
			if (!$this->verify_signature($decoded['slrpc_message'], $decoded['signature'], $this->key_remote)) {
				$this->log('Signature verification failed; discarding');
				die;
			}
		}

		$decoded = $this->decrypt_message($decoded['slrpc_message']);

		if (!is_string($decoded)) return new WP_Error('not_decrypted', 'Response from remote site was not successfully decrypted', $decoded['slrpc_message']);
		...
		if (isset($json_decoded['incoming_rand']) && !empty($this->message_random_number) && $json_decoded['incoming_rand'] != $this->message_random_number) {
			// @codingStandardsIgnoreLine
			$this->log('SLRPC: Message mismatch (possibly MITM) (sent_rand=' + $this->message_random_number + ', returned_rand='.$json_decoded['incoming_rand'].'): dropping', 'error');
	...
	public function create_listener() {
		...
		if ((!empty($_POST) && !empty($_POST['slrpc_message']) && !empty($_POST['format'])) || (!empty($_SERVER['REQUEST_METHOD']) && 'OPTIONS' == $_SERVER['REQUEST_METHOD'] && $http_origin)) {
	...
	public function wp_loaded_final() {
		...
		$message_for = empty(sanitize_text_field($_POST['key_name'])) ? '' : (string) sanitize_text_field($_POST['key_name']);
	...
	public function wp_loaded() {
		...
		*/
		...
		$http_origin = function_exists('get_http_origin') ? get_http_origin() : (empty($_SERVER['HTTP_ORIGIN']) ? '' : $_SERVER['HTTP_ORIGIN']);
		if (!empty($_SERVER['REQUEST_METHOD']) && 'OPTIONS' == sanitize_text_field($_SERVER['REQUEST_METHOD']) && $http_origin) {
			if (in_array($http_origin, $this->allow_cors_from)) {
				// @codingStandardsIgnoreLine
				if (!defined('SLRPC_DO_NOT_SEND_CORS_HEADERS') || !SLRPC_DO_NOT_SEND_CORS_HEADERS) {
		...
		if (empty(sanitize_text_field($_POST['format'])) || (1 != $_POST['format'] && 2 != $_POST['format'])) return;
		...
		*/
		...
		// $slrpc_message = function_exists('wp_unslash') ? wp_unslash($_POST['slrpc_message']) : stripslashes_deep($_POST['slrpc_message']);
		
		// Data should not have any slashes - it is base64-encoded
		$slrpc_message = (string) $_POST['slrpc_message'];
		...
		if ($format >= 2) {
			if (empty(sanitize_text_field($_POST['signature']))) {
			...
			if (!$this->verify_signature($slrpc_message, $_POST['signature'], $this->key_remote)) {
		...
		try {
			$slrpc_message = $this->decrypt_message($slrpc_message);
		} catch (Exception $e) {
			$this->log('Exception ('.get_class($e).'): '.$e->getMessage(), 'error');
			die;
		}

		$slrpc_message = json_decode($slrpc_message, true);

		if (empty($slrpc_message) || !is_array($slrpc_message) || empty($slrpc_message['command']) || !is_string($slrpc_message['command'])) {
			$this->log('Could not decode JSON on incoming message', 'error');
			die;
		}

		if (empty($slrpc_message['time'])) {
		...
		if (empty($slrpc_message['key_name']) || $_POST['key_name'] != $slrpc_message['key_name']) {
			$this->log('key_name mismatch between encrypted and unencrypted portions', 'error');
			die;
		}

		if ($this->extra_replay_protection) {
			$message_hash = $this->calculate_message_hash((string) $_POST['slrpc_message']);
			if ($this->message_hash_seen($message_hash)) {
		...
		$time_difference = absint($slrpc_message['time'] - time());
		...
		if ($this->sequence_protection_tolerance) {
			...
			if (!isset($slrpc_message['sequence_id']) || !is_numeric($slrpc_message['sequence_id'])) {
				$this->log('a numerical sequence number is required, but none was included in the message - dropping', 'error');
				die;
			}

			$message_sequence_id = (int) $slrpc_message['sequence_id'];
		...
		$this->incoming_message = $slrpc_message;

		$command = (string) $slrpc_message['command'];
		$data = empty($slrpc_message['data']) ? null : $slrpc_message['data'];

		// @codingStandardsIgnoreLine
		if ($http_origin && !empty($slrpc_message['cors_headers_wanted']) && (!defined('SLRPC_DO_NOT_SEND_CORS_HEADERS') || !SLRPC_DO_NOT_SEND_CORS_HEADERS)) {
			header("Access-Control-Allow-Origin: $http_origin");
			header('Access-Control-Allow-Credentials: true');
		}
		...
		if ('ping' == $command) {
			...
			if (has_filter('slrpc_command_'.$command)) {
				$response = apply_filters('slrpc_command_'.$command, null, $data, $this->key_name_indicator);
			} else {
				$response = array('response' => 'rpcerror', 'data' => array('code' => 'unknown_rpc_command', 'data' => $command));
			}
		}

		$response = apply_filters('slrpc_action', $response, $command, $data, $this->key_name_indicator, $this);

		if (is_array($response)) {

			if ($this->debug) {
				$this->log('SLRPC response (pre-encoding/encryption): '.serialize($response));
			}
			...
			do_action('slrpc_action_send_response', $final_response, $command);
	...
	private function message_hash_seen($message_hash) {
		...
		$transient_name = 'slrpch_'.md5($this->key_name_indicator);
