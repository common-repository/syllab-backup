...
class SyllabPlus_Encryption {
	...
	public static function decrypt($fullpath, $key, $to_temporary_file = false) {
		...
		$ensure_phpseclib = $syllabplus->ensure_phpseclib('Crypt_Rijndael');
		
		if (is_wp_error($ensure_phpseclib)) {
			$syllabplus->log("Failed to load phpseclib classes (".$ensure_phpseclib->get_error_code()."): ".$ensure_phpseclib->get_error_message());
			$syllabplus->log("Failed to load phpseclib classes (".$ensure_phpseclib->get_error_code()."): ".$ensure_phpseclib->get_error_message(), 'error');
			return false;
		}
		...
		if (defined('SYLLABPLUS_DECRYPTION_ENGINE')) {
			if ('openssl' == SYLLABPLUS_DECRYPTION_ENGINE) {
				$rijndael->setPreferredEngine(CRYPT_ENGINE_OPENSSL);
			} elseif ('mcrypt' == SYLLABPLUS_DECRYPTION_ENGINE) {
				$rijndael->setPreferredEngine(CRYPT_ENGINE_MCRYPT);
			} elseif ('internal' == SYLLABPLUS_DECRYPTION_ENGINE) {
		...
		$bytes_decrypted = 0;
		$buffer_size = defined('SYLLABPLUS_CRYPT_BUFFER_SIZE') ? SYLLABPLUS_CRYPT_BUFFER_SIZE : 2097152;
		...
		while ($bytes_decrypted < $file_size) {
			...
			if (0 == $bytes_decrypted) {
				if (SyllabPlus_Manipulation_Functions::str_ends_with($fullpath, '.gz.crypt')) {
				...
				} elseif (SyllabPlus_Manipulation_Functions::str_ends_with($fullpath, '.zip.crypt')) {
	...
	public static function encrypt($fullpath, $key) {
		...
		if (!function_exists('mcrypt_encrypt') && !extension_loaded('openssl')) {
			$syllabplus->log(sprintf(__('Your web-server does not have the %s module installed.', 'syllabplus'), 'PHP/mcrypt / PHP/OpenSSL').' '.__('Without it, encryption will be a lot slower.', 'syllabplus'), 'warning', 'nocrypt');
		}
		...
		if (false === ($file_handle = fopen($fullpath, 'rb'))) {
			$syllabplus->log("Failed to open file for read access: $fullpath");
			return false;
		}
		...
		if (file_exists($encrypted_path)) {
			...
			$syllabplus->log("Temporary encryption file found, will try to resume the encryption");
			...
			if ($data_encrypted >= $block_length) {
				...
				if (false === ($encrypted_handle = fopen($encrypted_path, 'rb+'))) {
					$syllabplus->log("Failed to open file for write access on resumption: $encrypted_path");
					$resumption = false;
				}
				...
				if (-1 === (fseek($encrypted_handle, $data_encrypted - ($block_length + $partial_data_size)))) {
					$syllabplus->log("Failed to move file pointer to correct position to get IV: $encrypted_path");
					$resumption = false;
				}

				// Read previous block length from file
				if (false === ($iv = fread($encrypted_handle, $block_length))) {
					$syllabplus->log("Failed to read from file to get IV: $encrypted_path");
					$resumption = false;
				}
				...
				if (-1 === (fseek($file_handle, $true_data_encrypted))) {
					$syllabplus->log("Failed to move file pointer to correct position to resume encryption: $fullpath");
					$resumption = false;
				}
			...
			if (!$resumption) {
				$syllabplus->log("Could not resume the encryption will now try to start again");
				...
				if (-1 === (fseek($file_handle, 0))) {
					$syllabplus->log("Failed to move file pointer to start position to restart encryption: $fullpath");
					$resumption = false;
				}
			}
		}
		...
		if (!$resumption) {
			...
			if (false === ($encrypted_handle = fopen($encrypted_path, 'wb+'))) {
				$syllabplus->log("Failed to open file for write access: $encrypted_path");
				return false;
			}
		}
		...
		while ($data_encrypted < $file_size) {
			...
			if (false === ($file_part = fread($file_handle, $buffer_size))) {
				$syllabplus->log("Failed to read from file: $fullpath");
				return false;
			}
			...
			if (false === fwrite($encrypted_handle, $encrypted_data)) {
				$syllabplus->log("Failed to write to file: $encrypted_path");
				return false;
			}
			...
			if ($time_since_last_logged > 5) {
				...
				$syllabplus->log("Encrypting file: completed $data_encrypted bytes");
			}
		...
		if (false === rename($encrypted_path, $result_path)) {
			$syllabplus->log("File rename failed: $encrypted_path -> $result_path");
			return false;
		}
	...
	public static function spool_crypted_file($fullpath, $encryption) {
		...
		if ('' == $encryption) $encryption = SyllabPlus_Options::get_syllab_option('syllab_encryptionphrase');
		
		if ('' == $encryption) {
			header('Content-type: text/plain');
			_e("Decryption failed. The database file is encrypted, but you have no encryption key entered.", 'syllabplus');
			$syllabplus->log('Decryption of database failed: the database file is encrypted, but you have no encryption key entered.', 'error');
		} else {
			...
			if (is_array($decrypted_file)) {
				...
				header('Content-type: text/plain');
				echo __("Decryption failed. The most likely cause is that you used the wrong key.", 'syllabplus')." ".__('The decryption key used:', 'syllabplus').' '.$encryption;
