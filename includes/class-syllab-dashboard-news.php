...
if (!class_exists('Syllab_Dashboard_News')) :
/**
 * Handles all stuffs related to Dashboard News
 */
class Syllab_Dashboard_News {
	...
	/**
	 * constructor of class Syllab_Dashboard_News
	 *
	...
	private function do_ajax_dashboard_news() {
		$ajax_callback_page = !empty(sanitize_text_field($_GET['pagenow'])) ? sanitize_text_field($_GET['pagenow']) : '';
		return (in_array($ajax_callback_page, $this->valid_callback_pages) && !empty(sanitize_text_field($_GET['widget'])) && 'dashboard_primary' == sanitize_text_field($_GET['widget']));
	}
	...
	private function get_dashboard_news_html() {
		...
		$formatted_news = str_replace('<li>', '<li class="'.$this->slug.'_dashboard_news_item">'.'<a href="'.SyllabPlus::get_current_clean_url().'" class="dashicons dashicons-no-alt" title="'.esc_attr($this->translations['dismiss_tooltip']).'" onClick="'.$this->slug.'_dismiss_dashboard_news(); return false;" style="float: right; box-shadow: none; margin-left: 5px;"></a>', $formatted_news);
		...
		function <?php echo $this->slug; ?>_dismiss_dashboard_news() {
			if (confirm("<?php echo esc_js($this->translations['dismiss_confirm']); ?>")) {
				jQuery.ajax({
					...
					data : {
						action: '<?php echo esc_html($this->slug); ?>_ajax_dismiss_dashboard_news',
						nonce : '<?php echo wp_create_nonce($this->slug.'-dismiss-news-nonce');?>'
					},
					success: function(response) {
						jQuery('.<?php echo esc_html($this->slug); ?>_dashboard_news_item').slideUp('slow');
					},
					error: function(response, status, error_code) {
						console.log("<?php echo esc_html($this->slug); ?>_dismiss_dashboard_news: error: "+status+" ("+error_code+")");
