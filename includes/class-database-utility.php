...
class SyllabPlus_Database_Utility {
	...
	public static function table_has_composite_private_key($table, $wpdb_obj = null) {
		...
		$table_structure = $wpdb->get_results("DESCRIBE ".SyllabPlus_Manipulation_Functions::backquote($table));
	...
	public static function set_sql_mode($modes = array(), $remove_modes = array(), $db_handle = null) {
		...
		if (is_scalar($initial_modes_str) && !is_bool($initial_modes_str)) {
			...
			$syllabplus->log("Couldn't get the sql_mode value (".serialize($initial_modes_str).")");
		...
		if (isset($initial_modes_str) && false == array_diff(explode(',', $initial_modes_str), $modes)) {
			$syllabplus->log("SQL compatibility mode is: $modes_str");
		} else {
			$syllabplus->log("SQL compatibility mode".((false === $res) ? " not" : "")." successfully changed".(isset($initial_modes_str) ? " from $initial_modes_str" : "")." to $modes_str");
		}
	}
	...
	public static function is_generated_column_supported($engine = '') {
		...
		$random_table_name = $table_prefix.'syllab_tmp_'.rand(0, 9999999).md5(microtime(true));
		...
		$sql = array(
			...
			"CREATE INDEX `idx_wp_slp_generated_column_test_generated_column` ON `$random_table_name` (virtual_column) COMMENT 'virtual_column' ALGORITHM DEFAULT LOCK DEFAULT;",
		);
	...
	public static function is_stored_routine_supported() {
		...
		$function_name = 'syllab_test_stored_routine';
		$sql = array(
		...
		if ($is_stored_routine_supported) {
			...
			$is_stored_routine_supported = new WP_Error('routine_creation_error', sprintf(__('An error occurred while attempting to check the support of stored routines creation (%s %s)', 'syllabplus'), $wpdb->last_error.' -', $sql['CREATE_FUNCTION']));
		}
		$wpdb->suppress_errors($old_val);
	...
	public static function get_stored_routines() {
		...
		$old_val = $wpdb->suppress_errors();
		try {
			$err_msg = __('An error occurred while attempting to retrieve routine status (%s %s)', 'syllabplus');
			...
			foreach ((array) $stored_routines as $key => $routine) {
				...
				if (!empty($wpdb->last_error)) throw new Exception(sprintf(__('An error occurred while attempting to retrieve the routine SQL/DDL statement (%s %s)', 'syllabplus'), $wpdb->last_error.' -', $wpdb->last_query), 1);
...
class SyllabPlus_WPDB_OtherDB_Utility extends wpdb {
	/**
