 ...
 */
class SyllabPlus_Manipulation_Functions {
	...
	/**
	 * This options filter removes ABSPATH off the front of syllab_dir, if it is given absolutely and contained within it
	 *
	 * @param  String $syllab_dir Directory
	 * @return String
	 */
	public static function prune_syllab_dir_prefix($syllab_dir) {
		if ('/' == substr($syllab_dir, 0, 1) || "\\" == substr($syllab_dir, 0, 1) || preg_match('/^[a-zA-Z]:/', $syllab_dir)) {
			$wcd = trailingslashit(WP_CONTENT_DIR);
			if (strpos($syllab_dir, $wcd) === 0) {
				$syllab_dir = substr($syllab_dir, strlen($wcd));
			}
		}
		return $syllab_dir;
	}
