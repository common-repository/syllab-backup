...
/**
 * We just add a last_error variable for comaptibility with our SyllabPlus_PclZip object
 */
class SyllabPlus_ZipArchive extends ZipArchive {
...
class SyllabPlus_PclZip {
	...
	public function __construct() {
		...
		global $syllabplus;
		if (!defined('PCLZIP_TEMPORARY_DIR')) define('PCLZIP_TEMPORARY_DIR', trailingslashit($syllabplus->backups_dir_location()));
	}
	 ...
	 */
	public function syllabplus_getFromIndexBulk($indexes) {
	...
	public function getFromIndex($index, $length = 0, $flags = 0) {
		...
		if ($flags) {
			error_log("A call to SyllabPlus_PclZip::getFromIndex() set flags=$flags, but this is not implemented");
		}
	...
	public function open($path, $flags = 0) {
		...
		if (!is_dir($syllab_dir.'/emptydir') && !mkdir($syllab_dir.'/emptydir')) {
			$this->last_error = "Could not create empty directory ($syllab_dir/emptydir)";
			return false;
		}
	...
	public function close() {
		...
		foreach ($this->adddirs as $dir) {
			if (false == $this->pclzip->add($syllab_dir.'/emptydir', PCLZIP_OPT_REMOVE_PATH, $syllab_dir.'/emptydir', PCLZIP_OPT_ADD_PATH, $dir)) {
				$this->last_error = $this->pclzip->errorInfo(true);
				return false;
			}
			$activity = true;
...
class SyllabPlus_BinZip extends SyllabPlus_PclZip {
	...
	public function __construct() {
		...
		$this->binzip = $syllabplus_backup->binzip;
	...
	public function addFile($file, $add_as) {
		...
		$base = SyllabPlus_Manipulation_Functions::str_lreplace($add_as, '', $file);

		if ($file == $base) {
			// Shouldn't happen; but see: https://bugs.php.net/bug.php?id=62119
			$syllabplus->log("File skipped due to unexpected name mismatch (locale: ".setlocale(LC_CTYPE, "0")."): file=$file add_as=$add_as", 'notice', false, true);
		} else {
	...
	public function close() {
		...
		global $syllabplus, $syllabplus_backup;
		...
		$something_useful_happened = $syllabplus->something_useful_happened;
		...
		if (0 == count($this->addfiles) && 0 < count($this->adddirs)) {
			$dir = realpath($syllabplus_backup->make_zipfile_source);
			$this->addfiles[$dir] = '././.';
		}
		...
		foreach ($this->addfiles as $rdirname => $files) {
			...
			if (!is_resource($process)) {
				$syllabplus->log('BinZip error: proc_open failed');
			...
			while ((!feof($pipes[1]) || !feof($pipes[2]) || (is_array($files) && count($files)>0)) && false !== ($changes = @stream_select($read, $write, $except, 0, 200000))) {// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged, VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
				...
				if (is_array($read) && in_array($pipes[1], $read)) {
					...
					if ($w && $syllabplus_backup->debug) $syllabplus->log("Output from zip: ".trim($w), 'debug');
					...
					if (file_exists($this->path)) {
						...
						if ($new_size > $last_size*1.2 || $new_size > $last_size + 52428800) {
							$syllabplus->log(basename($this->path).sprintf(": size is now: %.2f MB", round($new_size/1048576, 1)));
							$last_size = $new_size;
						}
					}
				}
			...
			if (0 != $ret && 12 != $ret) {
				if ($ret < 128) {
					$syllabplus->log("Binary zip: error (code: $ret - look it up in the Diagnostics section of the zip manual at http://infozip.sourceforge.net/FAQ.html#error-codes for interpretation... and also check that your hosting account quota is not full)");
				} else {
					$syllabplus->log("Binary zip: error (code: $ret - a code above 127 normally means that the zip process was deliberately killed ... and also check that your hosting account quota is not full)");
				}
				if (!empty($w) && !$syllabplus_backup->debug) $syllabplus->log("Last output from zip: ".trim($w), 'debug');
				return false;
			}
