 ...
 */
class SyllabPlus_Filesystem_Functions {
	...
	public static function recursive_directory_size($directorieses, $exclude = array(), $basedirs = '', $format = 'text') {
		...
		return SyllabPlus_Manipulation_Functions::convert_numeric_size_to_text($size);
	...
	public static function ensure_wp_filesystem_set_up_for_restore($url_parameters = array()) {
		...
		$build_url = SyllabPlus_Options::admin_page().'?page=syllabplus&action=syllab_restore';
		...
		if (!WP_Filesystem($credentials)) {
			...
			$syllabplus->log("Filesystem credentials are required for WP_Filesystem");
			...
			if (false !== strpos($build_url, 'syllabplus_ajax_restore=do_ajax_restore')) $build_url = str_replace('syllabplus_ajax_restore=do_ajax_restore', 'syllabplus_ajax_restore=continue_ajax_restore', $build_url);
			
			request_filesystem_credentials($build_url, '', true, false);
			
			if ($wp_filesystem->errors->get_error_code()) {
				echo '<div class="restore-credential-errors">';
				echo '<p class="restore-credential-errors--link"><em><a href="' . apply_filters('syllabplus_com_link', "https://syllabplus.com/faqs/asked-ftp-details-upon-restorationmigration-updates/") . '" target="_blank">' . __('Why am I seeing this?', 'syllabplus') . '</a></em></p>';
	...
	public static function web_server_disk_space($will_immediately_calculate_disk_space = true) {
		if ($will_immediately_calculate_disk_space) {
			$disk_space_used = self::get_disk_space_used('syllab', 'numeric');
			if ($disk_space_used > apply_filters('syllabplus_display_usage_line_threshold_size', 104857600)) { // 104857600 = 100 MB = (100 * 1024 * 1024)
				$disk_space_text = SyllabPlus_Manipulation_Functions::convert_numeric_size_to_text($disk_space_used);
				$refresh_link_text = __('refresh', 'syllabplus');
			...
			$disk_space_text = '';
			$refresh_link_text = __('calculate', 'syllabplus');
			return self::web_server_disk_space_html($disk_space_text, $refresh_link_text);
		}
	}
	...
	public static function web_server_disk_space_html($disk_space_text, $refresh_link_text) {
		return '<li class="syllab-server-disk-space" title="'.esc_attr__('This is a count of the contents of your Syllab directory', 'syllabplus').'"><strong>'.__('Web-server disk space in use by SylLab Backup', 'syllabplus').':</strong> <span class="syllab_diskspaceused"><em>'.$disk_space_text.'</em></span> <a class="syllab_diskspaceused_update" href="#">'.$refresh_link_text.'</a></li>';
	}
	
	/**
	 * Cleans up temporary files found in the syllab directory (and some in the site root - pclzip)
	...
	public static function clean_temporary_files($match = '', $older_than = 43200, $include_cachelist = false) {
		...
		if ($older_than > 10000) {
			...
			$all_jobs = $wpdb->get_results("SELECT $key_column, $value_column FROM $table WHERE $key_column LIKE 'syllab_jobdata_%' LIMIT 100", ARRAY_A);
			
			foreach ($all_jobs as $job) {
				$nonce = str_replace('syllab_jobdata_', '', $job[$key_column]);
				...
				if (isset($val['temp_import_table_prefix']) && '' != $val['temp_import_table_prefix'] && $wpdb->prefix != $val['temp_import_table_prefix']) {
					...
					foreach ($tables_to_remove as $table_name) {
						$wpdb->query('DROP TABLE '.SyllabPlus_Manipulation_Functions::backquote($table_name));
					}
				}
				if ($delete) {
					delete_site_option($job[$key_column]);
					delete_site_option('syllabplus_semaphore_'.$nonce);
				}
			}
		}
		...
		$now_time = time();
		$files_deleted = 0;
		if ($handle = opendir($syllab_dir)) {
			while (false !== ($entry = readdir($handle))) {
				$manifest_match = preg_match("/syllabplus-manifest.json/", $entry);
				...
				if ((preg_match("/$match\.(tmp|table|txt\.gz)(\.gz)?$/i", $entry) || $cachelist_match || $ziparchive_match || $binzip_match || $manifest_match || $browserlog_match) && is_file($syllab_dir.'/'.$entry)) {
					...
					if (($match && ($ziparchive_match || $binzip_match || $cachelist_match || $manifest_match || 0 == $older_than) && $now_time-filemtime($syllab_dir.'/'.$entry) >= $older_than) || $now_time-filemtime($syllab_dir.'/'.$entry)>43200) {
						$skip_dblog = (0 == $files_deleted % 25) ? false : true;
						$syllabplus->log("Deleting old temporary file: $entry", 'notice', false, $skip_dblog);
						@unlink($syllab_dir.'/'.$entry);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
						$files_deleted++;
					}
				} elseif (preg_match('/^log\.[0-9a-f]+\.txt$/', $entry) && $now_time-filemtime($syllab_dir.'/'.$entry)> apply_filters('syllabplus_log_delete_age', 86400 * 40, $entry)) {
					$skip_dblog = (0 == $files_deleted % 25) ? false : true;
					$syllabplus->log("Deleting old log file: $entry", 'notice', false, $skip_dblog);
					@unlink($syllab_dir.'/'.$entry);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
					$files_deleted++;
				}
			}
		...
		// Since 1.9.32, we set them to go into $syllab_dir, so now we must check there too. Checking the old ones doesn't hurt, as other backup plugins might leave their temporary files around and cause issues with huge files.
		foreach (array(ABSPATH, ABSPATH.'wp-admin/', $syllab_dir.'/') as $path) {
			if ($handle = opendir($path)) {
				while (false !== ($entry = readdir($handle))) {
					...
					if (preg_match("/^pclzip-[a-z0-9]+.tmp$/", $entry) && $now_time-filemtime($path.$entry) >= 900) {
						$syllabplus->log("Deleting old PclZip temporary file: $entry (from ".basename($path).")");
	...
	public static function gzopen_for_read($file, &$warn, &$err) {
		if (!function_exists('gzopen') || !function_exists('gzread')) {
			...
			$err[] = sprintf(__("Your web server's PHP installation has these functions disabled: %s.", 'syllabplus'), $missing).' '.sprintf(__('Your hosting company must enable these functions before %s can work.', 'syllabplus'), __('restoration', 'syllabplus'));
			return false;
		}
		...
		$mess = __('The database file appears to have been compressed twice - probably the website you downloaded it from had a mis-configured webserver.', 'syllabplus');
		...
		if (false === ($fnew = fopen($file.".tmp", 'w')) || !is_resource($fnew)) {
			...
			@gzclose($dbhandle);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			$err_msg = __('The attempt to undo the double-compression failed.', 'syllabplus');
			...
			if (!rename($file.".tmp", $file)) {
				...
				$mess .= ' '.__('The attempt to undo the double-compression succeeded.', 'syllabplus');
	...
	public static function get_disk_space_used($entity, $format = 'text') {
		global $syllabplus;
		if ('syllab' == $entity) return self::recursive_directory_size($syllabplus->backups_dir_location(), array(), '', $format);
		...
		if ('all' == $entity) {
			...
			foreach ($backupable_entities as $entity => $data) {
				...
				$dirs = apply_filters('syllabplus_dirlist_'.$entity, $basedir);
				$size = self::recursive_directory_size($dirs, $syllabplus->get_exclude($entity), $basedir, 'numeric');
			...
			if ('numeric' == $format) {
				...
				return SyllabPlus_Manipulation_Functions::convert_numeric_size_to_text($total_size);
			}
			...
			$basedir = $backupable_entities[$entity];
			$dirs = apply_filters('syllabplus_dirlist_'.$entity, $basedir);
			return self::recursive_directory_size($dirs, $syllabplus->get_exclude($entity), $basedir, $format);
		}

		// Default fallback
		return apply_filters('syllabplus_get_disk_space_used_none', __('Error', 'syllabplus'), $entity, $backupable_entities);
	}
	...
	public static function unzip_file($file, $to, $starting_index = 0) {
		...
		if (!$added_unzip_action) {
			add_action('syllabplus_unzip_file_unzipped', array('SyllabPlus_Filesystem_Functions', 'unzip_file_unzipped'), 10, 5);
		...
		if (!defined('SYLLABPLUS_USE_INTERNAL_PCLZIP') || SYLLABPLUS_USE_INTERNAL_PCLZIP) {
	...
	/**
	 * Called upon the WP action syllabplus_unzip_file_unzipped, to indicate that a file has been unzipped.
	 *
	...
	public static function unzip_file_unzipped($file, $i, $info, $size_written, $num_files) {
		...
		$record_every_indexes = (defined('SYLLABPLUS_UNZIP_PROGRESS_RECORD_AFTER_INDEXES') && SYLLABPLUS_UNZIP_PROGRESS_RECORD_AFTER_INDEXES > 0) ? SYLLABPLUS_UNZIP_PROGRESS_RECORD_AFTER_INDEXES : 1000;
		...
		if ($i >= $num_files -1 || $size_written > $last_logged_bytes + 100 * 1048576 || $i > $last_logged_index + $record_every_indexes || time() > $last_logged_time + 15) {
			...
			$syllabplus->jobdata_set($jobdata_key, array('index' => $i, 'info' => $info, 'size_written' => $size_written));
			
			$syllabplus->log(sprintf(__('Unzip progress: %d out of %d files', 'syllabplus').' (%s, %s)', $i+1, $num_files, SyllabPlus_Manipulation_Functions::convert_numeric_size_to_text($size_written), $info['name']), 'notice-restore');
			$syllabplus->log(sprintf('Unzip progress: %d out of %d files (%s, %s)', $i+1, $num_files, SyllabPlus_Manipulation_Functions::convert_numeric_size_to_text($size_written), $info['name']), 'notice');
			
			do_action('syllabplus_unzip_progress_restore_info', $file, $i, $size_written, $num_files);
	...
	private static function unzip_file_go($file, $to, $needed_dirs = array(), $method = 'ziparchive', $starting_index = 0) {
		...
		$class_to_use = ('ziparchive' == $method) ? 'SyllabPlus_ZipArchive' : 'SyllabPlus_PclZip';

		if (!class_exists($class_to_use)) require_once(SYLLABPLUS_DIR.'/includes/class-zip.php');
		
		$syllabplus->log('Unzipping '.basename($file).' to '.$to.' using '.$class_to_use.', starting index '.$starting_index);
		...
		// if ('SyllabPlus_PclZip' == $class_to_use) mbstring_binary_safe_encoding();
		// if ('SyllabPlus_PclZip' == $class_to_use) reset_mbstring_encoding();
		...
		*/
		...
		for ($i = $starting_index; $i < $num_files; $i++) {
			...
			if (isset($info['size']) && 0 == $info['size']) {
				...
				// SyllabPlus_PclZip::getFromIndex() calls PclZip::extract(PCLZIP_OPT_BY_INDEX, array($i), PCLZIP_OPT_EXTRACT_AS_STRING), and this is expensive when done only one item at a time. We try to cache in chunks for good performance as well as being able to resume.
				if ($i > $content_cache_highest && 'SyllabPlus_PclZip' == $class_to_use) {

					$memory_usage = memory_get_usage(false);
					$total_memory = $syllabplus->memory_check_current();
					...
					if (!empty($cache_indexes)) {
						$content_cache = $z->syllabplus_getFromIndexBulk($cache_indexes);
					}
				}
			...
			do_action('syllabplus_unzip_file_unzipped', $file, $i, $info, $size_written, $num_files);
