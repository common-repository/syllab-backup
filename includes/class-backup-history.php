 ...
 */
class SyllabPlus_Backup_History {
	...
	public static function get_history($timestamp = false) {
		...
		$backup_history = SyllabPlus_Options::get_syllab_option('syllab_backup_history');
	...
	public static function add_jobdata($backup_history) {
		...
		while ($any_more) {
			...
			foreach ($columns as $nonce) {
				...
				$columns_sql .= "'syllab_jobdata_".esc_sql($nonce)."'";
			}
			...
			foreach ($all_jobdata as $values) {
				// The 16 here is the length of 'syllab_jobdata_'
	 ...
	 *
	 * @uses SyllabPlus_Admin::include_template()
	 *
	...
	public static function existing_backup_table($backup_history = false, $backup_count = 0) {
		...
		if (!is_array($backup_history) || empty($backup_history)) return '<div class="postbox"><p class="syllab-no-backups-msg">'.__('You have not yet made any backups.', 'syllabplus').'</p> <p class="syllab-no-backups-msg">'.__('If you have an existing backup that you wish to upload and restore from, then please use the "Upload backup files" link above.', 'syllabplus').' '.__('Or, if they are in remote storage, you can connect that remote storage (in the "Settings" tab), save your settings, and use the "Rescan remote storage" link.', 'syllabplus').'</p></div>';

		if (empty($backup_count)) {
			$backup_count = defined('SYLLABPLUS_EXISTING_BACKUPS_LIMIT') ? SYLLABPLUS_EXISTING_BACKUPS_LIMIT : 100;
		}
		...
		return $syllabplus_admin->include_template('wp-admin/settings/existing-backups-table.php', true, $pass_values);
	...
	public static function build_incremental_sets($backup_history) {
		...
		$backupable_entities = array_keys($syllabplus->get_backupable_file_entities(true, false));
			...
			foreach ($backupable_entities as $entity) {
				...
				foreach ($bdata[$entity] as $key => $filename) {
					...
					if (preg_match('/^backup_([\-0-9]{15})_.*_([0-9a-f]{12})-[\-a-z]+([0-9]+)?+(\.(zip|gz|gz\.crypt))?$/i', $filename, $matches)) {
						...
						if (!empty($accepted) && (false != ($btime = apply_filters('syllabplus_foreign_gettime', false, $accepted, $filename))) && $btime > 0) {
	 ...
	 */
	public static function filter_syllab_backup_history() {
		global $wpdb;
		$row = $wpdb->get_row($wpdb->prepare("SELECT option_value FROM $wpdb->options WHERE option_name = %s LIMIT 1", 'syllab_backup_history'));
	...
	public static function always_get_from_db() {
		add_filter('pre_option_syllab_backup_history', array('SyllabPlus_Backup_History', 'filter_syllab_backup_history'));
	}
	
	/**
	 * This function examines inside the syllab directory to see if any new archives have been uploaded. If so, it adds them to the backup set. (Non-present items are also removed, only if the service is 'none').
	 *
	...
	public static function rebuild($remote_scan = false, $only_add_this_file = false, $debug = false) {
		...
		$syllab_dir = $syllabplus->backups_dir_location();
		if (!is_dir($syllab_dir)) return array("Internal directory path does not indicate a directory ($syllab_dir)");
		...
		foreach ($backup_history as $btime => $bdata) {
			...
			foreach ($bdata as $key => $values) {
				...
				foreach ($values as $filename) {
					...
					if (preg_match('/^backup_([\-0-9]{15})_.*_([0-9a-f]{12})-[\-a-z]+([0-9]+)?+(\.(zip|gz|gz\.crypt))?$/i', $filename, $matches)) {
						$nonce = $matches[2];
						if (isset($bdata['service']) && ('none' === $bdata['service'] || (is_array($bdata['service']) && (array('none') === $bdata['service'] || (1 == count($bdata['service']) && isset($bdata['service'][0]) && empty($bdata['service'][0]))))) && !is_file($syllab_dir.'/'.$filename)) {
		...
		if ($remote_scan) {
			...
			$syllabplus->register_wp_http_option_hooks(true);
			...
			foreach ($storage_objects_and_ids as $method => $method_information) {
				...
				if (!$object->supports_feature('multi_options')) {
					error_log("SyllabPlus: Multi-options not supported by: ".$method);
					continue;
				}
		...
		if (!$handle = opendir($syllab_dir)) return array("Failed to open the internal directory ($syllab_dir)");
		...
		while (false !== ($entry = readdir($handle))) {
			...
			if (preg_match('/^backup_([\-0-9]{15})_.*_([0-9a-f]{12})-([\-a-z]+)([0-9]+)?(\.(zip|gz|gz\.crypt))?$/i', $entry, $matches)) {
				...
				$itext = (0 == $index) ? '' : $index;
			} elseif (false != ($accepted_foreign = apply_filters('syllabplus_accept_foreign', false, $entry)) && false !== ($btime = apply_filters('syllabplus_foreign_gettime', false, $accepted_foreign, $entry))) {
				...
				$index = apply_filters('syllabplus_accepted_foreign_index', 0, $entry, $accepted_foreign);
				$itext = $index ? $index : '';
				$potmessage = array(
					'code' => 'foundforeign_'.md5($entry),
					'desc' => $entry,
					'method' => '',
					'message' => sprintf(__('Backup created by: %s.', 'syllabplus'), $accept[$accepted_foreign]['desc'])
				);
				...
				$potmessage = array(
					...
					'desc' => $entry,
					'method' => '',
					'message' => __('This file does not appear to be an SyllabPlus backup archive (such files are .zip or .gz files which have a name like: backup_(time)_(site name)_(code)_(type).(zip|gz)).', 'syllabplus').' <a href="https://syllabplus.com/shop/syllabplus-premium/" target="_blank">'.__('If this is a backup created by a different backup plugin, then SyllabPlus Premium may be able to help you.', 'syllabplus').'</a>'
				);
			...
			if ($btime <= 100) continue;
			$file_size = @filesize($syllab_dir.'/'.$entry);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
			...
			if (!isset($backup_nonces_by_filename[$entry])) {
				...
				if (is_array($only_add_this_file)) {
					...
					list ($mess, $warn, $err, $info) = $syllabplus->analyse_db_file(false, array(), $syllab_dir.'/'.$entry, true);// phpcs:ignore VariableAnalysis.CodeAnalysis.VariableAnalysis.UnusedVariable
					if (!empty($info['label'])) {
		...
		foreach ($remote_files as $file => $services) {
			...
			if (isset($backup_history[$btime]) && array('service_instance_ids') !== array_keys($backup_history[$btime])) {
				...
				$messages['nonnative'] = array(
					'message' => __('One or more backups has been added from scanning remote storage; note that these backups will not be automatically deleted through the "retain" settings; if/when you wish to delete them then you must do so manually.', 'syllabplus'),
					'code' => 'nonnative',
					'desc' => '',
					'method' => ''
				);
		...
		$more_backup_history = apply_filters('syllabplus_more_rebuild', $backup_history);
	...
	/**
	 * This function will look through the backup history and return the nonce of the latest full backup that has everything that is set in the SyllabPlus settings to be backed up (this will exclude full backups sent to another site, e.g. for a migration or clone)
	 *
	...
	public static function save_backup($backup_time, $backup_array) {
		...
		$backup_history[$backup_time] = isset($backup_history[$backup_time]) ? apply_filters('syllabplus_merge_backup_history', $backup_array, $backup_history[$backup_time]) : $backup_array;
