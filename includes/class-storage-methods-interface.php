 ...
 */
class SyllabPlus_Storage_Methods_Interface {
	 ...
	 *
	 * @return Object|WP_Error - an instance of SyllabPlus_BackupModule, or an error
	 */
	public static function get_storage_object($method) {
		...
		$method_class = 'SyllabPlus_BackupModule_'.$method;
		
		if (!class_exists($method_class)) include_once SYLLABPLUS_DIR.'/methods/'.$method.'.php';
	...
	public static function get_remote_storage_options_and_templates() {
		...
		$storage_objects_and_ids = self::get_storage_objects_and_ids(array_keys($syllabplus->backup_methods));
		$options = array();
		$templates = array();
		...
		foreach ($storage_objects_and_ids as $method => $method_info) {
			...
			if (!$object->supports_feature('multi_options')) {
				ob_start();
				do_action('syllabplus_config_print_before_storage', $method, null);
				do_action('syllabplus_config_print_add_conditional_logic', $method, $object);
				$object->config_print();
	 ...
	 *
	 * @return Array - returns an array, with a key equal to each member of the $services list passed in. The corresponding value is then an array with keys 'object', 'instance_settings'. The value for 'object' is an SyllabPlus_BackupModule instance. The value for 'instance_settings' is an array keyed by associated instance IDs, with the values being the associated settings for the instance ID.
	 */
	public static function get_storage_objects_and_ids($services) {
		...
		foreach ($services as $method) {
			...
			if (is_a($remote_storage, 'SyllabPlus_BackupModule')) {
				...
				if ($remote_storage->supports_feature('multi_options')) {
					...
					$settings_from_db = SyllabPlus_Options::get_syllab_option('syllab_'.$method);
					...
					if (is_wp_error($settings)) {
						if (!empty($settings_from_db)) error_log("SyllabPlus: failed to convert storage options format: $method");
					...
					if (empty($settings['settings'])) {
						...
						if (empty($settings['settings'])) {
							...
							error_log("SyllabPlus: Warning: settings for $method are empty. A dummy field is usually needed so that something is saved.");
						}
				...
				error_log("SyllabPlus: storage method not found: $method");
			}
		}
	...
	public static function update_remote_storage_options_format($method) {
		...
		if (!file_exists(SYLLABPLUS_DIR.'/methods/'.$method.'.php')) return new WP_Error('no_such_method', 'Remote storage method not found', $method);
		
		// Sanity/inconsistency check
		$settings_keys = $syllabplus->get_settings_keys();
		
		$method_key = 'syllab_'.$method;
		
		if (!in_array($method_key, $settings_keys)) return new WP_Error('no_such_setting', 'Setting not found for this method', $method);
	
		$current_setting = SyllabPlus_Options::get_syllab_option($method_key, array());
		...
		$already_active = true;
		$updated = SyllabPlus_Options::update_syllab_option($method_key, $new_setting);
	 ...
	 *
	 * @return Array					- returns an array, with a key equal to only enabled service member of the $services list passed in. The corresponding value is then an array with keys 'object', 'instance_settings'. The value for 'object' is an SyllabPlus_BackupModule instance. The value for 'instance_settings' is an array keyed by associated enabled instance IDs, with the values being the associated settings for the enabled instance ID.
	 */
	...
	public static function get_remote_file($services, $file, $timestamp, $restore = false) {
		...
		foreach ($services as $service) {
			...
			if ($restore) {
				$service_description = empty($syllabplus->backup_methods[$service]) ? $service : $syllabplus->backup_methods[$service];
				$syllabplus->log(__("File is not locally present - needs retrieving from remote storage", 'syllabplus')." ($service_description)", 'notice-restore');
			}

			$object = $storage_objects_and_ids[$service]['object'];

			if (!$object->supports_feature('multi_options')) {
				error_log("SyllabPlus_Storage_Methods_Interface::get_remote_file(): Multi-options not supported by: ".$service);
				continue;
			}
			...
			foreach ($backups_instance_ids as $instance_id) {
				...
				if (isset($instance_ids[$instance_id])) {
					...
					// If we didn't find a instance id match, it could be a new SyllabPlus upgrade or a wipe settings with the same details entered so try the default options saved.
				...
				if (is_readable($fullpath) && false !== $download) {
					if ($restore) {
						$syllabplus->log(__('OK', 'syllabplus'), 'notice-restore');
					} else {
						clearstatcache();
						$syllabplus->log('Remote fetch was successful (file size: '.round(filesize($fullpath)/1024, 1).' KB)');
						$is_downloaded = true;
					}
					break 2;
				} else {
					if ($restore) {
						$syllabplus->log(__('Error', 'syllabplus'), 'notice-restore');
					} else {
						clearstatcache();
						if (0 === @filesize($fullpath)) @unlink($fullpath);// phpcs:ignore Generic.PHP.NoSilencedErrors.Discouraged
						$syllabplus->log('Remote fetch failed');
					}
				}
			}
		}
	 ...
	 * @param SyllabPlus_BackupModule $service_object The object of the service to use to download with.
	 *
	...
	private static function download_file($file, $service_object) {
		...
		$syllabplus->log("Requested file from remote service: $service: $file");
		...
		if (method_exists($service_object, 'download')) {
			...
			$syllabplus->log("Automatic backup restoration is not available with the method: $service.");
			$syllabplus->log("$file: ".sprintf(__("The backup archive for this file could not be found. The remote storage method in use (%s) does not allow us to retrieve files. To perform any restoration using SyllabPlus, you will need to obtain a copy of this file and place it inside SyllabPlus's working folder", 'syllabplus'), $service)." (".SyllabPlus_Manipulation_Functions::prune_syllab_dir_prefix($syllabplus->backups_dir_location()).")", 'error');
			return false;
		}
